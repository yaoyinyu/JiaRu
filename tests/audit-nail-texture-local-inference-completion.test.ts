...
test("completion audit reports external evidence blockers instead of false completion", async () => {
  ...
  assert.equal(report.gates.bestCandidateMetrics.ok, true);
  assert.equal(report.gates.desktopAcceptance.ok, true);
  assert.equal(report.gates.representativeReleaseTest.actual, 13);
  assert.deepEqual(report.blockingInputs.map((item: { code: string }) => item.code), [
    "USER_FAILURE_CASES", "REPRESENTATIVE_RELEASE_TESTSET", "MOBILE_DEVICE_ACCEPTANCE", "USER_BETA_QUALITY_REVIEW",
  ]);
});
