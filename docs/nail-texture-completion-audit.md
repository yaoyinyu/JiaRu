...
## candidate52标注视觉拒绝与上线加速分层（2026-09-03）

candidate52的两轮SAM与人工v3/v4均未通过原分辨率视觉门。v4虽有20/20合法polygon、同图零交叠和20/20提示几何PASS，但逐甲2×复核仍发现皮肤污染、错位和真实甲板遗漏，因此该机器PASS不得提升为训练真值，3张/20甲继续`trainingUse=prohibited`。

上线加速只新增计划标记，不增加正式门PASS：先收敛失败polygon，再使用train内来源组隔离shadow-dev最多比较两项短程单变量配方；胜出后仅训练一个完整候选并执行一次正式val30。若仍因小目标或边界失败，才启动全图召回+单甲高分辨率ROI精修的预注册对照。OpenAI Image2没有教师张量、蒸馏报告或学生权重，生成候选图不构成蒸馏。机器重放为506个进度标记、462个PASS、44个非PASS；14个正式门仍为4通过、10失败，`ok=false`、`decision=hold`，报告SHA-256为`8d06e7bd8317d19fbb8b1c813511085fb82f3227152bd1a3ce94b8252a5b991b`。产品继续HOLD。

## candidate52 v8真值终结与OpenAI Image2蒸馏边界（2026-09-03）

candidate52 v8已用不可覆盖目录完成3张/3来源组/20甲的整图与逐甲2×原分辨率终审，机器复验为20个合法polygon、0交叠、提示几何20/20；v1—v7失败版本继续隔离且不计数。哈希绑定终结器已生成3份`ok=true`训练真值候选报告，但规范索引合并、训练数据物化和来源隔离深审尚未完成，因此不构成训练输入PASS或模型质量PASS。

OpenAI内置图像生成实际贡献的是3张候选源图及其终审后的20个硬polygon；项目仍没有OpenAI教师logit、特征层、软mask、边界置信度、蒸馏报告或由其产生的学生权重。严格OpenAI Image2蒸馏仍为“未发生”，不能把合成数据增强改名为蒸馏。最短路线是先形成预期328张/1981 mask/112来源组的规范索引并完成物化深审，再执行最多两项shadow-dev短程单变量对照、一个正式候选和一次val30；生产manifest与产品HOLD不变。

同步后的机器重放读取507个进度标记，其中464个PASS、43个非PASS；14个正式门仍为4通过、10失败，`ok=false`、`decision=hold`。报告SHA-256为`a25e3131691ae8ed077dd85fbf696e7be29da8251fb2add7dd26f89ddbd6657d`；新增真值视觉与终结PASS没有被误写为训练输入、候选质量或生产发布PASS。

## candidate52训练拒绝与candidate53两阶段计划（2026-09-03）

candidate52已完成规范索引、物化、输入深验和单一直接训练，但正式512 val30不存在同时满足128匹配、至多16漏检和至多16误检的阈值：保持128/16时误检为22，误检达到16时只剩126/18。因此该候选在val阶段拒绝，未读取test100、未执行边界晋级、导出、登记或部署；训练完成和数据输入PASS均未被误写为模型质量PASS。
...
## candidate55运行时入口、训练输入与启动状态（2026-09-04）

历史矩形验证128/16/16与浏览器真实方形输入不等价：方形输入在产品去重前为126/18/17、去重后为125/19/13，机器报告SHA-256为`cead6cab8d822c74e3f26769ae303a0a270242859839ef9f907e805d0cec5891`。该事实只关闭评测语义未知项，不增加正式模型门PASS。

candidate55 v2训练输入新增1736张train真实候选正ROI，合计15479正+52负，val保持144正+12负，test0、孤儿0；112个新增train来源组与val交叠0，27902个candidate53基础文件逐字节一致。独立审计SHA-256为`1d52635394ae5aec26a194bd3e540f88e1026353a7d50e5b4287cfe99d34596d`并批准仅供candidate55训练。v1的诊断计数不闭合，继续禁止使用。

candidate55训练在第10轮按patience6早停，最佳第4轮内部ROI mask mAP50/mAP50-95为0.88943/0.63843；最佳权重5,983,381 bytes、SHA-256 `5e47bd7f684b7e05139c2df3569104e7d0ac5fe254fab29642821a92546a1e1f`，训练后输入证据再次深验一致。浏览器一致完整图val30报告`8b96ff5b1148ae304518146c5029d555b36d81c15522b48fd9bb5c8ad3a045bc`在全0.10候选stage2筛选下最多122匹配/22漏检/30误检，正式拒绝且`testImagesRead=0`。

保守精修诊断在阈值0.85保留stage1的125匹配/19漏检/13误检，同时把2px边界F1提高到`0.6012338079`，已严格超过`0.6011288951`边界门。这不构成正式模型PASS，因为识别门仍差3个匹配；但它把后续工作收敛为512方形stage1召回修复，禁止继续用ROI内部指标替代完整图识别。正式模型、test100、ONNX、登记、浏览器、真机、Beta、困难负样本三变体和回滚门均未晋升，产品继续HOLD。

candidate55已从candidate53精修权重启动单一24轮、patience6短程训练，未启用蒸馏；训练中状态不构成val、test、ONNX、浏览器或发布PASS。完成后必须先在来源隔离val30执行512方形letterbox加产品去重的联合门，达到至少128匹配、至多16漏检/误检及严格边界改善后才允许读取test100。生产manifest未改，完成度仍为`decision=hold`。

## candidate56/57训练、选择锁与受保护test100结论（2026-09-05）

| 证据 | 结论 | 完成度影响 |
| --- | --- | --- |
| candidate56训练 | candidate52深审输入train488/val30/test0，512方形训练17轮早停；最佳权重`64628504…c272` | 训练工程PASS，不构成模型发布PASS |
| candidate56 val30 | 直接链无128/16/16合格阈值；锁定candidate55精修的联合链同样退化 | 两条链VAL FAIL，未读取test100 |
| candidate57 val30 | 固定0.40/0.30/0.55组合、stage1 polygon与产品去重后为129/15/16，边界F1 `0.6139459` | val选择PASS，只授权一次受保护test100 |
| 选择锁与运行器 | 锁`35374464…9910`绑定两套权重、实现、val证据、冻结快照和一次性政策；预测制品与质量报告均绑定锁 | 治理证据PASS，不构成质量PASS |
...
## 2026-09-09 循环011剖面后的开发真值完整性HOLD

`REL-CURRENT-DEVELOPMENT-002`继续为`running/pending`：循环011实例迁移和原分辨率复核已完成，但12图中发现5张源图排除、4张真值返修，当前开发split在全量重审及新版本修复前不能用于新模型选择。固定66图/405 mask队列初审12、余54、最终批准0；没有任何正式发布门新增PASS，也没有创建releaseIdentity。下一活动已切换为真值重审；上一轮记录中的循环011入口仅属历史。 **〔本段由 Codex 新增〕**

本轮深重放四份train内质量报告与新增实例迁移剖面；为保持受保护评估隔离，未运行完整completion audit的历史数据重放，也未执行新模型推理。只读核对磁盘最近总审计仍为2026-09-08T12:54:30.597Z、SHA-256 `6756cd635b3a14b17f9dc44218a85e129e4e35d4cd47fea11f64eaa6d6617903`，18门5通过/13失败、530标记、`ok=false`、`decision=hold`；该快照的步骤文字可能滞后，本页顶部当前要求和唯一Goal已同步。本轮不把旧报告当作新执行结果；只有同一releaseIdentity的全部当前发布门新重放为complete才允许完成Goal。 **〔本段由 Codex 新增〕**

## 2026-09-09 固定开发真值全量重审完成后的门状态

`REL-CURRENT-DEVELOPMENT-002`继续为`running/pending`。固定66图/405-mask开发正集已完成66/66张原分辨率审核，结论为8张源图排除、16张标注返修、42张未发现新增问题，待审0、最终批准0；结果报告`9769da17…52b9`与版本化处置清单`63b94764…07b5`均为`trainingUse=prohibited`。下一活动为16张替代polygon和58图开发真值重建，不能将全量重审完成误记为当前开发门PASS。 **〔本段由 Codex 新增〕**

本轮没有运行completion audit v3、没有读取旧val30/test100或发布留出，也没有模型训练、候选推理、ONNX导出、releaseIdentity或正式接口变更。磁盘最近总审计仍为2026-09-08报告`6756cd63…7903`的18门5通过/13失败、`ok=false`、`decision=hold`；产品HOLD不变。 **〔本段由 Codex 新增〕**

## 2026-09-09 58图开发真值v2通过后的门状态

`REL-CURRENT-DEVELOPMENT-002`继续为`running/pending`。16张替代标注已通过原分辨率终审，排除8张源图后形成58图/354-mask开发正图真值v2；物化报告`8d6b97b3…1b63`与独立重放`cb352cee…a010`均PASS，验证354个polygon合法、同图交叠0、42张延续标签语义一致、16张替代标签变化，且与开发训练折来源组/图片SHA交叠0。这只完成当前发布开发门内部的真值修复子步骤，不构成胜出配方、正式校准、发布留出或产品PASS。 **〔本段由 Codex 新增〕**

下一活动为冻结58正图加原40张困难负图的98图评估口径，并按循环011锁定的512/0.25/产品去重合同单次复评和逐实例剖面；随后才能预注册循环012唯一变量。旧66图/405-mask指标、历史失败和受保护留出保持原样。本轮未运行completion audit v3，磁盘最近审计仍是`6756cd63…7903`的18门5通过/13失败、`ok=false`、`decision=hold`；无`releaseIdentity`、生产ONNX或新增发布门PASS。 **〔本段由 Codex 新增〕**
