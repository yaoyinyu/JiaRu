...
| `M2-T3-RELEASE-TEST-ANNOTATION` | 新增真实发布测试素材逐甲标注与整图复核 | 🟡 IN PROGRESS | v6为101张生成852个候选，SAM2完成57张核心图/345提示且0失败；原分辨率复核暂通过8张/59 mask，48张返修，1张源图裁断排除，35张压力图待第二阶段处理；候选不得直接当作test真值 |
...
| `M3-T4-QUALITY` | 真实测试集直接可用率、污染率和人工修正成本 | 🟡 IN PROGRESS | 新增92张来源隔离发布测试候选已完成intake，核心图首轮仅8张/59 mask通过；48张返修、35张压力图待审。仍等待`USER-FAILURE-01`和完整Beta逐图质量审核，发布门禁拒绝把未审候选计为合格test |
...
| `USER-TESTSET-01` | 最终形成至少 100–200 张来源隔离的独立真实发布测试图 | 🟡 IN PROGRESS | 新增92张来源隔离候选已入队，但当前仅8张/59 mask通过核心图首轮复核，48张返修、35张压力图待审；未审候选不计入合格test规模 |
