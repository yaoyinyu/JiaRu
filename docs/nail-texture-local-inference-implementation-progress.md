...
| `REL-T2-CANDIDATE` | 正式模型候选发布 | 🔴 HOLD | 2026-07-24候选已通过冻结100张部署512 mAP门，规范val30正式校准得到候选阈值0.50；但困难负样本裁水印变体出现2/100误检且训练负样本不能充当独立留出。生产manifest保持不变，须先用新批次无统一水印的独立负样本证明泛化，再完成Beta、移动真机和回滚门 |
...
| `USER-HARD-NEGATIVE-01` | 补充独立、无统一水印的hard negative留出批次 | 🟡 ENGINEERING IN PROGRESS | 原160张已严审为158张并用于第二候选训练，不再具备留出资格。训练完成后已启动新来源、不同提示族的100张独立留出并行生成；现场已生成68/100张，但尚未完成全批哈希冻结和正式原分辨率终审，冻结数量仍为0/100。必须在首次v2推理前锁定全批身份，完成不少于100张原分辨率审核后才运行三变体零误检/零delta门 |
