...
| `REL-T2-CANDIDATE` | 正式模型候选发布 | 🔴 HOLD（candidate5已拒绝） | 第二候选因独立留出7/5/6张误检被否决，第三候选因规范val30联合门失败被否决，第四候选因新独立留出3/2/3张误检被否决；candidate5虽过规范val30和冻结test100，仍在另一批训练后新100张独立留出上产生3/4/3张误检、5/4/6个检测及1/1 delta，零误检/零delta门失败。生产manifest、共享阈值和正式ONNX保持不变；下一候选须使用隔离且获商业训练授权的新负样本重新训练，并在训练后再建另一份全新未见留出 |

## 当前总体验收

当前生产状态继续HOLD：candidate5训练后的编号361—460新100张独立发布留出已完成精确授权、原子freeze、100/100原分辨率终审、批准清单和首次三变体审计，但部署512、阈值0.50的原图/裁切/模糊角标分别误检3/4/3张、5/4/6个检测，检测数delta为1/1，candidate5正式拒绝。误检对象属于有效非美甲难例，不能删图、改阈值绕过或回流训练；批准清单已以holdout角色追加到registry v4。生产manifest继续指向未部署的正式ONNX；下一候选的新训练证据、训练后的另一份全新未见留出、移动真机、真实用户失败案例、Beta人工门、生产资产和回滚证据仍缺，不能用冻结正样本test100、640诊断或smoke模型绕过。
