...
> **当前最高优先级（2026-08-24）：** candidate18是当前最优直接识别候选。补强真值已扩至43张/287 mask，合并输入为253张/1497 mask正样本+160困难负样本、train413/val30/test0。candidate18从candidate17直接微调YOLO11m-seg且未启用教师/蒸馏；val30锁定512/0.30后为128/144匹配、16漏检、21误检。冻结test100为515/554匹配、461完整mask、39漏甲、14重复、25额外、22无效、22图漏甲、50图可直接提取，召回0.92960通过，但完整mask比例0.83213、缺甲图片率0.22和唯一性门继续HOLD。下一轮继续新增独立train角色完整甲面硬真值，禁止test逐图反调。
...
| `REL-T2-CANDIDATE` | 正式模型候选发布 | 🔴 HOLD（candidate6逐实例门失败；candidate7严格真值补强中） | candidate6冻结test100逐实例报告仅491/554匹配、漏63，41/100图漏甲，另有18重复、30额外候选和29无效mask，仅38/100图直接可提取，故正式否决。candidate7现有旧真实33张/237 mask与新正样本47张/250 mask，严格合计80张/487 mask，不少于80张的固定目标按权威机器索引已达成；新正样本索引（`2026_8_14_candidate7_new_positive_training_truths_v1`）与旧真实索引均`approved_unique_training_truth_index`、0拒绝/冗余/冲突且按文件名零重叠。未通过原分辨率门、存在应见甲数冲突或皮肤污染的版本不计数，全批在精确授权和物化审计前禁训。生产manifest和共享阈值不变；通过正样本门后仍须全新独立负样本、真机、Beta、生产ONNX和双版本回滚。 |
...
candidate57相对candidate50增加12个匹配与12个完整mask，减少12个漏甲、11个背景误检及12个无效mask，说明方形stage1路线显著缩短了正样本差距；但正样本质量门仍未完全通过。下一轮必须回到新的来源隔离train证据或独立于test的预注册训练改动，生产manifest与产品HOLD不变。
