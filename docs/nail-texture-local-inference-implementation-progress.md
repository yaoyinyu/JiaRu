...
| `M2-T3-REAL-DATA` | 导入授权真实图片并建立来源隔离测试集 | 🟠 PARTIAL | 新增113张已获商业训练与长期回归授权；78张/507 mask正式导入，4张因甲面截断或失焦排除，31张继续返修。真实实验集保持99图并以deerplanet 13图/102 mask为冻结独立test；样本量尚未达到100–200张代表性发布测试要求 |
| `M2-T3-VISION-ANNOTATION` | 识图提示 + SAM2/YOLO 辅助重建真实甲面多边形 | 🟡 IN PROGRESS | 跨分辨率共识已提升2张/9 mask；继续对35张做3×3分块推理得到19图/88候选但视觉门0提升。33个视觉紧框+SAM2复核4张后确认源像素截断/失焦，4张排除；第二批现为78通过、4排除、31返修 |
...
| `USER-ANNOTATION-01` | 修正真实图片的甲面多边形 | 🟡 IN PROGRESS | 首批21张/174个甲面已完成；新增批次已有77张正样本/507个甲面mask和1张hard negative通过，4张源图排除，剩余31张继续返修。当前无需用户逐点重画 |
| `USER-AUTH-02` | 确认 `真实素材/2026_7_12` 新增 113 张素材是否可用于商业模型训练和长期回归测试 | ✅ PASS | 用户于2026-07-12选择A，明确允许用于商业模型训练和长期回归；78张已导入、4张因源图质量排除、31张返修项仍隔离 |
