...
| `M2-T3-REAL-DATA` | 导入授权真实图片并建立来源隔离测试集 | 🟠 PARTIAL | 真实正样本train100张/521 mask、val30张/144 mask和冻结test100张/554 mask已严格审核与隔离。第四候选的新100张独立困难负样本也已完成精确授权、freeze、100/100原分辨率终审及三变体审计，但以3/2/3张误检被拒绝。数据治理链真实闭环；剩余PARTIAL指向下一候选的新训练证据、训练后另一份未见留出及Beta失败样本，不是本批缺授权或缺审核 |
...
| `M2-T3-CANDIDATE5-INDEPENDENT-HOLDOUT-043` | 候选5训练后全新未见独立困难负样本留出 | 🟠 PARTIAL（待新清单与精确授权） | 需另建不少于100张、未参与候选5训练/调参/错误筛选的新来源留出，完成精确独立发布测试授权、原分辨率终审和部署512阈值0.50的原图/裁角/模糊角标零误检与零delta审计。candidate4编号261—360和candidate5训练160张均永久排除复用 |
...
| `M3-T4-QUALITY` | 真实测试集直接可用率、污染率和人工修正成本 | 🟡 IN PROGRESS | 第四候选冻结test100部署512 full/core/stress正样本质量门通过，但新100张独立困难负样本三变体误检3/2/3张，已在进入Beta前被拒绝。逐实例直接可用率、污染率和人工修正成本仍需未来先通过独立留出门的候选执行Beta人工审核 |
...
| `M3-T5-BETA` | Beta 发布决策 | 🔴 HOLD | 第二候选被独立留出误检否决，第三候选被规范val阈值联合门否决，第四候选被新独立留出3/2/3张误检否决；均未获准进入Beta。移动真机矩阵、用户失败案例、100张Beta人工直接可用率/污染率/修正成本证据仍缺，禁止promotion |
...
| `REL-T1-COMPLETION-AUDIT` | 实施规范最终完成度机器审计 | ✅ PASS（HOLD生效） | 当前profile已切换并哈希绑定第四候选权重`2bde9702…2b09`、阈值0.35、冻结test100质量报告和新100张独立审计。显式Python 3.13重放确认独立报告深验通过但三变体误检3/2/3张，正式审计369个唯一标记/361 PASS、14门4通过/10 HOLD；移动真机、用户失败案例、Beta产品质量、生产ONNX和回滚继续阻断，未错误promotion |
...
| `REL-T2-CANDIDATE` | 正式模型候选发布 | 🔴 HOLD（第四候选已拒绝） | 第二候选因独立留出7/5/6张误检被否决，第三候选因规范val30联合门失败被否决；第四候选虽过val30和冻结test100，但在新100张独立留出上三变体误检3/2/3张，零误检门失败。生产manifest、共享阈值和正式ONNX保持不变；下一候选还须使用隔离且获训练授权的新证据重新训练，并依次完成val、冻结test、另一份全新未见留出、Beta、移动真机和回滚门 |

## 当前总体验收

当前生产状态继续HOLD：第四候选的新100张独立发布留出已完成精确授权、原子freeze、100/100原分辨率终审、批准清单和首次三变体审计，但部署512原图/裁切/模糊角标分别误检3/2/3张，候选正式拒绝。误检对象属于有效非美甲难例，不能删图、提阈值绕过或回流训练。生产manifest继续指向未部署的正式ONNX；下一候选训练证据、另一份训练后全新未见留出、移动真机、真实用户失败案例、Beta人工门、生产资产和回滚证据仍缺，不能用冻结正样本test100、640诊断或smoke模型绕过。
