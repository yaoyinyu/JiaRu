...
| `M2-T3-REAL-DATA` | 导入授权真实图片并建立来源隔离测试集 | 🟠 PARTIAL | 真实正样本train100张/521 mask、val30张/144 mask和冻结test100张/554 mask已严格审核与隔离；第二候选所用158张困难负样本及训练后110张独立留出均已闭环，但独立留出三变体7/5/6张误检否决第二候选。第三候选新训练负样本160/160机器通过，精确授权请求包已冻结；用户确认、正式原分辨率终审及至少100张schema v2批准尚未完成 |
| `M2-T3-VISION-ANNOTATION` | 识图提示 + SAM2/YOLO 辅助重建真实甲面多边形 | 🟡 IN PROGRESS | 正式集409图/2142 mask不变；外部首批train100张/521个完整mask、来源隔离val30张/144 mask和冻结test100张/554 mask均已完成原分辨率终审。第三候选困难负样本不含正样本polygon，当前160张已通过机器审计和授权前逐图原分辨率质量预筛，但精确授权后的正式审核工作区与训练批准仍按0计；Beta失败样本仍待补充 |
...
| `M2-T3-HARD-NEGATIVE-V3-TRAINING-010` | 第三候选来源隔离训练负样本、训练与新独立留出 | 🟡 ENGINEERING IN PROGRESS | 固定机器计划160张已全部实存，计划绑定外部目录、001—160文件身份、registry SHA-256和家族/变体唯一性。进度报告160/160机器通过、0缺失、0失败、0未知，SHA-256为`4251378c…ae7`；精确授权请求包`3ffdfce3…899`已冻结但仍待用户确认，正式原分辨率终审未开始，批准训练负样本仍为0/100。确认授权并至少批准100张后才能物化第三候选；训练后还需另建不少于100张全新独立留出。失败110张只保留诊断/长期回归，禁止训练和重复充当未见发布证据 |
...
| `M2-T3-COMPLETION-EVIDENCE-PROFILE-012` | 当前候选完成度证据profile与跨报告身份绑定 | ✅ PASS（工程门，真实结论HOLD） | 默认完成度命令通过repo内profile以绝对路径/SHA-256固定冻结test100、schema v2质量报告、第二候选110图独立留出审计和部署512指标，并绑定权重`e46968b7…3882`及阈值0.45。profile与四个受保护CLI覆盖互斥，拒绝缺项/未知项、相对路径、重复文件身份、哈希漂移及跨报告候选混用；加载和写报告前双重验哈希。当前默认审计为361标记/352 PASS、14门4通过；旧110图freeze因缺新版registry绑定只保留历史诊断，7/5/6张误检与第三候选未完成继续正确HOLD |
| `M2-T3-HARD-NEGATIVE-GENERATION-PROGRESS-013` | 第三候选160项机器计划与可恢复进度审计 | ✅ PASS（工程门，当前批次HOLD） | `audit-training-hard-negative-generation-progress.py`严格绑定160项计划、registry路径/SHA-256、文件命名、完整解码、768像素最短边、SHA/dHash隔离和家族/变体唯一性；允许只增不漂移的断点续跑，旧已完成图片字节变化会被拒绝。当前Git外快照160/160全部通过、0缺失、0失败、0未知，报告SHA-256为`4251378c…ae7`、逐项当前摘要为`ef99b63c…a3e`；精确授权请求包SHA-256为`3ffdfce3…899`，所有图片继续`trainingUse=prohibited`，授权状态为待用户确认 |
...
| `M3-T4-QUALITY` | 真实测试集直接可用率、污染率和人工修正成本 | 🟡 IN PROGRESS | 第二候选已绑定冻结100张/554 mask完成部署512 full/core/stress评估，box/mask mAP50分别为0.9501/0.9454、0.9508/0.9567、0.9472/0.9221，schema v2质量报告和深度重放通过；但训练后独立留出110张三变体7/5/6张误检已否决该候选。第三候选尚未训练；逐实例直接可用率、污染率和人工修正成本仍需Beta人工审核 |
...
| `M3-T5-BETA` | Beta 发布决策 | 🔴 HOLD | 第二候选虽通过冻结100张mAP门，但训练后独立留出110张三变体7/5/6张误检并被否决；第三候选训练及训练后全新独立留出尚未完成。移动真机矩阵、用户失败案例、100张Beta人工直接可用率/污染率/修正成本证据仍缺，禁止promotion |
...
| `REL-T2-CANDIDATE` | 正式模型候选发布 | 🔴 HOLD（第二候选已否决） | 第二候选已完成50轮训练、规范val30阈值0.45校准和冻结test100评估，但训练后独立留出110张的原图/裁角/模糊角标三变体仍误检7/5/6张，涉及9张独立图片，因此正式晋升失败。生产manifest、共享阈值和正式ONNX保持不变；必须训练第三候选并使用训练后新冻结的另一套独立留出证明泛化，再完成Beta、移动真机和回滚门 |

## 当前总体验收

`npm.cmd run audit:mvp-readiness:refresh` 的当前已落盘报告仍是旧候选与旧用户输入口径，只能作为历史诊断，不能覆盖本表的新证据。当前生产状态继续HOLD：第二候选已绑定冻结100张/554 mask完成部署512质量评估，却又在训练后独立留出110张上出现7/5/6张三变体误检并被否决；第三候选训练负样本160/160机器通过且授权前视觉预筛160/160通过，但精确清单仍待用户确认，正式终审、schema v2终结、物化和训练均未完成。生产manifest继续指向未部署的正式ONNX；移动真机、用户失败案例、Beta人工门、生产资产和回滚证据也仍缺，不能用640诊断或smoke模型绕过。
...
| `USER-HARD-NEGATIVE-V3-AUTH-02` | 授权第三候选最终冻结训练负样本精确文件清单 | ⏸️ USER INPUT（清单已就绪） | 001—160候选已补齐，精确授权请求包逐项冻结相对路径、文件名和SHA-256，包SHA-256为`3ffdfce3…899`。需用户明确允许该精确清单用于商业模型训练和长期回归、禁止作为独立发布测试，且授权不放宽质量门。目录级旧授权不能替代新批逐文件授权；用户确认前不得执行正式建账、终审、schema v2批准或物化 |
