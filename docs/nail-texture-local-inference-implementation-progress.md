...
| `M2-T3-CANDIDATE48-NEW-ANNOTATION-SHARD-001` | 11张新图SAM候选、人工返修与首批原分辨率终审 | 🟠 进行中（2张/10 mask通过，9张待返修） | 11张/94预期甲面均已建工作区；首批15个人工polygon合法、零交叠且几何15/15。原分辨率只接受`00664/00915`各5甲，`00062`透明拇指仅覆盖甲尖而返修。两张通过项尚未合入规范训练索引，物化前继续禁训。 |

candidate47证明单纯复用19张已有真值仍不能解决置信度下的召回—误检冲突。当前下一步是逐图修好candidate48剩余9张的完整甲面边界，形成足量且来源隔离的新真值后再物化和训练；产品继续HOLD。
