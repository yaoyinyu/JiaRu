...
| `M2-T2-DATA-GATE` | 数据结构、来源授权、split 与训练环境门禁 | ✅ PASS | 正式有效集 400 图/2092 个 mask、400 条来源记录，split=293/45/62；第二批通过项按 deerplanet/more/other 集合隔离，来源授权、标签、split 比例和训练 readiness 通过 |
...
| `M2-T3-REAL-DATA` | 导入授权真实图片并建立来源隔离测试集 | 🟠 PARTIAL | 新增 113 张已获商业训练与长期回归授权；78 张审核通过图/507 mask 正式导入，35 张继续返修。真实实验集更新为 99 图并保持 deerplanet 13 图/102 mask 为冻结独立 test；样本量尚未达到 100–200 张代表性发布测试要求 |
| `M2-T3-VISION-ANNOTATION` | 识图提示 + SAM2/YOLO 辅助重建真实甲面多边形 | 🟡 IN PROGRESS | 对37张返修图执行 v6 640/conf20 与1024/conf10跨分辨率共识筛选，得到34图/209个稳定候选；五页及原分辨率视觉审核仅2张/9 mask通过，拼图误检、皮肤污染、漏甲项继续隔离。第二批累计78图/507 mask通过，35张继续返修 |
...
| `USER-ANNOTATION-01` | 修正真实图片的甲面多边形 | 🟡 IN PROGRESS | 首批21张/174个甲面已完成；新增批次已有77张正样本/507个甲面 mask 和1张 hard negative通过审核，剩余35张继续返修。当前无需用户逐点重画 |
| `USER-AUTH-02` | 确认 `真实素材/2026_7_12` 新增 113 张素材是否可用于商业模型训练和长期回归测试 | ✅ PASS | 用户于2026-07-12选择A，明确允许用于商业模型训练和长期回归；78张审核通过项已导入，35张返修项仍隔离 |
