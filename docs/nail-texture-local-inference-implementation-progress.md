...
| `M2-T3-REAL-DATA` | 导入授权真实图片并建立来源隔离测试集 | 🟠 PARTIAL | 真实正样本train100张/521 mask、val30张/144 mask和冻结test100张/554 mask已严格审核与隔离；第三候选训练链完成后因val阈值联合门失败被拒绝。第四候选通过预注册插值恢复val门并通过冻结test100；训练后全新100张独立发布困难负样本已生成、主审和预授权隔离审计，但仍缺精确独立发布测试授权、不可变freeze、正式终审与三变体验证 |
...
| `M2-T3-CANDIDATE4-INDEPENDENT-PREFREEZE-019` | 全新100张独立发布困难负样本生成、主审核与预授权隔离审计 | 🟠 PARTIAL（待精确用户授权） | 100张AI图在任何第四候选推理前生成，主审核判退并替换1张练习甲片语义不明确图；最终Git外连续编号261—360，候选清单`bf2ed2db…f939`。预授权机器审计`429af514…dd47`确认100/100解码、0完全重复、0批内dHash≤12近重复，并与受保护registry的368条记录完成36,800次比较，精确SHA、规范sourceIdentity和dHash≤12重合均为0，记录清单`0bfed8c3…4511`。当前训练/评估用途均禁止，candidate4从未推理；缺用户对精确清单的独立发布测试等四项用途授权，故原子freeze、正式终审和三变体门尚未开始 |
| `M2-T3-CANDIDATE4-EXACT-AUTH-CONTRACT-020` | 新独立留出schema v2逐文件授权与冻结端双重匹配 | ✅ PASS（工程门，用户授权仍待确认） | 新增不可覆盖、可深度重放的精确授权构建器，只消费候选清单`bf2ed2db…f939`和预授权审计`429af514…dd47`，实跑锁定261—360共100项记录哈希`0bfed8c3…4511`、候选权重、阈值和受保护registry，并生成唯一确认文本。授权明确排除商业训练且保持训练禁用；冻结器在初次枚举和原子提交前两次要求当前100项与schema v2授权完全相同，历史schema v1仅兼容重放。独立链专项24/24、完整串行699/699通过；该工程PASS不代表用户已授权，当前仍未freeze或推理 |
...
| `M3-T4-QUALITY` | 真实测试集直接可用率、污染率和人工修正成本 | 🟡 IN PROGRESS | 第三候选已绑定冻结100张/554 mask完成部署512 full/core/stress评估，box/mask mAP50为0.9145/0.9059、0.9180/0.9180、0.9324/0.8946；绝对质量门通过但均低于第二候选，且规范val无可用阈值，故第三候选已拒绝。逐实例直接可用率、污染率和人工修正成本仍需未来合格候选的Beta人工审核 |
...
| `M3-T5-BETA` | Beta 发布决策 | 🔴 HOLD | 第二候选被独立留出误检否决，第三候选被规范val阈值联合门否决；没有候选进入新独立留出或Beta。移动真机矩阵、用户失败案例、100张Beta人工直接可用率/污染率/修正成本证据仍缺，禁止promotion |
...
| `REL-T1-COMPLETION-AUDIT` | 实施规范最终完成度机器审计 | ✅ PASS（HOLD生效） | 总门已绑定新100张冻结快照、新候选质量报告、部署512指标和独立困难负样本水印强门；Windows中文路径通过`PYTHONIOENCODING=utf-8`完成深度重放身份核对。困难负样本门不接受training角色或外层手写PASS，正式门增至14项；独立困难负样本、水印捷径、移动真机、失败案例、Beta、生产资产和回滚仍阻断，未错误promotion |
...
| `REL-T2-CANDIDATE` | 正式模型候选发布 | 🔴 HOLD（第四候选待新独立留出） | 第二候选因独立留出三变体7/5/6张误检被否决，第三候选因规范val30联合门失败被否决。第四候选权重`2bde9702…2b09`、阈值0.35，已通过规范val30和冻结test100 full/core/stress部署512质量门；全新100张独立发布困难负样本虽已生成并通过预授权隔离审计，但尚未获得精确用途授权、原子freeze、正式终审或三变体推理。生产manifest、共享阈值和正式ONNX保持不变；还须完成独立负样本、Beta、移动真机和回滚门 |

## 当前总体验收

`npm.cmd run audit:mvp-readiness:refresh` 的当前已落盘报告仍是旧候选口径，只能作为历史诊断，不能覆盖本表的新证据。当前生产状态继续HOLD：第三候选仍按规范val门失败结论拒绝；第四候选通过预注册权重插值恢复val联合门，并通过冻结test100的full/core/stress部署512质量门。全新100张独立发布困难负样本已在任何candidate4推理前生成、原图预筛、统一编号261—360并完成受保护registry预授权隔离审计；schema v2逐文件授权契约与冻结端双重完全匹配门也已通过工程验收，但用户尚未发出唯一精确确认文本，因此不可生成授权源、执行原子freeze、正式终审或首次推理。生产manifest继续指向未部署的正式ONNX；新独立留出三变体、移动真机、用户失败案例、Beta人工门、生产资产和回滚证据仍缺，不能用冻结正样本test100、640诊断或smoke模型绕过。
