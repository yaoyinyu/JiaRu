...
| `M2-T2-DATA-GATE` | 数据结构、来源授权、split 与训练环境门禁 | ✅ PASS | 323 图/1521 个自动初标 mask；323 条来源记录；Ultralytics 8.4.92、Torch CUDA 环境检查通过；22 张真实图的初标仍需人工复核 |
...
| `M2-T3-REAL-DATA` | 导入授权真实图片并建立来源隔离测试集 | 🟠 PARTIAL | 22 张真实图已获商业训练与长期回归授权并完成导入；同一来源组整体进入 train，不拆分伪造独立测试集；等待人工复核及新增独立来源批次 |
| `M2-T3-VISION-ANNOTATION` | 识图提示 + SAM2 重建真实甲面多边形 | 🟡 IN PROGRESS | 已识别 22 张原图并生成 178 个候选多边形及叠加预览；深色甲面边界较准，浅色透明甲和大饰钻存在过分割，当前候选隔离保存且未导入训练集，等待逐图二次修正 |
...
| `M3-T5-BETA` | Beta 发布决策 | 🔴 HOLD | 工程链路已具备，但缺正式真实数据模型、真机矩阵和人工质量验收，禁止提前 promotion |
...
`npm.cmd run audit:mvp-readiness:refresh` 已刷新权威报告：数据、来源授权、训练工具链、浏览器接线、反馈闭环、质量/性能门禁、发布治理与验证命令均通过；总状态仍为 `ok=false`，原因是生产 manifest 指向的正式 ONNX 尚未产生。此 HOLD 符合规范，不能用 smoke 模型绕过。
...
| 标记 ID | 所需协助 | 状态 | 使用阶段 |
| --- | --- | --- | --- |
| `USER-DATA-01` | 50–100 张可授权的真实美甲参考图 | 🟠 PARTIAL | 已导入 22 张授权真实素材；仍需至少 28 张，且应来自新的独立拍摄或来源批次 |
...
| `USER-ANNOTATION-01` | 修正 22 张真实图片的甲面多边形 | ⏭ USER INPUT | 初筛已完成，但 110 个 fallback 多边形均不得作为真值；需使用标注工具逐甲面重画并通过严格标注审计 |
