...
| `USER-HARD-NEGATIVE-01` | 补充独立、无统一水印的hard negative留出批次 | 🟡 ENGINEERING IN PROGRESS | 原160张已严审为158张并用于第二候选训练，不再具备留出资格。训练完成后已启动新来源、不同提示族的100张独立留出并行生成；当前仅得到4张未终审文件，不计入冻结数量。必须在首次v2推理前锁定全批身份，完成不少于100张原分辨率审核后才运行三变体零误检/零delta门 |
