...
| `USER-HARD-NEGATIVE-01` | 补充独立、无统一水印的hard negative留出批次 | 🟡 ENGINEERING IN PROGRESS | 用户已确认10张方向样图质量并授权Codex继续生成；新批已生成160张、20提示族×8场景，机器门160/160可解码、0完全重复、0感知近重复。当前模型已对全批执行误检筛选，因此本批后续若用于错误驱动训练，不能再作为下一候选的未见独立发布留出；逐文件原分辨率终审、训练角色冻结和新来源发布留出仍待完成 |
...
| `M2-T3-HARD-NEGATIVE-INDEPENDENT-GENERATION-003` | 160张新批次困难负样本生成、机器审计与当前模型筛选 | ✅ PASS（候选生成与诊断） | 160张全部保存到Git外部`困难负样本/独立输出/2026_7_24`，160/160解码、0完全重复、0感知近重复；视觉总览排除1张四指机械手，159张待逐文件原分辨率终审。候选权重在0.35阈值误检72张/174个候选，0.20阈值误检96张/257个候选；结论只证明候选难度和诊断价值，不授权训练、不满足独立发布留出或三变体零误检门 |
