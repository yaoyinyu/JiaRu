...
- [ ] 完善真实数据来源、授权、分组拆分和人工复核。（train100张/521 mask、val30张/144 mask和冻结test100张/554 mask均完成原分辨率审核与来源隔离；第二候选已使用158张批准困难负样本完成训练，并在规范val30与冻结test100上完成评估。训练后另行生成的110张独立困难负样本已在首次第二候选推理前完成冻结和原分辨率终审，110/110通过，但部署512三变体审计在原图/裁角/模糊角标上分别误检7/5/6张，涉及9张独立图片，第二候选因此被正式否决。第三候选新训练困难负样本计划160张已全部补齐，机器绑定进度审计160/160通过，报告SHA-256为`4251378c…ae7`；精确授权请求包`3ffdfce3…899`已冻结当前路径和SHA-256，但仍待用户确认，正式原分辨率终审和至少100张正式批准均未完成，不能把机器通过计作训练通过。训练后还需建立全新独立留出并通过三变体零误检/零delta门，同时继续完成Beta人工复核。完整露出甲面仍要求单一完整mask和原图尺寸视觉审核）
- [x] 训练 FP32 基线并评估输入尺寸。
...
- [ ] 为第三候选建立来源隔离的新训练困难负样本并完成新一轮发布泛化验证；固定机器计划160张已全部补齐，计划逐项绑定序号、文件名、提示族/变体、外部目录及受保护registry SHA-256。进度快照160/160完整解码、尺寸、命名、家族/变体唯一性、批内及registry SHA/dHash隔离通过，0缺失、0失败、0未知，报告SHA-256为`4251378c…ae7`；精确授权请求包SHA-256为`3ffdfce3…899`，逐项冻结当前相对路径和图片SHA-256。该请求仍待用户按精确清单确认，正式原分辨率终审尚未执行，训练批准继续为0/100。新训练批次针对独立留出暴露的贝壳、甲虫鞘翅、笔帽、安全反光片、梭形器件、玻璃镇纸、景泰蓝托盘、金属锦鲤和光盘反光等误检族扩展，但没有复制、裁剪、遮挡、重绘或提示复刻这110张留出。用户确认后先以显式白名单和权威受保护registry完成授权建账；registry SHA-256为`c533b3d9…3108`，精确绑定3份历史training与1份失败holdout清单并规范去重为368条保护记录，随后以SHA-256/规范sourceIdentity/dHash256完成隔离。再经严格原分辨率终审至少批准100张进入训练；第三候选训练、规范val30校准和冻结test100评估完成后，必须用新版registry强门另生成并在首次第三候选推理前冻结不少于100张的全新独立留出，达到三变体零误检/零delta后才允许晋升。旧110图freeze未绑定registry，只能保留为第二候选历史诊断。
