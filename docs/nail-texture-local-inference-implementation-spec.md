...
- [ ] 明确图片是否允许用于内部训练、商业发布训练和回归测试。（首批 22 张与 300 张 AI 图已确认；新增 113 张等待 `USER-AUTH-02`）
- [ ] 提供实际用户可能上传的典型失败案例。
...
- [ ] 完善真实数据来源、授权、分组拆分和人工复核。（首批已完成；新增批次等待授权、剩余标注与独立测试组冻结）
- [x] 训练 FP32 基线并评估输入尺寸。
- [x] 评估量化，但不牺牲纹理细边缘。（INT8 候选经门禁拒绝，FP32 保持默认）
- [ ] 建立真实设备性能和内存报告。（桌面工程冒烟已完成；Android/iPhone/iPad 真机矩阵等待执行）
