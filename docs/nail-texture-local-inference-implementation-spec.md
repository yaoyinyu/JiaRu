...
- [ ] 确认 MVP 首先只做单张上传图片的纹理抠图。
- [ ] 指定优先目标设备和最低设备档位。
- [ ] 提供首批 50–100 张真实参考图。
- [ ] 明确图片是否允许用于内部训练、商业发布训练和回归测试。
- [ ] 提供实际用户可能上传的典型失败案例。
- [ ] 在 Beta 阶段参与“直接可用/需要修正/不可用”审核。

### 16.2 工程侧需要完成

- [ ] 修复 Worker 环境判断和 ONNX Runtime 加载。
- [ ] 接入真实 ONNX 冒烟模型。
- [ ] 固定并版本化模型输入输出协议。
- [ ] 实现 letterbox 和精确坐标还原。
- [ ] 实现真实输出转置、Top-K、NMS 和按框 mask 解码。
- [ ] 优化 prototype 和 typed array 内存使用。
- [ ] 建立真实输出 fixture 与 Python 对照测试。
- [ ] 完善真实数据来源、授权、分组拆分和人工复核。
- [ ] 训练 FP32 基线并评估输入尺寸。
- [ ] 评估量化，但不牺牲纹理细边缘。
- [ ] 建立真实设备性能和内存报告。
- [ ] 将质量、性能和模型资产门禁接入发布决策。
- [ ] 保持 fallback、手动修正和回滚能力。
