...
- [ ] 完善真实数据来源、授权、分组拆分和人工复核。（正式训练集已有80张原图/516 mask及7张截图派生图/41 mask导入；新增92张独立发布测试父图均完成核心/压力首轮及三十二轮受审计处理，当前61张/352 mask暂通过、6张返修、25张源图裁断或遮挡排除；核心57张为45张通过、0返修、12排除，压力35张为16张/83 mask通过、6返修、13排除，尚未冻结为100–200张代表性test真值。完整露出甲面必须由单一完整mask覆盖；父截图完整但首轮派生区域选错时应重新提取并按父哈希/稳定来源组替换旧区域，不得误排除或重复计数；1001张Claude生成图在本批授权与逐图标注审核前仍仅作为合成候选池）
- [x] 训练 FP32 基线并评估输入尺寸。
