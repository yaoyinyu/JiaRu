...
- [ ] 完善真实数据来源、授权、分组拆分和人工复核。（新增实拍批次授权与80张/516 mask导入已完成，5张源图因甲面截断/失焦排除，28张待返修；独立真实test仍需扩充至100–200张代表性样本。另有1001张Claude生成图完成机器审计，但在本批授权与逐图标注审核前仅作为合成候选池）
- [x] 训练 FP32 基线并评估输入尺寸。
...
- [x] 对派生照片建立逐图 `sourceGroup` 的SAM2标注与机器审计流程；首批9张均有审核决定，7张/41 mask通过、2张返修，尚待安全导入正式集。
