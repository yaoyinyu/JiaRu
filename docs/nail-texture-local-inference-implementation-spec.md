...
## 26. candidate24/25否决与下一轮困难负样本扩充合同（2026-08-27）
...
- 下一直接学生候选不再用相同正样本和学习率反复训练。先扩充来源隔离的训练困难负样本，重点覆盖高光、透明、长椭圆且容易被误作甲面的非甲对象；不得读取冻结test100或独立发布留出的逐图结果选样。
- 当前固定160项困难负样本计划仍是候选池合同。每张必须先通过原分辨率源图质量审核，再通过计划、registry、SHA-256、dHash、解码、尺寸、命名和前序快照单调性审计；AI生成来源不放宽清晰度、完整性、单场景、无人手/甲面、无文字/水印/拼图和无结构错误门。
...
- 截至本节同步，磁盘与递归机器快照为70/160通过、90缺失、0失败、0未知；前四个softgel家族、`samara_dry_clusters`、`seed_pod_glossy_macro`和`seed_capsule_translucent`各10/10。透明蒴果仍需拒绝种子、绒毛或蒴果瓣连接关系不自然的生成版本，不能仅凭“透明轮廓像甲面”而放宽结构真实性。正式审核、schema v2终结、角色隔离与物化审计前保持`trainingUse=prohibited`；下一精确缺口为序号071 `winged_seed_mixed_scale`。

## 27. candidate26 本地甲面候选复核器合同（2026-08-28）

### 27.1 问题与目标

candidate24在来源隔离val30、部署512口径下以阈值0.25保持128/144个匹配实例，但产生27个误检；提高单一检测阈值会先损失真实甲面，无法严格优于candidate21的128匹配/19误检。candidate26因此预注册为“分割候选生成 + 小型甲面候选复核”的串行端侧链路：分割模型继续负责像素级mask，复核器只判断既有候选是否为真实完整甲面，不生成新mask、不修改mask边界，也不得把规则回退写成模型识别成功。
...
- 即使val30通过，candidate26仍只是候选；冻结test100、训练后全新独立困难负样本三变体零误检、浏览器、四类真机、Beta、产品质量和双版本回滚门全部保持不变。
...
- 因而该链路具备解决当前误检瓶颈的技术可能性，但是否真正改善必须由candidate26训练和val30联合重放证明；本文档不预先宣称PASS。
