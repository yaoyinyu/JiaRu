...
- [ ] 按schema v2唯一确认文本授权`candidate5-training-v2`最终冻结160张精确文件清单（`requestedItemsSha256=67e8b519…3b80`）的明确用途；当前回复未绑定清单摘要且用途范围不完整，未登记授权。
- [ ] 提供实际用户可能上传的典型失败案例。
...
- [ ] 等待用户按schema v2精确请求文本授权`candidate5-training-v2`最终冻结160张清单及其四种明确用途；授权后再执行正式A授权建账、原分辨率终审、schema v2批准、训练物化与GPU输入深审。授权、质量和输入门未清零前不得训练，也不得将本批改作独立发布测试。
