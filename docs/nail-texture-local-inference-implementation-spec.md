...
- [ ] 完善真实数据来源、授权、分组拆分和人工复核。（正式训练集已有80张原图/516 mask及7张截图派生图/41 mask导入；独立发布测试原67张/384 mask与新增33张/170 mask均完成完整甲面审核并合并冻结为100张/554 mask、core78/stress22、29个父来源组，训练用途固定prohibited。新快照及评估专用物化逐文件复算图片、annotation、联合和聚合哈希，与train100、val30及正式409图训练集按文件名、图片SHA-256、sourceGroup/parentSourceGroup均零重叠；代表性规模100/100已满足。v6在旧67张部署512口径的box/mask mAP50=0.8370/0.8313仍是拒绝结果，不能因扩充真值而改写；新100张须用于下一候选模型的冻结评估。正式训练输入仍因hard negative 1/100、缺99张而HOLD；完整露出甲面继续要求单一完整mask和原图尺寸视觉审核，1001张Claude生成图在本批授权与逐图标注审核前仍仅作为合成候选池）
- [x] 训练 FP32 基线并评估输入尺寸。
...
- [x] 完成正式训练与发布剩余依赖链的最终阻断审计；复验困难负样本正式化报告、完成度报告及全部非PASS标记，确认现有37张域外项已穷尽复筛且仅1张合格，train100、val30、冻结test100之外仍缺99张困难负样本。候选质量、生产ONNX、移动真机、Beta和回滚均依赖新训练或外部证据，不得用降低门槛、回收不合格素材或伪造传递报告制造进度；连续三轮无新输入时允许把持续目标标记为等待用户输入的阻塞态，收到合格素材后从原分辨率审核和候选物化门恢复。
