...
- [x] 验证真值唯一索引当前为6个批准报告、6张唯一图片/24个完整mask、0冗余、0冲突，SHA-256为`adac6dc3…33c7`；6项均绑定val角色且训练用途固定prohibited。该进度不等于整套验证集获批：剩余24张仍须返修，30/30全量通过、物化和来源隔离审计完成前禁止校准或候选训练。
