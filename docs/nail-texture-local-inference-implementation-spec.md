...
- 数据路径只做一次：`00710/00225`已形成2张/9 mask终审真值候选；`00624`原图事实把冻结批总数从39校正为38，其当前9 polygon虽机器合法且零交叠，但逐甲放大仍有透明延长甲和装饰外缘边界风险，保持返修。重新绑定后的candidate51中间索引v2仍为323张/1944 mask/107来源组（SHA-256 `2b179256…38d2`、规范条目`33580c22…a993`），并继续禁止训练直到物化审计。随后完成`00624/00625/00846`剩余3张/29 mask；失败版本隔离、不计数。全批终审与物化通过后只启动一个candidate51直接监督训练，不再复制candidate16蒸馏或candidate50插值实验。
