...
9. 下一学生采用“三类可核验增量”再训练，不等待全部返修清空，也不做无效小步重训：相对candidate9首轮至少新增10张且50枚原分辨率审核真值，并覆盖透明/裸色、镜面或高光、侧视/遮挡、多甲同框/复杂姿态中至少4类；同时绑定上述同甲唯一性后处理版本。第二轮最终21张/130 mask，相对首轮新增10张/55 mask并覆盖四类难例，已达到触发门。candidate10因自动学习率0.002造成val退化而在test前否决；candidate11以AdamW 0.0002和冻结前10层训练，并仅在val30锁定模型、阈值0.50与后处理0.60/0.85/0.12，但冻结test100仍有61漏甲、完整mask比例0.78700，正式HOLD。下一轮不得用test100选择参数；GPT-5.6 Sol只能从train角色教师工作区和抽象失败类型生成更多原分辨率真值课程，并允许在val30预先比较更强学生表征，禁止把test100图片、预测、标签或派生裁片回流训练。
