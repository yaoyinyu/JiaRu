...
- [ ] 完善真实数据来源、授权、分组拆分和人工复核。（正式训练集已有80张原图/516 mask及7张截图派生图/41 mask导入；新增92张独立发布测试父图均完成核心/压力首轮及三十四轮受审计处理，当前67张/385 mask暂通过、0张返修、25张源图裁断或遮挡排除；核心57张为45张通过、0返修、12排除，压力35张为22张/116 mask通过、0返修、13排除，审核返修队列清零但尚未冻结为100–200张代表性test真值。完整露出甲面必须由单一完整mask覆盖；SAM多轮提示仍合并皮肤/邻指时可切换原分辨率人工多边形，混合返修工具允许保留已审polygon并替换失败项，但仍须原图尺寸/来源组、合法性、零交叠、几何、整图与逐甲2×原图/叠加视觉审核；父截图完整但首轮派生区域选错时应重新提取并按父哈希/稳定来源组替换旧区域，不得误排除或重复计数；1001张Claude生成图在本批授权与逐图标注审核前仍仅作为合成候选池）
- [x] 训练 FP32 基线并评估输入尺寸。
- [ ] 完成2026_7_14新增1277张实拍候选的用途授权、近重复归并和逐图原分辨率完整甲面审核。（技术候选入库已完成：1277/1277可解码、196个来源组、批内0精确重复/70对近重复、跨1053张参照0精确重复/86对近重复；授权确认前`trainingUse=prohibited`，不计入训练或代表性test）
...
- [x] 基于已审计v8快照构建v9来源隔离实验集；7张/41 mask仅进入train/val，冻结13张/102 mask test逐文件哈希不变。v9冻结test box/mask mAP50=0.8411/0.8393，按0.85 box门及不退化门拒绝，v6保持最佳候选。
