...
- [ ] 完善真实数据来源、授权、分组拆分和人工复核。（train100张/521 mask、val30张/144 mask和冻结test100张/554 mask均完成原分辨率审核与来源隔离；626张AI困难负样本严格审核通过99张并与历史1张合并为100张，候选训练及冻结100图full/core/stress评估已完成。部署512三组mAP门通过，但同批AI图带系统性右下角标记，裁右下12%后出现2/100张误检且训练负样本不能作为独立留出。当前未完成项变更为：补充不同生成批次/提示族、无统一水印并与训练图无派生关系的独立困难负样本留出，继续完成Beta人工复核；完整露出甲面仍要求单一完整mask和原图尺寸视觉审核）
- [x] 训练 FP32 基线并评估输入尺寸。
...
- [ ] 补齐并冻结独立困难负样本留出集；候选必须在第二候选训练完成后才生成，来自不同生成批次/提示族，无统一水印，且不得是训练图的裁剪、遮挡、重绘或其他派生。当前已生成110/110张新来源候选，序号161—270连续、无缺号或重复序号且命名全部合规；但全批哈希冻结和正式原分辨率终审均未开始，正式冻结及质量批准仍按0/100计。必须在首次v2推理前锁定全批，以原分辨率审核严格排除低清、残缺、真人甲面、文字/水印和非目标域，不少于100张全部以`independent-holdout`角色运行同一三变体消融；未达到零误检/零变体delta前候选不得晋升。
