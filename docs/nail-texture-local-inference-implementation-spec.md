...
- [ ] 补齐并冻结独立困难负样本留出集；候选必须来自不同生成批次/提示族，最好无统一水印，且不得是训练图的裁剪、遮挡、重绘或其他派生。建议新提供150—200张候选以按同等原分辨率规则筛出不少于100张合格图，其中至少30张从未参与训练、阈值选择或返修决策并以`independent-holdout`角色运行同一消融；未达到零误检/零变体delta前候选不得晋升。
