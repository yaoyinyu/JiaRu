...
- [ ] 补齐并冻结独立困难负样本留出集；候选必须在第二候选训练完成后才生成，来自不同生成批次/提示族，无统一水印，且不得是训练图的裁剪、遮挡、重绘或其他派生。当前已开始并行生成100张新来源候选，仅4张未终审文件已落盘且不计入冻结数量。必须在首次v2推理前锁定全批，以原分辨率审核严格排除低清、残缺、真人甲面、文字/水印和非目标域，不少于100张全部以`independent-holdout`角色运行同一三变体消融；未达到零误检/零变体delta前候选不得晋升。
