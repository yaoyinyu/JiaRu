...
- [ ] 为第三候选建立来源隔离的新训练困难负样本并完成新一轮发布泛化验证；新训练批次必须针对独立留出暴露的贝壳、甲虫鞘翅、笔帽、安全反光片、梭形器件、玻璃镇纸、景泰蓝托盘、金属锦鲤和光盘反光等误检族扩展，但不得复制、裁剪、遮挡、重绘或提示复刻这110张留出。建议先生成120—180张候选，经严格原分辨率终审后至少批准100张进入训练；第三候选训练、规范val30校准和冻结test100评估完成后，必须再生成并在首次第三候选推理前冻结不少于100张的全新独立留出，达到三变体零误检/零delta后才允许晋升。
