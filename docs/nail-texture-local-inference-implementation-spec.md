...
- 在任何候选模型或旧mask辅助前，先从44张/14来源组模型无关库存冻结5张/4来源组/预计40枚完整甲面的源图；文件名清单SHA-256固定为`d1fe39422ca54deb12b435581a7beed433beaec2c21826e6091725b1135351a1`。该顺序证明选图不受candidate50/test100逐图输出驱动。
...
- 数据路径只做一次：优先修复能形成完整5甲的`00710`与`00225`，随后处理同源双手`00624/00625`及复杂双手`00846`；失败版本隔离、不计数。达到预登记的有效新真值增量后只启动一个candidate51直接监督训练，不再复制candidate16蒸馏或candidate50插值实验。
