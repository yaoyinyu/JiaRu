...
- standing project authorization取消逐清单等待，但不自动把生成候选改成可训练。候选池在正式审核、schema v2终结、角色隔离和物化审计完成前保持`trainingUse=prohibited`；失败生成版本只保留在生成缓存，不得复制进候选目录或计数。
...
- 截至本节同步，磁盘与递归机器快照为60/160通过、100缺失、0失败、0未知；前四个softgel家族、`samara_dry_clusters`和`seed_pod_glossy_macro`各10/10。正式审核、schema v2终结、角色隔离与物化审计前保持`trainingUse=prohibited`；下一精确缺口为序号061 `seed_capsule_translucent`。
