...
| AR 纹理试戴 | `/ar-tryon` | 待验证 | 上传含完整手背的参考图时优先通过MediaPipe Hands逐指定位；完整手部关键点失败后，可对包含2—5枚清晰已上色甲面的局部近景执行本地非皮肤甲面定位，并用同手空间聚类排除远端背景与贴边物体，再失败才进入人工补选。局部路径已覆盖黑色、彩色、低饱和银色及镜面高光甲面；形成4—5个连贯候选时显示为“已定位”，2—3个候选仍提示补充。两条辅助路径均没有像素级mask；实时AR支持拇/食/中/无/小指各自独立的长度75%—140%、宽度70%—145%及甲尖/指根位置±20%校准，摄像头最终贴合仍需多设备真机验收 |
...
| 美甲纹理自动识别 | `recognizeNailTextures()`、`detectNailsFromHandImage()`、`detectPartialCloseupNailCandidates()` | 进行中（精细模型阻塞；两级辅助定位可用） | 浏览器模型推理、Worker和后处理已实现，但candidate5因独立困难负样本门失败而拒绝，禁止切换生产权重或阈值。完整手部照片可由同源MediaPipe生成逐指候选；局部近景把非皮肤且未过曝的连续区域作为初始候选，从而保持低饱和银色与镜面高光甲面完整，再以面积、形状、纹理方差、皮肤邻接、贴边拒绝及空间簇筛选2—5枚。4—5枚为中置信、2—3枚为低置信；两条路径均没有像素级mask，不能替代正式分割模型质量结论 |
...
5. 手部几何定位成功时生成`source: "mediapipe"`候选并按五指索引自动分配；无完整手部或无可靠几何时，再调用`detectPartialCloseupNailCandidates()`尝试局部近景定位；
6. 局部近景路径先筛选面积、形状、颜色对比和平滑皮肤邻接合格且不接触图像边缘的已上色连通区域，再以源图最长边32%为相邻距离建立空间连通簇，只保留候选数最多、同数时总分最高的一簇并截取最多5个；4—5个候选标为`confidence: "medium"`，2—3个标为`confidence: "low"`，全部标为`source: "partial-closeup"`；不足2个时返回空数组并进入人工模式；
...
当前 `NailArtPicker` 将正式模型检测输入最长边限制在 800 像素，最多请求 10 个候选，Worker 超时为 15 秒。fallback原候选不会进入可确认区域；MediaPipe静态图检测超时为20秒，使用`modelComplexity: 1`、最多1只手、检测/跟踪阈值0.5。局部近景分析会再次把最长边限制为480像素；初始前景要求像素不属于皮肤且灰度低于225，连通域填充率下限0.32、平滑皮肤邻接下限0.24。只有同一空间簇至少2个候选时才整体返回，最多5个。该路径是本地几何候选生成器，不提供像素mask或正式模型质量承诺。人工新增区域默认按源图短边约8%宽、短边约16%长建立，并限制在原图边界内。候选确认后以 `NailAssignment[]` 返回，每个 `ImageBitmap` 的所有权转移给 `/ar-tryon` 页面，页面在替换或卸载时负责释放。
...
截至v1.1.270，上传完整手背参考图可通过本站同源MediaPipe资源自动生成五指候选；局部近景即使没有完整手部关键点，只要同图形成2—5枚清晰已上色甲面的连贯空间簇，也可生成候选。局部路径已针对低饱和银色与镜面高光的碎片化进行修复，本轮已复现的低饱和和提亮失效变体在真实浏览器均为5/5区域、5/5分配并完成五纹理提取，远端背景与底部贴边物体未进入候选。两条路径都只是几何级辅助定位，没有像素级甲面mask；透明/裸色/极小/单甲、与背景低对比或结构证据不足时仍会进入人工处理。正式精细自动识别继续受“无批准生产ONNX”阻塞，不能把辅助定位通过表述为candidate5或后续分割模型PASS。实时AR已启用高精度手部关键点、TIP/DIP/PIP深度尺度、手背平面横轴、侧转仿射剪切、低拖尾平滑和五指独立校准，自动几何/页面回归通过；但自动化环境没有取得真实摄像头帧，跨设备原分辨率视觉贴合仍未PASS。
