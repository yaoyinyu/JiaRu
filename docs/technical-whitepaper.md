...
| 正式纹理模型 | `/models/nail-texture-seg/manifest.json` | 阻塞 | v6 候选 ONNX 已在隔离导出目录通过精度、资产、协议和桌面性能门；92张真实发布测试父图已完成核心/压力首轮及三十三轮受审计处理，但目前仅63张派生/原图、365 mask通过视觉复核，尚不能替代冻结13张test或解除移动真机/Beta门 |
| 数据集治理 | `model/datasets/nail-texture-v1` | 进行中 | 正式训练集仍为409图、2142个mask，split=300/46/63。新增101张真实素材中9张跨旧批次精确重复排除；其余92张仅用于独立发布测试与长期回归，当前63张/365 mask暂通过、4张返修、25张排除，训练用途始终禁止。2026_7_14新增1277张实拍候选已完成解码、去重和196个来源组登记，但授权与逐图视觉审核待确认，训练用途固定为prohibited。1001张Claude生成图仍为未授权、未标注的合成候选池 |
...
- v6 是当前首个同时通过独立真实精度、模型资产、输出协议和桌面热性能门的正式候选，但冻结独立 test 仍仅13张。2026-07-13新增92张来源隔离真实发布测试父图均已完成核心/压力两路首轮处理及三十三轮受审计处理，累计仅63张/365 mask暂通过、4张返修、25张源图裁断或遮挡排除；核心返修队列虽已清零，但压力集仍未完成。在真值冻结与代表性审核完成前不得把“13+92”直接计为105张合格test。Android/iPhone/iPad 与 Beta 人工直接可用率也未验收，因此生产 promotion 保持 HOLD。
...
1. v6 正式候选已产出并通过当前精度/资产/桌面性能门，但生产 manifest 尚未切换；新增92张来源隔离发布测试父图已完成首轮及三十三轮受审计处理，但仅63张/365 mask暂通过，4张返修、25张源图裁断或遮挡排除，尚未形成冻结真值；2026_7_14的1277张新增实拍图仍待用途授权、近重复归并和逐图完整甲面视觉审核，不能直接补足代表性test。因此原13张独立真实test仍不足以替代100–200张代表性测试集；同时仍缺用户典型失败案例和至少100张Beta人工质量审核，正式模型审计继续blocked；
