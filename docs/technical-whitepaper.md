...
| 正式纹理模型 | `/models/nail-texture-seg/manifest.json` | 阻塞 | v6候选ONNX的资产、协议和桌面性能门通过，但冻结67张/384 mask在部署512下box/mask mAP50=0.8370/0.8313，box未达0.85且压力组退化，候选被拒绝；补足代表性规模的33张候选已完成身份隔离和首轮视觉审核，但26张仍需返修，正式快照仍为67/100，移动真机与Beta门也未解除 |
| 数据集治理 | `model/datasets/nail-texture-v1` | 进行中 | 正式训练集仍为409图、2142个mask，split=300/46/63。新增101张真实素材中9张跨旧批次精确重复排除；其余92张仅用于独立发布测试与长期回归，其中67张/384 mask冻结为受审核候选、25张排除、18个父来源组，训练用途始终禁止。2026_7_14目录1271张候选经近重复与26个来源组原子分片审核，565张进入待标注候选、601张排除；565张按来源组互斥规划为train 502、val 30、独立发布test 33。首批train已有100张唯一图片/521个完整mask、0冲突，达到最低正样本门。val经原图清退、整组替补和批次014—018返修后形成30张唯一图片/144个完整mask、0冗余、0冲突；整图与全部逐甲2×、polygon合法性和严格零交叠审核通过。val-only物化为30图/30 annotation/30 label、train/test均0、孤儿0，并与train 100张及冻结test 67张按文件名、图片SHA-256和来源组重算为零重叠；最终审计决定为`approved_as_calibration_truth`。33张独立发布测试候选已完成来源组替换和工作区物化，YOLO生成204个候选、SAM2完成204/204 mask，33/33原分辨率首审为7张通过、26张返修、0张排除。当前已有7张直通项与8张仅删误检项形成15张/76 mask候选真值索引，另1张拓扑修复候选完成，剩余17张已准备89个SAM返修提示但尚未运行GPU；15张既有候选仍须使用加固后的终结器重放并与其余18张一起完成冻结前审核，正式test继续按67/100计数。37张潜在hard negative逐图复核只保留1张安全候选；schema v2终结器已重放其审核、A授权、当前图片字节和隔离证据并输出HOLD（SHA-256 `c35214e9…65a4`），实际WEBP字节在未来物化时会把伪`.jpg`扩展安全改为`.webp`且不修改源文件。当前仍缺99张，正式候选训练继续HOLD。1001张Claude生成图仍为未授权、未标注的合成候选池 |
...
| 模型发布治理 | `scripts/*release*` | 进行中 | 已有注册、切换、回滚、质量门、逐图失败画像、规范val30阈值深审、三路候选编排、最终完成度审计和外部验收证据构建器；2026-07-22完成度审计v2复跑为333个进度标记/321个PASS、13个正式门中2个通过并输出HOLD。该2/13衡量完整发布闭环，不代表候选训练仅完成15% |
