...
| 候选5训练负样本 | `build-candidate5-hard-negative-generation-plan.py`、Git外`2026_7_29_candidate5_training_v2` | 进行中（60/160，训练禁止） | candidate4留出已由registry v2保护；160项失败类型计划固定。鱼饵三个提示族001—030与家具五金三个提示族031—060已逐图生成、完成生成阶段可见初筛和增量机器审计，60通过、0失败、0未知，最新报告`cbd2442c…94cb`。尚缺100张，且全批未获商业训练精确授权、未做正式原分辨率终审，不得进入训练 |
...
1. 第二候选因训练后独立留出三变体7/5/6张误检被否决，第三候选因规范val30联合门失败被否决。第四候选通过冻结test100的full/core/stress部署512质量门，但其100张新独立留出在三变体中产生3/2/3张误检，已被正式拒绝。candidate4留出现已单调追加至保护registry v2，永久禁止训练。候选5已依据鱼饵反光件、椭圆家具五金和长椭圆发夹三类失败建立160项新训练候选计划；当前60/160，鱼饵三个提示族与家具五金三个提示族已完成生成阶段初筛和机器门，仍缺100张，且未获商业训练精确授权。旧110图、145张训练清单和candidate4留出均不能替代新训练样本或下一候选的未见留出。同时仍缺用户典型失败案例、四类移动真机、至少100张Beta人工质量报告、生产ONNX与回滚证据。生产manifest、共享阈值和正式发布状态继续HOLD；
