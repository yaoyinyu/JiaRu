...
| 验证真值当前快照 | 外部`val-annotation-workspace-v1/validation-truth-index-v1.json` | 进行中 | v1.1.187：角色感知终审当前批准16个报告，归并为16张唯一图片/74个完整mask、0冗余、0冲突，索引SHA-256为`a583b884c3ca7af0f09000887fdb6eb3970bf772c73a65dd9c816984a7f6ef8d`。批次010的源图门拦截三张实际仅4甲、`01184`拇指甲触边和`00455`甲面被横向拇指遮挡，改用五甲完整的`00383/00351…_3`。SAM完成2图/10提示、0 fallback、几何10/10；视觉门保留8个干净polygon，人工重画点纹长甲与侧向拇指甲，并在逐甲2×首轮发现轮廓偏窄后扩大。终版两图10甲通过整图、全部逐甲2×、合法性、零交叠和几何审核。所有16项绑定`assignedRole=val`且`trainingUse=prohibited`；剩余14张仍须返修，30/30完成和物化审计前禁止阈值校准及候选训练 |
