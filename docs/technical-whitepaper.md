...
| 美甲纹理自动识别 | `recognizeNailTextures()` | 进行中 | 浏览器推理、Worker、后处理和 fallback 已实现；第二候选已用158张严格审核困难负样本完成续训，并通过规范val30、冻结100张/554 mask部署512 full/core/stress质量门，候选阈值为0.45。训练负样本三变体误检由上一候选的52/65/52张降为0/0/0，但该批已参与训练，仍缺训练后冻结的新来源独立留出，不能晋升生产 |
...
| 正式纹理模型 | `/models/nail-texture-seg/manifest.json` | 阻塞 | 第二候选权重SHA-256为`e46968b7…3882`。规范val30部署512 box/mask mAP50为0.8788/0.8364，阈值0.45经只读深验通过；冻结test100的full/core/stress为0.9501/0.9454、0.9508/0.9567、0.9472/0.9221，schema v2质量报告决定`accept_candidate_release`并深度重放通过。训练困难负样本158张的原图/裁切/模糊三变体均为0误检，但固定`releaseGeneralizationEligible=false`；且core box相对上一候选下降0.02073，略超跨候选0.02诊断线。候选继续HOLD，生产manifest和共享阈值不变，仍需新来源独立负样本留出、移动真机、Beta和失败案例门 |
| 数据集治理 | `model/datasets/nail-texture-v1` | 进行中 | 正式训练集仍为409图、2142个mask，split=300/46/63。第二候选规范包为train258（100张正样本/521 mask + 158张零字节标签困难负样本）、val30/144 mask、test0、孤儿0；物化报告SHA-256为`033b8d02…2ec`，文件树SHA-256为`8709fa80…1170`，GPU前输入深审SHA-256为`b9bd6897…94dd`，与冻结test100张/554 mask零重叠。2026-07-24批160张已按原分辨率严格审核为158张通过、2张排除，schema v2批准清单SHA-256为`5e91f8ec…a265`。该批已被上一候选筛选并用于第二候选训练，只能作为训练/诊断证据；训练后新来源独立留出候选已生成110/110张，序号161—270连续、无缺号或重复序号且命名全部合规，但尚未形成首次v2推理前的哈希冻结和正式逐图终审证据，正式冻结及质量批准仍按0/100计。必须先冻结全批、严格排除低清/残缺/真人甲面，并保留不少于100张通过三变体零误检/零delta门 |
...
| 2026-07-24 新批困难负样本与第二候选 | 外部`2026_7_24_hard-negative-v2`、候选权重SHA-256 `e46968b7…3882` | 评估完成（发布HOLD） | 158张批准负样本与100张正样本组成train258，val30保持来源隔离；从上一候选续训50轮完成。规范val30部署512为0.8788/0.8364并校准候选阈值0.45；冻结test100的full/core/stress为0.9501/0.9454、0.9508/0.9567、0.9472/0.9221，质量报告SHA-256为`b2b7167d…c16e`并通过深验。训练负样本在上一候选阈值下三变体误检52/65/52张、检测115/138/118个；第二候选阈值下三变体均0误检、0检测，训练抑制目标达成，但报告固定`releaseGeneralizationEligible=false`。core box相对上一候选下降0.02073，需保留为候选选择风险；训练后独立留出、设备、Beta和生产资产门仍未完成 |
...
| 候选三路训练发布编排 | `run-training-release-pipeline.ts --candidate-mode`、`verify-training-release.ts --candidate-mode` | 已完成（第二候选训练与评估完成） | 三套dataset与六份证据强隔离链已用于100正样本/158困难负样本/30验证图；50轮训练所得`e46968b7…3882`完成规范val、阈值0.45深验、冻结test100 full/core/stress与640诊断评估。当前未导出生产ONNX，原因是训练后独立负样本留出、移动真机、Beta和生产资产门未通过，而不是训练输入数量不足 |
...
| 训练/评估/导出 | `model/training/*` | 进行中 | 第二候选`e46968b7…3882`已完成50轮GPU训练、规范val30阈值0.45校准和冻结test100全量/core/stress评估；训练负样本三变体0误检，但跨候选core box下降0.02073且未见独立负样本留出尚未完成。候选未导出ONNX或登记发布，生产manifest不变 |
| 模型发布治理 | `scripts/*release*` | 进行中 | 已有注册、切换、回滚、质量门、逐图失败画像、规范val30阈值深审、三路候选编排、最终完成度审计和外部验收证据构建器。完成度审计现直接深验困难负样本水印证据并固定要求独立留出、100张、三变体零误检/零delta；现场审计为346个进度标记/335个PASS、14个正式门中4个通过并输出HOLD。剩余门包括水印/独立负样本、移动真机、失败案例、Beta产品质量、生产资产和回滚，不得把4/14解释为候选训练完成度 |
