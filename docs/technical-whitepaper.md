...
| 候选6正样本补强 | `record-positive-reinforcement-authorization.py`、`build-positive-reinforcement-annotation-workspace.py`及Git外`positive-reinforcement-annotation-workspace-v1` | 进行中（初审160/160；返修真值42张/176 mask） | 用户已对`requestedItemsSha256=1cdf2825…5bfe`的160张/76组/971枚可见甲面作精确商业训练授权，授权记录`67a17f2d…1b93`深验通过。8分片/80页首轮原分辨率审核为13张通过候选、147张返修、0排除。首批删误检返修终审4张/15 mask通过；第二批逐甲紧框仅1张/5 mask通过，两张透明低对比甲经人工多边形新增10 mask；后续20张人工/混合多边形新增90枚完整甲面，均通过polygon合法、同图零交叠、几何90/90及整图/逐甲2倍终审。另有`nail_00521…_1.jpg`因旧第5区域为白手套背景而哈希绑定为rework、未晋级。batch022完成`nail_00653…_3.jpg`返修但因已由真值018登记而不重复计数；batch023—025完成花纹/透明长甲、裸粉长甲与侧视透明拇指。batch026对`nail_00735…_3.jpg`保留4个完整甲面polygon、删除2个手链珠子误检并人工补齐顶端白色长甲。batch027复核`nail_00572…_7.jpg`：保留2枚完整旧polygon，重画横向透明长甲、左缘凹口长甲及漏标的侧视透明拇指；首版分别混入下方非甲面、保留凹口或漏红色装饰，经视觉门修订后5/5合法、零交叠、几何5/5并通过整图/逐甲2倍终审。唯一训练真值索引现为42张/176 mask、0拒绝/冗余/冲突，权威与重放SHA-256均为`85a564c9…a51a`；索引仍明确`trainingUse=prohibited-until-materialization-audit`。正样本水印不自动导致排除：仅当文字/水印遮挡或触碰甲面、破坏清晰度或完整轮廓时排除；未接触甲面的水印须记录来源风险并通过来源隔离，禁止将其当作识别捷径。该澄清不放宽困难负样本水印消融与独立发布留出门。其余118张尚未形成最终真值，完整来源隔离和训练物化尚未完成，禁止训练、val调参、冻结test推理、导出或部署 |
