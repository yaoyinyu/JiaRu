...
1. v6资产、协议和桌面性能证据有效，但冻结67张/384 mask在部署512下的扩展质量门失败，正式拒绝发布；部署阈值画像进一步确认压力组召回仅0.6983，透明相邻长甲、多甲同屏、低对比漏整甲/局部识别及手指/腕表误检是首要方向。候选训练数据门当前为train 100/100张、521个完整mask，val 30/30张、144个完整mask并已通过规范物化、校准真值终审及与train/冻结test的三类身份零重叠；hard negative严格复筛后仅1/100张安全候选，尚未正式物化且缺99张。按三类最低图片数量为131/230，约57%，但训练真值索引仍固定`trainingUse=prohibited-until-materialization-audit`，完整候选训练包、来源隔离终审和新val证据桥接报告尚未形成，因此不得启动正式候选训练。现有候选训练代码还没有把train不少于100张与hard negative不少于100张编码为强制门，属于待补齐的治理缺口；不能利用该缺口绕过文档标准。同时仍缺33张代表性发布test、用户典型失败案例、四类移动真机和至少100张Beta人工质量审核，这些阻塞正式发布，但不替代上述候选训练输入门；
