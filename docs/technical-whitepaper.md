...
| 美甲纹理自动识别 | `recognizeNailTextures()` | 进行中 | 浏览器推理、Worker、后处理和 fallback 已实现；2026-07-24候选已通过冻结100张/554 mask的部署512 full/core/stress mAP门，但AI困难负样本裁水印消融出现2/100张误检且没有独立负样本留出集，仍无可晋升生产的模型 |
...
| 训练/评估/导出 | `model/training/*` | 进行中 | v4、v5、v7、v8、v9历史候选均被拒绝；2026-07-24候选`bcb145b5…1a50`已通过冻结100图部署512 mAP门，full/core/stress box/mask mAP50为0.9654/0.9601、0.9715/0.9718、0.9436/0.9240。候选尚未导出ONNX或登记发布，因为困难负样本裁水印消融失败且缺独立留出 |
| 模型发布治理 | `scripts/*release*` | 进行中 | 已有注册、切换、回滚、质量门、逐图失败画像、规范val30阈值深审、三路候选编排、最终完成度审计和外部验收证据构建器。冻结质量报告schema v2已绑定新100图候选并深度重放通过；Windows中文路径身份核对通过显式`PYTHONIOENCODING=utf-8`修复。现场审计为343个进度标记/332个PASS、13个正式门中4个通过并输出HOLD；剩余门包括水印/独立负样本、移动真机、失败案例、Beta产品质量、生产资产和回滚，不得把4/13解释为候选训练完成度 |
...
1. 2026-07-24候选已使用通过深审的train100张/521 mask、困难负样本100张/空标签和val30张/144 mask完成续训；冻结test100张/554 mask在部署512的full/core/stress mAP门及schema v2深度重放通过。当前首要阻塞转为困难负样本泛化：训练负样本原图和右下角模糊版在0.35部署阈值为0误检，但裁去右下12%后出现2/100张误检，说明固定生成器标记可能成为捷径；训练集本身固定`releaseGeneralizationEligible=false`。必须补充不同批次/提示族、最好无统一水印且与训练图无派生关系的新候选，建议150—200张以严格筛出不少于100张，其中至少30张冻结为从未参与训练、阈值或返修的独立留出，并在同一消融达到零误检/零变体delta。除此之外仍缺用户典型失败案例、四类移动真机、至少100张Beta人工质量报告、生产ONNX与回滚证据；这些门均不得用正样本mAP替代。生产manifest、共享阈值和正式发布状态继续HOLD；
...
本轮候选训练和冻结100张评估已经完成，不需要等待565张待标注候选全部标注。下一轮训练是否启动取决于独立困难负样本留出结果：若新留出在原图、裁角和模糊角标三种变体上保持零误检/零delta，可以继续阈值、导出与Beta；若仍失败，应把误检样本及同类新训练样本按来源组加入训练后再训，但独立留出本身永远禁止训练和调参。正式发布还需满足300—500张Beta真实正样本、至少200张负样本总体规模、移动真机、用户失败案例和100张Beta人工质量门。冻结正样本mAP通过不得被表述为整体发布通过，完整发布审计通过率也不得反向解释为模型质量完成度。
