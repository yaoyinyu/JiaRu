...
| 美甲纹理自动识别 | `recognizeNailTextures()` | 进行中 | 浏览器推理、Worker、后处理和 fallback 已实现；第二候选已用158张严格审核困难负样本完成续训，并通过规范val30、冻结100张/554 mask部署512 full/core/stress质量门，候选阈值为0.45。训练负样本三变体误检由上一候选的52/65/52张降为0/0/0；但训练后独立留出110张在原图/裁角/模糊角标上仍有7/5/6张误检，第二候选正式晋升失败，必须以新来源训练负样本训练第三候选 |
...
| 正式纹理模型 | `/models/nail-texture-seg/manifest.json` | 阻塞 | 第二候选权重SHA-256为`e46968b7…3882`。规范val30部署512 box/mask mAP50为0.8788/0.8364，阈值0.45经只读深验通过；冻结test100的full/core/stress为0.9501/0.9454、0.9508/0.9567、0.9472/0.9221，schema v2质量报告决定`accept_candidate_release`并深度重放通过。训练困难负样本158张的三变体均为0误检，但训练后独立留出110张的原图/裁角/模糊角标分别误检7/5/6张，第二候选不得晋升。第三候选训练负样本160项机器计划已补齐，进度审计160/160机器通过；精确授权请求包已冻结当前路径与SHA-256，但尚待用户确认，正式原分辨率审核也尚未执行。生产manifest和共享阈值不变 |
| 数据集治理 | `model/datasets/nail-texture-v1` | 进行中 | 正式训练集仍为409图、2142个mask，split=300/46/63。第二候选规范包为train258（100张正样本/521 mask + 158张零字节标签困难负样本）、val30/144 mask、test0、孤儿0；物化报告SHA-256为`033b8d02…2ec`，文件树SHA-256为`8709fa80…1170`，GPU前输入深审SHA-256为`b9bd6897…94dd`，与冻结test100张/554 mask零重叠。训练后独立留出110张固定`trainingUse=prohibited`，不得回流训练或继续充当第三候选未见留出。第三候选外部计划逐项绑定001—160文件身份、家族/变体、registry路径及SHA-256；160张全部通过命名、完整解码、768像素、批内与受保护全集SHA/dHash隔离，0缺失、0失败、0未知，机器进度报告SHA-256为`4251378c…ae7`。精确授权请求包SHA-256为`3ffdfce3…899`，当前仍为待用户确认，训练批准仍按0/100。权威受保护注册表SHA-256为`c533b3d9…3108`，绑定3份历史training清单和1份失败holdout清单，4份manifest深验通过并规范去重为368条保护记录；授权不替代质量终审 |
...
| 规范val30阈值校准门 | `evaluate.py`、`calibrate-model-score-threshold.py` | 已完成（第二候选阈值0.45） | 规范30图/144 mask终审、角色隔离与当前文件字节被独立重放；第二候选`e46968b7…3882`部署512 box/mask mAP50为0.8788/0.8364，正式决定`calibrated_threshold_ready_for_candidate_manifest`、`manifestScoreThreshold=0.45`，报告SHA-256为`36a41ebd…26b1`并通过`--verify-report --expected-weights`重放。该值只授权候选，不修改生产manifest或共享阈值 |
...
| 候选三路训练发布编排 | `run-training-release-pipeline.ts --candidate-mode`、`verify-training-release.ts --candidate-mode` | 已完成（第二候选被独立留出拒绝） | 三套dataset与六份证据强隔离链已用于100正样本/158困难负样本/30验证图；50轮训练所得`e46968b7…3882`完成规范val、阈值0.45深验、冻结test100 full/core/stress与640诊断评估。训练后独立负样本留出110张已完成冻结、终审和三变体推理，但因7/5/6张误检决定HOLD。第三候选开训不缺正样本、val、GPU、编排代码或候选数量；当前硬缺口为用户确认冻结的160项精确授权清单、正式原分辨率终审、schema v2终结和规范训练集物化 |
...
| 训练/评估/导出 | `model/training/*` | 进行中 | 第二候选`e46968b7…3882`已完成50轮GPU训练、规范val30阈值0.45校准和冻结test100全量/core/stress评估；训练负样本三变体0误检，但独立留出110张在三变体上仍有7/5/6张误检，且跨候选core box下降0.02073。第二候选未导出ONNX或登记发布，生产manifest不变；下一阶段为新训练负样本治理和第三候选训练 |
...
第三候选160项外部计划已全部补齐。`candidate3-training-v1-progress-160.json`只读深验得到160通过、0缺失、0失败、0未知，报告SHA-256为`4251378c…ae7`，逐项当前摘要为`ef99b63c…a3e`。授权请求包`candidate3-training-v1-exact-authorization-request.json`逐项冻结相对路径和当前图片SHA-256，包SHA-256为`3ffdfce3…899`，但它本身不构成用户授权。授权前以4个互斥分片逐张查看原分辨率图片，160张均通过清晰度、主体完整性、无人手/真人甲面、无文字水印/品牌标志及无明显AI结构缺陷预筛；该预筛不替代授权后的正式审核工作区、逐项决定、schema v2终结或训练资格，全部图片继续`trainingUse=prohibited`。

独立AI困难负样本先使用`build-independent-hard-negative-review-workspace.py`构建原分辨率审核契约：输入必须同时包含A授权、机器审计、当前train/val真值索引和冻结test清单；脚本逐文件复验路径、SHA-256、尺寸、完整解码和三角色身份隔离，并生成不缩放的1:1像素审核页与空白决定CSV。审核人员必须另存完成版CSV，禁止覆盖空白模板。`finalize-independent-hard-negative-review.py`随后重放授权、机器审计、受保护角色、审核页和原图当前字节；通过项必须有审核说明且不得带缺陷码，排除项必须填写受控缺陷码。该终审只生成candidate-only清单，仍由`finalize-reviewed-hard-negative-manifest.py`深验并达到100张后才可形成schema v2训练批准清单。若该批已被当前模型用于错误筛选且结果会影响训练，它不得再承担下一候选的未见独立发布留出角色。
...
1. 第二候选已使用train100张/521 mask、困难负样本158张/空标签和val30张/144 mask完成50轮续训；冻结test100张/554 mask的部署512 full/core/stress mAP门及schema v2深度重放通过，规范val30校准阈值为0.45。训练后独立留出110张已在首次第二候选推理前冻结并完成原分辨率终审，但三变体仍误检7/5/6张，故第二候选被正式否决。第三候选固定计划160张现已全部实存并通过机器审计，进度报告SHA-256为`4251378c…ae7`；精确授权请求包`3ffdfce3…899`已经冻结160项当前路径和SHA-256，但仍待用户按精确清单确认，正式原分辨率终审和不少于100张schema v2批准均未完成。训练后还必须通过新版registry隔离强门另建不少于100张全新独立留出并达到三变体零误检、零delta；旧110图freeze缺少registry绑定，只能作为第二候选历史诊断。除此之外仍缺用户典型失败案例、四类移动真机、至少100张Beta人工质量报告、生产ONNX与回滚证据；这些门均不得用正样本mAP或val阈值替代。生产manifest、共享阈值和正式发布状态继续HOLD；
