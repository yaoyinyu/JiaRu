...
| 数据集治理 | `model/datasets/nail-texture-v1` | 进行中 | 正式有效集当前为 322 图、1585 个 mask；首批 21 张授权实拍图的 174 个审核甲面已全部导入。新增 113 张实拍素材已有 70 张正样本和 1 张 hard negative 形成隔离审核包（471 个 mask），尚未取得该批次正式训练授权，未并入正式数据集 |
...
2026-07-12 实拍数据同步：首批素材的有效来源目录已由误写的 `真实素材/2027_7_11` 纠正为 `真实素材/2026_7_11`。用户明确禁用的 `ca2d14573bfe3c0c83cdc3100e074b3c.jpg` 已从有效图片、原始标注、YOLO 标签、来源表和 split 中移除；其余 21 张审核通过图片共 174 个甲面已覆盖旧 fallback 标注并通过来源审计与标签审计。第二批 `真实素材/2026_7_12` 共 113 张已完成可解码、重复项和视觉初筛；v2/v3 辅助模型经逐图视觉复核后，已有 70 张正样本和 1 张 hard negative 组成隔离审核包，共 471 个 mask，标签审计为 0 错误和 1 条允许的空负样本 warning。该批次尚未获得正式训练与长期回归授权，因此不得并入正式数据集或用于正式候选模型。
