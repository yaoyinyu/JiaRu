...
1. v6资产、协议和桌面性能证据有效，但冻结67张/384 mask在部署512下的扩展质量门失败，正式拒绝发布；部署阈值画像进一步确认压力组召回仅0.6983，透明相邻长甲、多甲同屏、低对比漏整甲/局部识别及手指/腕表误检是首要方向。代表性下限仍缺33张。2026_7_14当前1271张候选已获A授权并完成近重复视觉归并，105张重复或非照片候选排除；剩余1166张中911张已完成源图筛选、457张仅进入待标注候选、454张排除，另255张仍待逐图完整甲面视觉审核，全部仍需来源组互斥分配且不能直接用于训练或补足test；同时仍缺用户典型失败案例、四类移动真机和至少100张Beta人工质量审核，正式模型审计继续blocked；
...
下一轮有效候选训练不需要等待全部1166张候选完成标注，也不受移动真机或Beta发布证据阻止；其启动口径是先完成剩余6个源图质量分片，再从审核通过项中按完整来源组形成互斥数据闭环。首批至少新增100—200张完整mask经原分辨率审核通过的真实正样本、约100张hard negative、至少30张合格且来源隔离的val，并从新来源补足33张独立发布test达到100张代表性下限后，即可物化数据并启动下一轮候选训练。正式发布仍需继续满足300—500张Beta真实正样本、至少200张负样本、移动真机、用户失败案例和100张Beta人工质量门；候选训练启动不得被表述为发布通过。
