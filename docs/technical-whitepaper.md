...
| 正式纹理模型 | `/models/nail-texture-seg/manifest.json` | 阻塞 | v6 候选 ONNX 已在隔离导出目录通过精度、资产、协议和桌面性能门；新增92张真实发布测试候选已完成来源隔离，但仅8张/59 mask通过首轮标注复核，尚不能替代冻结13张test或解除移动真机/Beta门 |
| 数据集治理 | `model/datasets/nail-texture-v1` | 进行中 | 正式训练集仍为409图、2142个mask，split=300/46/63。新增101张真实素材统一命名并保留原名/来源/哈希映射，9张跨旧批次精确重复已排除；92张获准仅用于独立发布测试与长期回归，按57张核心图、35张压力图隔离，禁止进入训练集。1001张Claude生成图仍为未授权、未标注的合成候选池 |
...
- v6 是当前首个同时通过独立真实精度、模型资产、输出协议和桌面热性能门的正式候选，但冻结独立 test 仍仅13张。2026-07-13新增92张来源隔离真实发布测试候选后，首轮核心图仅8张/59 mask通过、48张返修、1张源图裁断排除、35张压力图待审；在标注真值与代表性审核完成前不得把“13+92”直接计为105张合格test。Android/iPhone/iPad 与 Beta 人工直接可用率也未验收，因此生产 promotion 保持 HOLD。
...
1. v6 正式候选已产出并通过当前精度/资产/桌面性能门，但生产 manifest 尚未切换；新增92张来源隔离发布测试候选仍处于标注复核阶段（8张/59 mask暂通过、48张返修、1张源图裁断排除、35张压力图待审），因此冻结13张独立真实test仍不足以替代100–200张代表性测试集；同时仍缺用户典型失败案例和至少100张Beta人工质量审核，正式模型审计继续blocked；
