...
| 美甲纹理自动识别 | `recognizeNailTextures()` | 进行中 | 浏览器推理、Worker、后处理和 fallback 已实现。第二、第三候选已分别因独立留出误检和规范val30联合门失败被否决；第四候选通过哈希绑定的候选2/3权重插值恢复了val30联合门并通过冻结test100的full/core/stress质量门。全新100张独立发布困难负样本已在任何第四候选推理前生成、原图筛查、统一编号并完成预授权机器隔离审计；schema v2授权构建器已把261—360逐文件记录哈希`0bfed8c3…4511`及候选权重、阈值和保护集证据锁定，但尚缺用户原文确认，故生产推理仍不切换候选权重或阈值 |
...
| 正式纹理模型 | `/models/nail-texture-seg/manifest.json` | 阻塞 | 第四候选为候选2与候选3的alpha=0.40插值权重，SHA-256为`2bde9702…2b09`，校准阈值0.35。规范val30部署512 box/mask mAP50为0.8711/0.8395，阈值门precision/recall/F1约为0.82/0.80/0.8099、每图误检0.83、单图最多7候选，联合门通过。冻结test100的full/core/stress box/mask mAP50为0.9425/0.9387、0.9454/0.9467、0.9484/0.9178，schema v2质量报告`f4432c3c…1115`深验并通过该门。新独立发布留出已具备逐文件精确授权契约但尚未获得用户原文确认和原子冻结，第四候选从未对其推理，因此不得导出ONNX、登记或写入生产manifest，生产共享阈值不变 |
| 数据集治理 | `model/datasets/nail-texture-v1` | 进行中 | 正式训练集仍为409图、2142个mask，split=300/46/63。`candidate3-training-v1`精确授权记录绑定160项，授权商业训练、数据质量审核、模型诊断和长期回归，明确排除独立发布测试且不放宽质量门；终审145张通过、15张排除，第三候选规范包为train245/val30/test0。新独立发布留出候选另在Git外生成100张，统一编号`261—360`，候选清单SHA-256为`bf2ed2db…f939`；预授权机器审计100/100解码、0完全重复、0批内dHash近重复，并与受保护registry 368条记录完成36,800次比较，精确SHA、规范sourceIdentity和dHash≤12重合均为0，报告SHA-256为`429af514…dd47`。`build-independent-hard-negative-user-authorization.py`已将精确100项记录哈希`0bfed8c3…4511`、四项允许用途、商业训练禁用及候选权重/阈值/保护集绑定为不可覆盖schema v2契约；当前仍等待用户原文确认后才能生成授权源和原子冻结，旧110张失败留出和145张训练清单均不得替代 |
...
| 2026-07-28 第四候选权重插值与预冻结独立留出 | `interpolate-yolo-checkpoints.py`、`build-independent-hard-negative-user-authorization.py`、外部`nail-texture-candidate-2026-07-28-v4-interpolation-sweep` | 现有质量门通过（发布仍HOLD） | 在查看结果前预注册alpha=0.10/0.20/0.30/0.40，只用规范val30与145张训练角色困难负样本选择比例，冻结test100不参与选择。四档均过val门，alpha=0.40以训练角色原图/裁角/模糊误检图5/5/5、合计15最优，权重`2bde9702…2b09`、阈值0.35。冻结test100 full/core/stress部署512质量门全部通过，报告`f4432c3c…1115`。另生成100张全新AI困难负样本并在首次模型推理前完成原图预筛、编号261—360和registry预审，0批内/保护集重合；逐文件schema v2授权契约已锁定记录哈希`0bfed8c3…4511`，仍缺用户原文确认与原子freeze，故未运行候选推理、未导出或晋升 |
...
| 候选三路训练发布编排 | `run-training-release-pipeline.ts --candidate-mode`、`verify-training-release.ts --candidate-mode` | 进行中（第四候选待独立留出授权） | 第三候选训练链保持历史拒绝；新增checkpoint插值工具以哈希绑定方式产生第四候选，四档预注册扫描、规范val门、训练角色负样本选择和锁定后冻结test100均已完成并通过。全新100张独立发布困难负样本已生成且候选模型从未推理，逐文件schema v2授权构建与冻结端双重完全匹配门已通过工程验收；下一步必须先取得用户精确原文授权、执行不可变freeze与正式原分辨率终审，再运行三变体泛化门，当前不能导出或登记候选 |
...
| 训练/评估/导出 | `model/training/*` | 进行中 | 第四候选`2bde9702…2b09`通过候选2/3权重插值获得，规范val30、训练角色负样本诊断和冻结test100 full/core/stress/640诊断已完成；部署512三组冻结指标全部过门，640仍只作诊断。新的100张独立留出已生成并在推理前完成预授权机器隔离审计，精确授权构建器实跑确认100项记录哈希`0bfed8c3…4511`和唯一确认文本；用户授权源/freeze/正式终审和三变体推理仍未开始。第四候选未导出ONNX或登记发布，生产manifest不变 |
...
1. 第二候选因训练后独立留出三变体7/5/6张误检被否决，第三候选因规范val30联合门失败被否决。第四候选以候选2/3的alpha=0.40哈希绑定插值恢复val召回/误检平衡，并通过冻结test100的full/core/stress部署512质量门；但这仍不是完整发布。全新100张AI困难负样本已在任何第四候选推理前完成生成、原图预筛、连续编号261—360和受保护registry预授权审计，100/100解码、0批内近重复、与368条保护记录精确/来源/dHash重合均为0。当前缺少用户对该精确清单用于独立发布测试的明确授权，因此不能执行原子freeze、正式审核或首次推理；旧110图、145张训练清单和现有100张冻结正样本test均不能替代该门。同时仍缺用户典型失败案例、四类移动真机、至少100张Beta人工质量报告、生产ONNX与回滚证据。生产manifest、共享阈值和正式发布状态继续HOLD；
...
| 日期 | 版本 | 变更摘要 | 影响范围 |
| --- | --- | --- | --- |
| 2026-07-28 | v1.1.228 | 补强第四候选独立留出的授权边界：新增`build-independent-hard-negative-user-authorization.py`，只消费固定候选清单与预授权审计，生成并要求绝对路径、100张、连续261—360完全一致的唯一用户确认文本；schema v2授权源逐项绑定相对路径、SHA-256、尺寸、格式、字节数、dHash、来源组及候选权重、阈值报告和受保护registry，明确排除商业训练并保持训练禁用。冻结器兼容历史schema v1只读重放，同时对新schema v2在首次枚举和原子提交前两次验证当前100项与用户授权清单完全相同，拒绝换图、增删、改名和证据漂移。真实候选只读预检确认记录哈希`0bfed8c3…4511`且第四候选仍未推理；独立链专项24/24、完整串行699/699通过，编码审计、Python编译、ESLint、生产构建和差异检查通过。用户尚未发出精确授权，故未生成授权源、未冻结或推理；未提交或推送Git | 独立留出逐文件授权、原子冻结、证据防漂移、历史兼容、完整回归、发布HOLD |
