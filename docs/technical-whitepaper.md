...
| 正式纹理模型 | `/models/nail-texture-seg/manifest.json` | 阻塞 | 第二候选权重SHA-256为`e46968b7…3882`。规范val30部署512 box/mask mAP50为0.8788/0.8364，阈值0.45经只读深验通过；冻结test100的full/core/stress为0.9501/0.9454、0.9508/0.9567、0.9472/0.9221，schema v2质量报告决定`accept_candidate_release`并深度重放通过。训练困难负样本158张的三变体均为0误检，但训练后独立留出110张的原图/裁角/模糊角标分别误检7/5/6张，第二候选不得晋升。第三候选训练负样本已固定160项机器计划，现有75/160张、仍缺85张；进度审计75/75机器通过，但精确授权和正式原分辨率审核尚未完成。生产manifest和共享阈值不变 |
| 数据集治理 | `model/datasets/nail-texture-v1` | 进行中 | 正式训练集仍为409图、2142个mask，split=300/46/63。第二候选规范包为train258（100张正样本/521 mask + 158张零字节标签困难负样本）、val30/144 mask、test0、孤儿0；物化报告SHA-256为`033b8d02…2ec`，文件树SHA-256为`8709fa80…1170`，GPU前输入深审SHA-256为`b9bd6897…94dd`，与冻结test100张/554 mask零重叠。训练后独立留出110张固定`trainingUse=prohibited`，不得回流训练或继续充当第三候选未见留出。第三候选外部计划逐项绑定001—160文件身份、家族/变体、registry路径及SHA-256；当前75张全部通过命名、完整解码、768像素、批内与受保护全集SHA/dHash隔离，85张缺失、0失败、0未知，训练批准仍按0/100。权威受保护注册表SHA-256为`c533b3d9…3108`，绑定3份历史training清单和1份失败holdout清单，4份manifest深验通过并规范去重为368条保护记录；授权不替代质量终审 |
...
| 候选三路训练发布编排 | `run-training-release-pipeline.ts --candidate-mode`、`verify-training-release.ts --candidate-mode` | 已完成（第二候选被独立留出拒绝） | 三套dataset与六份证据强隔离链已用于100正样本/158困难负样本/30验证图；50轮训练所得`e46968b7…3882`完成规范val、阈值0.45深验、冻结test100 full/core/stress与640诊断评估。训练后独立负样本留出110张已完成冻结、终审和三变体推理，但因7/5/6张误检决定HOLD。第三候选开训不缺正样本、val、GPU或编排代码；当前硬缺口为按固定机器计划补齐85张训练负样本、冻结精确清单授权、原分辨率终审和规范训练集物化 |
...
1. 第二候选已使用train100张/521 mask、困难负样本158张/空标签和val30张/144 mask完成50轮续训；冻结test100张/554 mask的部署512 full/core/stress mAP门及schema v2深度重放通过，规范val30校准阈值为0.45。训练后独立留出110张已在首次第二候选推理前冻结并完成原分辨率终审，但三变体仍误检7/5/6张，故第二候选被正式否决。第三候选当前首要阻塞是与失败留出隔离的新训练困难负样本：固定计划160张，实存75张、仍缺85张；当前机器审计75/75通过，但精确文件清单授权、原分辨率终审和不少于100张正式批准均未完成。训练后还必须通过新版registry隔离强门另建不少于100张全新独立留出并达到三变体零误检、零delta；旧110图freeze缺少registry绑定，只能作为第二候选历史诊断。除此之外仍缺用户典型失败案例、四类移动真机、至少100张Beta人工质量报告、生产ONNX与回滚证据；这些门均不得用正样本mAP或val阈值替代。生产manifest、共享阈值和正式发布状态继续HOLD；
