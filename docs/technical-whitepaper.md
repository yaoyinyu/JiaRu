...
| AI 美甲生图 | `/ai-generate`、`POST /api/generate-ai` | 已完成（用户验收） | 已接入Agnes Image 2.1 Flash；`.cn`模型列表认证HTTP 200且目标模型可见，协议专项测试、Lint、全量707项测试和生产构建通过。2026-08-10用户确认AI美甲真实生图功能验证通过；成本、内容安全和生产持续可用性继续作为运营监测项 |
...
| 正式纹理模型 | `/models/nail-texture-seg/manifest.json` | 阻塞 | 第五候选权重SHA-256为`26daf742…a56`，规范val30阈值0.50，冻结test100总体mAP门通过，但新增逐实例识别报告`0411bab4…6571`在部署512下只匹配464/554甲面、漏90枚，49/100图存在漏甲，仅39/100图可直接完整提取，并发现8个重复、23个额外候选和23个拓扑无效预测mask，决定HOLD。新独立负样本三变体也为3/4/3张误检；candidate5正式拒绝，不得导出生产ONNX、登记或写入生产manifest |
...
| 训练/评估/导出 | `model/training/*` | 进行中（candidate5停止于评估） | 第五候选在RTX 4060上早停于16轮、最佳epoch=1，权重`26daf742…a56`；val30阈值0.50及冻结test100正样本质量门通过，但训练后独立负样本三变体报告`9cb13d1f…8016`决定HOLD。candidate5未导出ONNX、未登记或部署，生产manifest与共享阈值不变；须先训练并验证后续候选，再谈导出、浏览器协议、注册、双版本回滚和生产激活 |
...
- `/editor` 上传照片在浏览器本地处理；
- `/ar-tryon` 摄像头帧在浏览器内存中处理，不录制、不上传；MediaPipe程序和模型由本站同源静态目录提供；
...
截至v1.1.273，上传完整手背参考图可通过本站同源MediaPipe资源自动生成五指候选；局部近景没有完整手部关键点时，甲色路径可覆盖低饱和银色、镜面高光和干燥/纹理化甲缘，低对比边界路径可在部分裸色/半透明近景中生成低置信候选。但这些规则对含标题、说明文字、重复缩略图或多个视觉区域的复杂拼图不具备可靠泛化能力，最新现场仍为0区域。两条路径都只是几何级辅助定位，没有像素级甲面mask；禁止继续用单图加规则方式冒充模型质量。正式精细自动识别继续受“无批准生产ONNX”阻塞；candidate5对最新拼图裁片虽召回五枚主图甲面，却同时误检标题文字，符合其独立负样本门失败后的正式拒绝状态。下一步必须训练并批准新候选，不能把辅助定位或candidate5诊断表述为正式模型PASS。实时AR已启用高精度手部关键点、TIP/DIP/PIP深度尺度、手背平面横轴、侧转仿射剪切、低拖尾平滑和五指独立校准，自动几何/页面回归通过；但自动化环境没有取得真实摄像头帧，跨设备原分辨率视觉贴合仍未PASS。
