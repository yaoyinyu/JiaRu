...
| 美甲纹理自动识别 | `recognizeNailTextures()` | 进行中 | 浏览器推理、Worker、后处理和 fallback 已实现；2026-07-24候选已通过冻结100张/554 mask的部署512 full/core/stress mAP门，规范val30正式校准得到候选阈值0.50，但AI困难负样本裁水印消融出现2/100张误检且没有独立负样本留出集，仍无可晋升生产的模型 |
...
| 正式纹理模型 | `/models/nail-texture-seg/manifest.json` | 阻塞 | 2026-07-24候选权重SHA-256为`bcb145b5…1a50`，冻结100张/554 mask在部署512的full/core/stress box/mask mAP50分别为0.9654/0.9601、0.9715/0.9718、0.9436/0.9240，schema v2质量报告与深度重放均通过；规范val30给出候选阈值0.50并重放通过，但同一批AI困难负样本的裁水印变体在0.35部署阈值出现2/100张误检，训练集原图/模糊角标版虽为0误检也不能替代独立留出泛化证据。候选继续HOLD，生产manifest和共享阈值不变，移动真机、Beta和失败案例门也未解除 |
| 数据集治理 | `model/datasets/nail-texture-v1` | 进行中 | 正式训练集仍为409图、2142个mask，split=300/46/63。当前候选输入已形成train正样本100张/521 mask、AI困难负样本99张加历史1张/100个空标签、val30张/144 mask、test0；规范物化train=200、val=30且与冻结test100张/554 mask按文件名、图片SHA-256和来源组零重叠，文件树SHA-256为`4a97f5b6…12b8`。626张AI图严格原分辨率审核只放行99张，未因AI来源放宽清晰度、完整性或目标域门；其余527张不回收凑数。现有100张负样本来自同一生成批次且带系统性右下角标记，只可作训练，不具备独立留出资格。2026-07-24经用户确认方向后，已另生成160张无统一水印的独立输出候选，20个提示族×8种场景；160/160完整解码、0完全重复、0感知近重复，视觉总览发现1张四指机械手并排除，剩余159张仍待逐文件原分辨率终审。当前候选模型在全160张原图上以0.35阈值误检72张/174个候选、0.20阈值误检96张/257个候选，证明该批具有真实错误驱动价值；但由于误检结果已用于分析，本批不能在后续针对性训练后继续冒充未见独立发布留出，全部暂时`trainingUse=prohibited`。正式晋升仍需冻结训练角色后另建不少于100张的新来源独立留出并通过三变体零误检门 |
...
| 规范候选训练数据与输入门 | `finalize-reviewed-hard-negative-manifest.py`、`materialize-canonical-candidate-training-dataset.py`、`audit-candidate-training-input.py`、`train-yolo-seg.py --candidate-mode` | 已完成（真实候选输入PASS） | 固定正式下限为train正样本100、正式hard negative 100、val30且CLI不得下调。当前批准清单、角色隔离、规范物化和GPU前输入深审均已通过；候选包为train200（100张正样本/521 mask + 100张零字节标签困难负样本）、val30/144 mask、test0、孤儿0，文件树SHA-256为`4a97f5b6…12b8`。输入审计修复并复验内部物化报告不再被误判为孤儿；训练入口只清理已知Ultralytics标签缓存，支持对既有训练目录安全补写摘要。输入PASS只授权本次训练，不授权模型发布 |
...
| 规范val30阈值校准门 | `evaluate.py`、`calibrate-model-score-threshold.py` | 已完成（候选阈值0.50） | 规范30图/144 mask终审、角色隔离与当前文件字节被独立重放；评估使用独立runtime副本并复验源树前后逐文件哈希，制品索引绑定稳定文件清单、逐文件SHA和30图显式预测记录。历史v6在该验证集无合格阈值；新候选`bcb145b5…1a50`的部署512 box/mask mAP50为0.928/0.815，正式决定`calibrated_threshold_ready_for_candidate_manifest`、`manifestScoreThreshold=0.50`，报告SHA-256为`064b0a26…79f6`并通过`--verify-report --expected-weights`重放。该值只授权候选，不修改生产manifest或共享阈值 |
...
| 候选三路训练发布编排 | `run-training-release-pipeline.ts --candidate-mode`、`verify-training-release.ts --candidate-mode` | 已完成（候选训练完成） | 三套dataset与六份证据的强隔离链已用于真实100/100/30候选输入；v6续训50轮所得权重完成规范val、冻结test100的full/core/stress与640诊断评估。当前未导出生产ONNX，原因是困难负样本水印消融与独立留出未通过，而不是训练输入数量不足 |
...
| 训练/评估/导出 | `model/training/*` | 进行中 | v4、v5、v7、v8、v9历史候选均被拒绝；2026-07-24候选`bcb145b5…1a50`已通过冻结100图部署512 mAP门，full/core/stress box/mask mAP50为0.9654/0.9601、0.9715/0.9718、0.9436/0.9240，并从规范val30得到候选阈值0.50。候选尚未导出ONNX或登记发布，因为困难负样本裁水印消融失败且缺独立留出 |
