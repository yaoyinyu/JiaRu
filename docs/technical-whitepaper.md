...
| 训练真值当前权威快照 | 外部工作区根目录`training-truth-index-v1.json` | 进行中 | v1.1.177：101个批准报告归并为100张唯一图片/521个完整mask，2个历史拒绝报告、1个冗余报告、0冲突，索引SHA-256为`13f606b547c32d2b8f34651f55e1bca1e826bf3ac13bdcdca345a1cef267f125`。批次067的`00538/00301`新增2张/10 mask；`01113`因仅4枚人类甲面完整可见而未进入本批。SAM2.1 large完成2图/10提示、1 fallback，初轮几何3 pass/7 suspect且视觉门发现整指、皮肤、衣物及背景污染，终版10枚甲面全部以人工polygon重建并通过整图与逐甲2×视觉审核、合法性、同图零交叠和几何10/10。最低100张train正样本门已达到；val当前12/30张、54 mask，仍缺18张；约100张hard negative、整批物化和来源隔离仍未完成，训练用途继续禁止。权威索引位于审核工作区根目录；`final/training-truth-index-v1.json`现为字节一致的当前镜像。本行取代v1.1.176及更早数量快照 |
| 验证真值当前快照 | 外部`val-annotation-workspace-v1/validation-truth-index-v1.json` | 进行中 | v1.1.184：30张/7个val-only来源组/159枚首轮预期完整甲面的审核已完成；角色感知终审当前批准12个报告，归并为12张唯一图片/54个完整mask、0冗余、0冲突，索引SHA-256为`28e609c7ffceba0bd09c4dfeed259cc2ecb040153b2c518e4a8173e4cf039b0f`。批次008的`00933/00942`均确认5枚长甲完整露出；SAM完成2图/10提示、0 fallback、几何10/10，但视觉门仍拦截横向透明甲吸收整段手指以及多甲甲根皮肤、锯齿和附着装饰边界风险。终版保留5个已审polygon、人工重画5个问题polygon，逐甲2×继续收紧第1甲侧边立体装饰。两图10个终版polygon均通过整图/全部逐甲2×、合法性、零交叠和几何审核。所有12项绑定`assignedRole=val`且`trainingUse=prohibited`；剩余18张仍须返修，30/30完成和物化审计前禁止阈值校准及候选训练 |
