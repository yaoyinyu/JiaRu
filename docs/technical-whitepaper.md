...
| 候选6正样本补强 | `record-positive-reinforcement-authorization.py`、`build-positive-reinforcement-annotation-workspace.py`及Git外`positive-reinforcement-annotation-workspace-v1` | 进行中（初审160/160；返修真值52张/225 mask） | 用户已对`requestedItemsSha256=1cdf2825…5bfe`的160张/76组/971枚可见甲面作精确商业训练授权，授权记录`67a17f2d…1b93`深验通过。8分片/80页首轮原分辨率审核为13张通过候选、147张返修、0排除。首批删误检返修终审4张/15 mask通过；第二批逐甲紧框仅1张/5 mask通过，两张透明低对比甲经人工多边形新增10 mask；后续25张人工/混合多边形新增114枚完整甲面，均通过polygon合法、同图零交叠、几何114/114及整图/逐甲2倍终审。另有`nail_00521…_1.jpg`因旧第5区域为白手套背景而哈希绑定为rework、未晋级。batch022完成`nail_00653…_3.jpg`返修但因已由真值018登记而不重复计数；batch023—032完成多类花纹/透明长甲、漏甲、误检清理、失焦止损、附着钻饰和边界重画。batch033—036完成透明长甲混合返修、远景人像五甲全人工返修、重复拇指清退及遮挡粉色长甲补回。batch037选择同组更清晰且五甲完整可见的`nail_01163…_2.jpg`，删除3个重复、整指或皮肤污染候选，保留4个完整mask并以人工多边形替换原无效拓扑豹纹甲；终版5/5合法、零交叠、几何5/5，整图和逐甲2倍确认甲根、甲缘、延长甲尖及纹理完整，无皮肤、毛衣、背景、水印、重复或无效拓扑污染。唯一训练真值索引现为52张/225 mask、0拒绝/冗余/冲突，权威与重放SHA-256均为`487976b7…4b10`；索引仍明确`trainingUse=prohibited-until-materialization-audit`。正样本水印不自动导致排除：仅当文字/水印遮挡或触碰甲面、破坏清晰度或完整轮廓时排除；未接触甲面的水印须记录来源风险并通过来源隔离，禁止将其当作识别捷径。该澄清不放宽困难负样本水印消融与独立发布留出门。其余108张尚未形成最终真值，完整来源隔离和训练物化尚未完成，禁止训练、val调参、冻结test推理、导出或部署 |
