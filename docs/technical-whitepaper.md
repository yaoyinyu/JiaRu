...
| 数据集治理 | `model/datasets/nail-texture-v1` | 进行中 | 正式有效集为 398 图、2083 个 mask，split=292/45/61；第二批授权后累计导入 76 图（75 正样本、1 hard negative）和 498 个 mask，并按 deerplanet/more/other 集合隔离来源；剩余 37 张继续返修 |
...
| mask | 2083 |
| 有效 mask | 2083 |
| train / val / test | 292 / 45 / 61 |
| 错误文件 | 0 |
| warning 文件 | 2 |

2026-07-12 实拍数据同步：首批素材的有效来源目录已由误写的 `真实素材/2027_7_11` 纠正为 `真实素材/2026_7_11`。用户明确禁用的 `ca2d14573bfe3c0c83cdc3100e074b3c.jpg` 已从有效图片、原始标注、YOLO 标签、来源表和 split 中移除；其余 21 张审核通过图片共 174 个甲面已覆盖旧 fallback 标注并通过来源审计与标签审计。第二批 `真实素材/2026_7_12` 共 113 张已获得用户选择 A 的商业模型训练与长期回归授权；首轮视觉复核通过 70 张正样本和 1 张 hard negative，随后使用 v6 对 42 张返修项生成 640/conf=0.20 的 317 个候选并逐图复核，仅新增提升 5 张/27 mask。对剩余 37 张继续执行 1024/conf=0.10 高分辨率推理，共生成 458 个候选，但重复 mask、皮肤污染和拼图误检显著增加，视觉门未新增提升项。第二批现累计 75 张正样本、1 张 hard negative、498 个 mask 正式导入，37 张继续返修。正式集按 deerplanet、more 和其他集合隔离 `sourceGroup`，重建为 292/45/61，来源授权、标签、split 比例及训练 readiness 均通过。
