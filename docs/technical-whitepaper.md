...
| 正式纹理模型 | `/models/nail-texture-seg/manifest.json` | 阻塞 | 第二候选权重SHA-256为`e46968b7…3882`。规范val30部署512 box/mask mAP50为0.8788/0.8364，阈值0.45经只读深验通过；冻结test100的full/core/stress为0.9501/0.9454、0.9508/0.9567、0.9472/0.9221，schema v2质量报告决定`accept_candidate_release`并深度重放通过。训练困难负样本158张的三变体均为0误检，但训练后独立留出110张的原图/裁角/模糊角标分别误检7/5/6张，报告决定`hold_hard_negative_watermark_shortcut_instability`并深度重放通过；第二候选不得晋升。core box相对上一候选下降0.02073的风险仍保留，生产manifest和共享阈值不变，下一步为新来源训练负样本、第三候选训练及训练后全新独立留出 |
| 数据集治理 | `model/datasets/nail-texture-v1` | 进行中 | 正式训练集仍为409图、2142个mask，split=300/46/63。第二候选规范包为train258（100张正样本/521 mask + 158张零字节标签困难负样本）、val30/144 mask、test0、孤儿0；物化报告SHA-256为`033b8d02…2ec`，文件树SHA-256为`8709fa80…1170`，GPU前输入深审SHA-256为`b9bd6897…94dd`，与冻结test100张/554 mask零重叠。训练后独立留出110张已在首次v2推理前完成不可变冻结，冻结manifest SHA-256为`1e15ae63…c022`；110张全部通过原分辨率终审，独立批准清单SHA-256为`5aaf9ac8…d7d1`，固定`trainingUse=prohibited`。该留出已暴露9张独立误检图片并影响后续训练规划，因此不得回流训练，也不得继续充当第三候选的未见发布留出；第三候选必须使用另行生成、授权和审核的新训练负样本，并在训练角色冻结后再生成全新来源独立留出 |
...
| 候选三路训练发布编排 | `run-training-release-pipeline.ts --candidate-mode`、`verify-training-release.ts --candidate-mode` | 已完成（第二候选被独立留出拒绝） | 三套dataset与六份证据强隔离链已用于100正样本/158困难负样本/30验证图；50轮训练所得`e46968b7…3882`完成规范val、阈值0.45深验、冻结test100 full/core/stress与640诊断评估。训练后独立负样本留出110张已完成冻结、终审和三变体推理，但因7/5/6张误检决定HOLD。当前未导出生产ONNX，下一训练输入缺口是与该留出隔离的新训练负样本，而不是正样本或val数量 |
...
| 模型发布治理 | `scripts/*release*` | 进行中 | 已有注册、切换、回滚、质量门、逐图失败画像、规范val30阈值深审、三路候选编排、最终完成度审计和外部验收证据构建器。独立留出链现固定100张下限、训练用途禁止、权重/阈值绑定、首次推理账本、512输入、确定性三变体和推理重放；专项22/22通过。真实110图审计因7/5/6张误检按预期HOLD。总完成度审计测试中4个旧手写夹具尚未适配新深验契约，当前专项34/38，整库回归结论需修复夹具后重跑；生产发布仍需第三候选、全新留出、移动真机、失败案例、Beta产品质量、生产资产和回滚 |
...
| 最终完成度审计 | `audit-nail-texture-local-inference-completion.ts`；核对实施规范清单、全部关键证据和生产资产，缺证据时输出HOLD及责任方；用户/工程清单必须非空、候选checkbox格式合法且文本唯一，所有候选进度行必须可解析且标记ID唯一。困难负样本独立强门直接调用水印审计只读深验并固定要求`independent-holdout`、不少于100张、三变体零误检/零delta，训练证据或手写外层PASS不得满足发布门 |
...
1. 2026-07-24候选已使用通过深审的train100张/521 mask、困难负样本100张/空标签和val30张/144 mask完成续训；冻结test100张/554 mask在部署512的full/core/stress mAP门及schema v2深度重放通过，规范val30已校准出候选阈值0.50。当前首要阻塞仍是困难负样本泛化：训练负样本原图和右下角模糊版在0.35部署阈值为0误检，但裁去右下12%后出现2/100张误检，说明固定生成器标记可能成为捷径；训练集本身固定`releaseGeneralizationEligible=false`。完成度审计已把该要求提升为直接强门，固定要求不同批次/提示族、最好无统一水印且与训练图无派生关系的独立留出不少于100张，三种变体零误检且零delta。建议提供150—200张候选以严格筛选；其中至少30张从未参与训练、阈值或返修。除此之外仍缺用户典型失败案例、四类移动真机、至少100张Beta人工质量报告、生产ONNX与回滚证据；这些门均不得用正样本mAP或val阈值替代。生产manifest、共享阈值和正式发布状态继续HOLD；
