...
| 训练真值当前权威快照 | 外部工作区根目录`training-truth-index-v1.json` | 进行中 | v1.1.177：101个批准报告归并为100张唯一图片/521个完整mask，2个历史拒绝报告、1个冗余报告、0冲突，索引SHA-256为`13f606b547c32d2b8f34651f55e1bca1e826bf3ac13bdcdca345a1cef267f125`。批次067的`00538/00301`新增2张/10 mask；`01113`因仅4枚人类甲面完整可见而未进入本批。SAM2.1 large完成2图/10提示、1 fallback，初轮几何3 pass/7 suspect且视觉门发现整指、皮肤、衣物及背景污染，终版10枚甲面全部以人工polygon重建并通过整图与逐甲2×视觉审核、合法性、同图零交叠和几何10/10。最低100张train正样本门已达到；val当前16/30张、74 mask，仍缺14张；约100张hard negative、整批物化和来源隔离仍未完成，训练用途继续禁止。权威索引位于审核工作区根目录；`final/training-truth-index-v1.json`现为字节一致的当前镜像。本行取代v1.1.176及更早数量快照 |
| 验证真值当前快照 | 外部`val-annotation-workspace-v1/validation-truth-index-v1.json` | 进行中 | v1.1.188：角色感知终审当前批准19个报告，归并为19张唯一图片/89个完整mask、0冗余、0冲突，索引SHA-256为`924d04f0ba6db2b24fa6ca19f2e29bcca6841edc12fe6edd8343ad3d1da9faf4`。批次011—012完成`00351…_4/00826/00828`三图15甲，均通过整图、全部逐甲2×、合法性、零交叠和几何审核；批次013确认`00935`右侧拇指甲根触边、实际仅4枚完整甲面而排除。其余原val源图复核后共有11张因裁断、遮挡或完整甲数错误不可用；替补角色扩展器已把11张/7个完整来源组迁移为val候选，并拒绝独立test、已有train真值、首批train工作区、部分来源组或图片漂移。替补SAM候选55枚仍在批次014—016返修；全部19项及替补候选固定`trainingUse=prohibited`，30/30完成和整批物化审计前禁止阈值校准及候选训练 |
