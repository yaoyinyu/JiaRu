...
| AI 美甲生图 | `/ai-generate`、`POST /api/generate-ai` | 待验证 | 已接入Agnes Image 2.1 Flash；`.cn`模型列表认证HTTP 200且目标模型可见，协议专项测试、Lint、全量707项测试和生产构建通过。尚未调用付费生图，结果质量、成本和内容安全仍待实测 |
...
2. AI生图已接入Agnes Image 2.1 Flash并通过模型列表认证、协议测试和构建，但尚未执行真实生图，也未完成生产环境持续可用性、结果质量、成本和内容安全验证；
