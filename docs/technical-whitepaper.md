...
| 美甲纹理自动识别 | `recognizeNailTextures()` | 进行中（candidate5发布拒绝） | 浏览器推理、Worker、后处理和 fallback 已实现。第五候选虽完成GPU训练、规范val30阈值校准和冻结test100的full/core/stress正样本质量门，但训练后新100张独立困难负样本在部署512、阈值0.50的原图/裁右下12%/模糊右下角中分别误检3/4/3张、5/4/6个检测，检测数delta为1/1，零误检/零delta门失败。不得切换生产权重或阈值 |
