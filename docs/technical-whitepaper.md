...
| 数据集治理 | `model/datasets/nail-texture-v1` | 进行中 | 正式训练集仍为409图、2142个mask，split=300/46/63。新增101张真实素材中9张跨旧批次精确重复排除；其余92张仅用于独立发布测试与长期回归，其中67张/384 mask冻结为受审核候选、25张排除、18个父来源组，训练用途始终禁止。2026_7_14目录当前实存1271张候选、196个来源组，用户已选择A并允许商业训练、独立发布测试和长期回归；近重复视觉审核排除105张，剩余1166张/193来源组已由26个来源组原子分片完成逐图审核：565张仅进入待标注候选，601张排除，0张待审。批次终结门已证明26/26分片和1166/1166图片唯一覆盖；565张按来源组互斥规划为train 502、val 30、独立发布test 33，首批标注选择train中的160张/39来源组、预计966个完整甲面。首批外部标注工作区已生成160张/39来源组，YOLO与SAM2形成881个候选polygon；SAM2运行160/160、0错误、0 fallback，几何审计796 pass、85 suspect、0 missing。首批mask原分辨率初审已完成分片001—010全量160/160张：15张暂通过、138张返修、7张因甲面裁断或遮挡残缺排除；分片010的17张为2张直接通过、15张返修、0排除。当前唯一79张/416 mask已通过哈希绑定原分辨率视觉、polygon合法性和同图零交叠终审，成为训练真值候选；分片009—010及跨分片批次011—018累计又提升26张，清除戒指、方向盘控件、手链流苏、珍珠链、衣物、毛衣孔洞、整段手指、背景标识、头发/背景和非要求侧面误检，并补齐深色甲、星形拇指甲、透明侧视拇指、竖指延长甲、横向侧甲、两枚低对比漏标拇指及两枚甲根凹口甲面。双手侧向拇指图即使几何通过也因只露弯曲局部且吸收大块指腹而拒绝真值；批次015—016的两张漏拇指图同样因SAM吸收甲根皮肤而转为人工多边形，未以几何通过冒充完整mask。批次018对两张五甲齐全样本仅替换问题甲面，补齐淡紫/白色渐变甲的甲根侧缺口。批次019—023对两张零候选图执行SAM与人工多边形尝试，但因皮肤吸收、相邻甲交叠及透明低对比甲面边界不可信而0张晋级；`nail_01119…_6`双手10甲通过直通终审。唯一性审计发现`nail_00491…_2`的003与039报告绑定相同annotation SHA-256，规范选择039且同图只计一次；80个批准报告归并为79张唯一图片/416 mask，1个冗余、0冲突。白色立体装饰长甲图因透明边缘缺口继续返修。批次028—030删除`00704`人物眼睛误检，人工重画左侧横向甲、右上拇指并收紧中央亮片甲甲根；批次031对`00533`保留3甲并人工重画SAM失败且交叠的两枚甲面，两张共新增10个完整mask。批次032人工补齐`00531`短拇指并清除横向甲毛衣污染；批次033—034人工重画`00951`侧视装饰拇指、蝴蝶结甲和银粉方甲缺口；批次035—036逐甲清理同来源`00274/00276/00278`的甲根缺口、锯齿和皮肤外刺，三张新增15个完整mask；批次037—038对同源`00277`补齐两枚漏甲并清除整段手指污染，再补足两枚可见甲尖，新增5个完整mask。SAM几何审计使用精确polygon交集面积拦截真实重叠；人工多边形仅保留已审polygon并替换问题甲面，仍须通过几何、合法性、零交叠和原分辨率视觉。返修终结器同时接受受审SAM报告和受审人工多边形报告，但两者均不得绕过最终真值审计；最低100张train正样本仍缺21张，val真值0/30和约100张hard negative尚未建立。全部真值仍须整批数据物化与来源隔离审计后才能解除训练禁用。1001张Claude生成图仍为未授权、未标注的合成候选池 |
...
| 训练真值当前权威快照 | 外部工作区根目录`training-truth-index-v1.json` | 进行中 | v1.1.173：93个批准报告归并为92张唯一图片/481个完整mask，1个冗余报告、0冲突，索引SHA-256为`a9d5936a1b52a8cc6ee5c33e750ea603be225a45336590e7b28e1f4442df2246`。批次064的`00476/00712/00291`新增3张/15 mask；首轮SAM及`00291`收紧重试经原分辨率视觉门发现金属碎片、衣物、皮肤、邻指污染和75.9302像素装饰交叠，终版改为6个人工polygon与9个已审SAM polygon，完整覆盖灰粉亮片、透明银粉和银灰水钻长甲。15/15 polygon合法、同图零交叠、几何全通过。最低100张train正样本完成92%、仍缺8张；val 0/30与约100张hard negative不变，整批物化和来源隔离前继续禁止训练。权威索引位于审核工作区根目录；`final/training-truth-index-v1.json`仍是历史副本。本行取代v1.1.171及更早数量快照 |
