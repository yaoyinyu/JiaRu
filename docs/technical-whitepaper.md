...
1. v6资产、协议和桌面性能证据有效，但冻结67张/384 mask在部署512下的扩展质量门失败，正式拒绝发布；部署阈值画像进一步确认压力组召回仅0.6983，透明相邻长甲、多甲同屏、低对比漏整甲/局部识别及手指/腕表误检是首要方向。代表性下限仍缺33张。2026_7_14当前1271张候选已获A授权并完成近重复视觉归并，105张重复或非照片候选排除，剩余1166张仍待逐图完整甲面视觉审核和来源组互斥分配，不能直接用于训练或补足test；同时仍缺用户典型失败案例、四类移动真机和至少100张Beta人工质量审核，正式模型审计继续blocked；
