...
| 候选5训练负样本 | `build-candidate5-hard-negative-generation-plan.py`、`build-training-hard-negative-authorization-request.py`、Git外`2026_7_29_candidate5_training_v2` | 等待授权（160/160，训练禁止） | candidate4留出已由registry v2保护；16个提示族共160项已逐图生成并完成生成阶段双人可见初筛。全批机器审计160通过、0缺失/失败/未知，报告`9aa129a1…25c3`；精确授权请求包`a71188d8…5d56`逐项冻结当前文件。生成与请求准备通过不等于训练批准；当前仍等待用户精确商业训练授权，未做正式原分辨率终审、schema v2批准、物化或GPU输入深审，不得进入训练，也不得改作独立发布测试 |
...
| 模型发布治理 | `scripts/*release*` | 进行中 | 已有注册、切换、回滚、质量门、逐图失败画像、规范val30阈值深审、三路候选编排、最终完成度审计和外部验收证据构建器。独立留出链固定100张下限、训练用途禁止、权重/阈值绑定、首次推理账本、512输入、确定性三变体和推理重放。默认完成度命令现通过仓库内evidence profile按路径与SHA-256绑定当前冻结test100、schema v2质量报告、第二候选110图独立留出审计和部署512指标，并强制四份证据属于同一权重`e46968b7…3882`及阈值0.45；真实110图7/5/6张误检仍按预期HOLD。生产发布仍需第三候选、全新留出、移动真机、失败案例、Beta产品质量、生产资产和回滚 |
...
候选5使用`build-training-hard-negative-authorization-request.py`把已完成深验的160项进度报告转换为精确授权请求。构建器只接受`READY/ready_to_request_exact_user_authorization`、160通过且零缺失/失败/未知的报告，逐项冻结相对路径、SHA-256、dHash256、尺寸、提示ID、提示族和变体，并复用正式授权构建器执行深度验证；输出始终为`HOLD/awaiting_exact_user_confirmation`、`trainingUse=prohibited`。真实当前报告`generation-progress-160-v20.json`为160/160通过，SHA-256为`9aa129a1…25c3`、items摘要为`1153e267…d6f6`；请求包`candidate5-training-v2-exact-authorization-request-v1.json` SHA-256为`a71188d8…5d56`、请求项摘要为`67e8b519…3b80`。该请求包没有授权效力；用户精确确认、正式A授权建账、原分辨率终审、schema v2批准、物化和GPU输入深审前，本批不得训练或改作独立发布测试。
...
1. 第二候选因训练后独立留出三变体7/5/6张误检被否决，第三候选因规范val30联合门失败被否决。第四候选通过冻结test100的full/core/stress部署512质量门，但其100张新独立留出在三变体中产生3/2/3张误检，已被正式拒绝。candidate4留出现已单调追加至保护registry v2，永久禁止训练。候选5依据鱼饵反光件、椭圆家具五金和长椭圆发夹三类失败建立的160项计划已全部生成；16个提示族完成生成阶段双人可见初筛和机器门，160/160通过、0缺失/失败/未知，精确授权请求包也已冻结。当前仍缺用户对该精确清单的商业训练授权，以及授权后的正式A建账、原分辨率终审、schema v2批准、训练物化和GPU输入深审。生成通过数和授权请求均不代表训练批准数。旧110图、145张训练清单和candidate4留出均不能替代新训练样本或下一候选的未见留出。同时仍缺用户典型失败案例、四类移动真机、至少100张Beta人工质量报告、生产ONNX与回滚证据。生产manifest、共享阈值和正式发布状态继续HOLD；
