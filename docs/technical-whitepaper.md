...
| 训练真值当前权威快照 | 外部工作区根目录`training-truth-index-v1.json` | 进行中 | v1.1.174：96个批准报告归并为95张唯一图片/496个完整mask，1个冗余报告、0冲突，索引SHA-256为`12e3d613c8602efd4cad16a6b6c67457304ca7610d1dfacbf0e711fb9e0e3510`。批次065的`00293/00711/00292`新增3张/15 mask；`00915`两轮SAM仍合并手指/手掌后未凑数放行，改选`00292`。终版由6个逐甲审核通过的SAM polygon和9个人工polygon组成，第二轮原分辨率视觉门继续清除拇指皮肤与衣物边界并补齐银灰长甲完整甲尖。15/15 polygon合法、同图零交叠、几何全通过。最低100张train正样本完成95%、仍缺5张；val 0/30与约100张hard negative不变，整批物化和来源隔离前继续禁止训练。权威索引位于审核工作区根目录；`final/training-truth-index-v1.json`仍是历史副本。本行取代v1.1.173及更早数量快照 |
