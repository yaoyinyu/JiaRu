...
截至v1.1.272，上传完整手背参考图可通过本站同源MediaPipe资源自动生成五指候选；局部近景没有完整手部关键点时，甲色路径可覆盖低饱和银色、镜面高光和干燥/纹理化甲缘，新增低对比边界路径可在裸色/半透明甲面被肤色种子吞掉时通过闭合甲缘和单一4—5枚连续链生成低置信候选。第二现场截图裁片由0/5恢复为5/5区域和唯一分配，五框视觉落在真实甲面、背景/掌纹0候选，一次提取后五纹理均存在；旧深色/银色现场裁片继续走原甲色路径并保持5/5。两条路径仍只是几何级辅助定位，没有像素级甲面mask；极弱边缘、严重模糊、甲缘遮挡、少于4枚低对比甲或竞争结构簇仍会进入人工处理。正式精细自动识别继续受“无批准生产ONNX”阻塞，不能把辅助定位通过表述为candidate5或后续分割模型PASS。实时AR已启用高精度手部关键点、TIP/DIP/PIP深度尺度、手背平面横轴、侧转仿射剪切、低拖尾平滑和五指独立校准，自动几何/页面回归通过；但自动化环境没有取得真实摄像头帧，跨设备原分辨率视觉贴合仍未PASS。
