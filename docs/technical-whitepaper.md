...
| 美甲纹理自动识别 | `recognizeNailTextures()` | 进行中 | 浏览器推理、Worker、后处理和 fallback 已实现。第五候选已完成GPU训练、规范val30阈值校准和冻结test100的full/core/stress质量门，部署512全量box/mask mAP50为0.9569/0.9561，候选阈值0.50；训练后全新未见独立困难负样本已补齐120张并仅按源图质量拟选100张，但尚待用户精确授权、原子freeze、正式终审与三变体审计，不能据此切换生产权重或阈值 |
...
| 候选5训练后独立留出 | Git外`2026_7_29_candidate5_post_train_holdout_v1_candidates`、`2026_7_29_candidate5_post_train_holdout_plan_v1.json` | 待精确授权（120候选拟选100） | 10个家族各12张的120张候选已补齐；只按原分辨率源图质量和家族代表性每族拟选10张，精确拟选清单`proposed-independent-holdout-selection-v1.json`为100张、排除20张，内容摘要`bc386756…1525`，文件SHA-256为`320318b1…0f89`。120张全部解码，最短边1022、最长最短边1254，SHA-256均唯一，候选内部dHash256距离不大于12的配对为0；与registry v3的628条唯一保护记录完成来源/感知隔离预审且0错误。牙科第一轮12张因整体过度接近独立甲片语义已移出候选并按更明确的后牙咬合面重新生成，拟选时又排除仍含高占比尖牙/门牙正面形态的02、03。未对本批运行candidate5推理，未按模型输出挑图，未取得用户精确授权，未原子freeze、未形成正式1:1终审PASS、批准清单或三变体审计；全批继续`trainingUse=prohibited`，不得视为独立发布证据 |
...
| 规范val30阈值校准门 | `evaluate.py`、`calibrate-model-score-threshold.py` | 已完成（第四候选通过） | 规范30图/144 mask终审、角色隔离与当前文件字节被重放。第四候选`2bde9702…2b09`部署512 box/mask mAP50为0.8711/0.8395，在0.35阈值达到precision/recall/F1约0.82/0.80/0.8099、每图误检0.83、单图最多7候选并输出合格阈值；第三候选无合格阈值的拒绝证据继续保留。val通过只允许进入后续冻结测试，不单独授权manifest发布 |
...
| 候选三路训练发布编排 | `run-training-release-pipeline.ts --candidate-mode`、`verify-training-release.ts --candidate-mode` | 进行中（第五候选正样本质量门PASS） | 第五候选已完成隔离训练、规范val30校准和冻结test100的full/core/stress评估；正式质量报告深验PASS。由于同一编排入口会对train/val/test共用单一`imgsz`且val失败后不会自动停止，本轮按隔离阶段命令执行，避免错误消费冻结证据。下一强门是训练后新建、冻结并授权不少于100张未见独立困难负样本，按部署512阈值0.50通过原图/裁角/模糊角标零误检与零delta；冻结命令已增加目标权重摘要和阈值显式断言，防止误用旧候选。通过前不能导出或登记生产候选 |
...
| 训练/评估/导出 | `model/training/*` | 进行中 | 第五候选在RTX 4060上早停于16轮、最佳epoch=1，权重`26daf742…a56`；val30阈值0.50及冻结test100正式质量门通过，报告`8fa2161b…f8f3`。本轮未导出ONNX或登记发布，生产manifest不变；须先完成全新独立负样本三变体门，再推进ONNX、浏览器协议、注册、双版本回滚和生产激活 |
...
1. 第二候选因训练后独立留出三变体7/5/6张误检被否决，第三候选因规范val30联合门失败被否决。第四候选通过冻结test100的full/core/stress部署512质量门，但其100张新独立留出在三变体中产生3/2/3张误检，已被正式拒绝。candidate4留出现已单调追加至保护registry v2，永久禁止训练。候选5依据鱼饵反光件、椭圆家具五金和长椭圆发夹三类失败建立的160项计划已全部生成；16个提示族完成生成阶段双人可见初筛和机器门，160/160通过、0缺失/失败/未知。当前唯一请求已升级为schema v2并绑定160张清单摘要及四项明确用途，SHA-256为`4a47e126…27fd`；仍缺用户逐字确认，以及授权后的正式A建账、原分辨率终审、schema v2批准、训练物化和GPU输入深审。生成通过数和授权请求均不代表训练批准数。旧110图、145张训练清单和candidate4留出均不能替代新训练样本或下一候选的未见留出。同时仍缺用户典型失败案例、四类移动真机、至少100张Beta人工质量报告和生产ONNX；首发回滚工程策略已通过双批准版本命令链测试，但必须等待两个真正通过全部门的正式版本才能产生回滚证据。生产manifest、共享阈值和正式发布状态继续HOLD；
...
本节先记录停止旧会话时的权威恢复点，再记录本次按磁盘证据恢复后的进展；拟选清单仍不代表新增模型、数据授权或发布证据。
...
4. **机器预审与源图拟选**：120张全部可解码、SHA-256唯一，最短边1022—1254；候选内部dHash256距离不大于12的配对为0，registry v3深验0错误。原分辨率比较按清晰度、主体完整性、单一真实场景、无人物/甲面/文字水印/明显生成缺陷及家族代表性每族拟选10张。精确JSON拟选清单为100张、排除20张，内容摘要`bc386756…1525`、文件SHA-256 `320318b1…0f89`；联系表仅用于比较，不是正式1:1终审PASS。
5. **当前授权门与阻塞项**：尚待用户对上述100张逐文件清单精确授权；授权前没有candidate5推理、模型输出选图、原子freeze、正式机器审计、100张1:1终审、批准清单或三变体报告，全批保持`trainingUse=prohibited`。获得精确授权后才能依次执行原子freeze、机器预审、100张1:1原分辨率终审、批准清单及部署512原图/裁右下12%/模糊右下角三变体零误检图、零误检检测、零delta门。
...
7. **运行与恢复约束**：用户报告的`.codex`磁盘I/O现象仍只作为operator report；本次未扫描`.codex/sessions`，未启用子智能体或高并发。不得按模型输出改动当前拟选清单，不得把源图比较写成正式PASS，也不得将本批转为商业训练。完整发布另缺用户真实失败案例、四类移动真机、至少100张Beta人工质量、生产ONNX/浏览器协议、正式注册及双批准版本回滚证据，生产HOLD不变。

旧交接中的51/120、69张缺口和“最终100张尚未选定”已被本次磁盘复核、生成结果和精确拟选清单替代；“尚未授权、freeze、正式终审和推理”仍然有效。用户对`.codex`异常I/O的观察继续只是operator report，不升级为未经验证的根因结论。
