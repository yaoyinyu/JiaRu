...
| 数据集治理 | `model/datasets/nail-texture-v1` | 进行中 | 正式训练集仍为409图、2142个mask，split=300/46/63。新增101张真实素材中9张跨旧批次精确重复排除；其余92张仅用于独立发布测试与长期回归，其中67张/384 mask冻结为受审核候选、25张排除、18个父来源组，训练用途始终禁止。2026_7_14目录1271张候选经近重复与26个来源组原子分片审核，565张进入待标注候选、601张排除；565张按来源组互斥规划为train 502、val 30、独立发布test 33。首批train已有100张唯一图片/521个完整mask、0冲突，达到最低正样本门。val经原图清退、整组替补和批次014—018返修后形成30张唯一图片/144个完整mask、0冗余、0冲突；整图与全部逐甲2×、polygon合法性和严格零交叠审核通过。val-only物化为30图/30 annotation/30 label、train/test均0、孤儿0，并与train 100张及冻结test 67张按文件名、图片SHA-256和来源组重算为零重叠；最终审计决定为`approved_as_calibration_truth`。37张潜在hard negative逐图复核只保留1张安全候选，仍为candidate-only且缺99张；因此正式候选训练继续HOLD。1001张Claude生成图仍为未授权、未标注的合成候选池 |
...
| 规范候选训练数据与输入门 | `materialize-canonical-candidate-training-dataset.py`、`audit-candidate-training-input.py`、`train-yolo-seg.py --candidate-mode` | 已完成（工程门） | 固定正式下限为train正样本100、正式hard negative 100、val 30，CLI不得下调。物化前重算四角色文件名、图片SHA-256和来源组隔离；候选包固定train=正样本+空标签hard negative、val=规范校准真值、test为空，并绑定逐文件/聚合哈希。输入审计独立重算allow-list、annotation→YOLO标签、polygon合法性/零交叠、负样本字节空标签和孤儿文件；训练入口在加载Ultralytics或占用GPU前深度重放PASS，发布编排即使`--skip-train`也先预检并要求旧训练摘要绑定相同dataset、输入报告和权重哈希。真实输入100/1/30运行正确HOLD，外部报告SHA-256为`a0cec8aa…51493`，候选数据集目录未创建，仍缺99张合格hard negative |
...
| 模型发布治理 | `scripts/*release*` | 进行中 | 已有注册、切换、回滚、质量门、逐图失败画像、val专用阈值校准、最终完成度审计和外部验收证据构建器；2026-07-20刷新审计为327个进度标记/315个PASS、2/10发布门通过并输出HOLD。该2/10衡量完整发布闭环，不代表候选训练仅完成20% |
...
训练发布编排器使用同一权威报告：`run-training-release-pipeline.ts --candidate-mode --candidate-input-report <report>`会把`candidate-input-preflight`固定为第一步，即使`--skip-train`、dry-run或跳过环境检查也不能绕过。恢复已有训练时还须复验`train-summary.json`绑定同一dataset路径/SHA-256、输入报告路径/SHA-256及当前权重SHA-256；任何不一致都在评估、导出和治理之前停止。候选输入深审已经替代旧正式数据集来源授权门，默认编排仍为实验运行，不暗示发布资格。
...
Git 推送若出现`Failed to connect to github.com port 443`，应先用只读的DNS、HTTPS和`git ls-remote`复核链路，再比较`origin/master...master`，避免在提交其实已经到达远端时重复推送。2026-07-20复核时Git智能HTTPS端点已恢复，`git ls-remote`约1.18秒返回，本地`master`与`origin/master`均为`cc1f263`且ahead/behind为0/0；该现象属于传输层瞬时超时，不改变模型、数据或发布状态。
...
1. v6资产、协议和桌面性能证据有效，但冻结67张/384 mask在部署512下的扩展质量门失败，正式拒绝发布；部署阈值画像进一步确认压力组召回仅0.6983，透明相邻长甲、多甲同屏、低对比漏整甲/局部识别及手指/腕表误检是首要方向。候选训练数据门当前为train 100/100张、521个完整mask，val 30/30张、144个完整mask并已通过规范物化、校准真值终审及与train/冻结test的三类身份零重叠；hard negative严格复筛后仅1/100张安全候选，尚未晋升`approved_hard_negative_manifest`且缺99张。按三类最低图片数量为131/230，约57%。规范候选数据集物化器、独立完整输入审计、训练入口深度重放和发布编排前置预检均已实现并将100/100/30编码为不可下调的强制门；真实输入运行输出`hold_canonical_candidate_dataset_materialization`，观察数量100/1/30，HOLD报告SHA-256为`a0cec8aafaf1aef7c69124824cc9c108a8af02df69730bbad725bc88bb151493`，候选数据集目录未创建。因此当前唯一候选训练数据阻塞是补充、授权、原分辨率审核并正式汇总剩余99张hard negative；补齐后仍须重跑物化和输入审计取得PASS，不能直接训练。同时仍缺33张代表性发布test、用户典型失败案例、四类移动真机和至少100张Beta人工质量审核，这些阻塞正式发布，但不替代上述候选训练输入门。移动真机采集页、Profiler/Instruments CSV转换、设备验收v2及完成度深度重放已就绪，但没有真实物理设备证据前四个设备报告仍必须缺失或HOLD；
...
下一轮有效候选训练不需要等待565张待标注候选全部完成标注，也不受移动真机或Beta发布证据阻止。train最低正样本门100/100张和val门30/30张已经完成；当前素材侧必须补齐并审核99张合格hard negative，工程侧必须建立规范候选训练数据集物化器与完整训练输入审计，把100张批准正样本、至少100张正式hard negative和30张批准val绑定到逐文件/聚合哈希、来源隔离及冻结test零重叠证据，并打通新val终审到`--candidate-mode`的桥接报告。上述真实PASS形成后才允许启动下一轮候选训练。另缺33张独立发布test用于随后冻结评估；正式发布仍需继续满足300—500张Beta真实正样本、至少200张负样本、移动真机、用户失败案例和100张Beta人工质量门。候选训练启动不得被表述为发布通过，完整发布审计2/10也不得反向解释为训练准备度20%。
