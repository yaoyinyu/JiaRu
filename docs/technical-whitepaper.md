...
| AR 纹理试戴 | `/ar-tryon` | 待验证 | 上传含完整手背的参考图时优先通过MediaPipe Hands逐指定位；完整手部关键点失败后，可对包含2—5枚清晰已上色甲面的局部近景执行本地甲面定位，并用最大手部肤色支持域、原始肤色环、局部色差、形状门和同手空间聚类排除背景，再失败才进入人工补选。局部路径已覆盖黑色、彩色、低饱和银色、镜面高光及干燥/纹理化甲缘；形成4—5个连贯候选时显示为“已定位”，2—3个候选仍提示补充。两条辅助路径均没有像素级mask；实时AR支持拇/食/中/无/小指各自独立的长度75%—140%、宽度70%—145%及甲尖/指根位置±20%校准，摄像头最终贴合仍需多设备真机验收 |
...
6. 局部近景路径先以小核开/闭运算处理非皮肤且未过曝的连通区域，再要求候选同时通过面积、形状、纹理方差、触边、最大手部肤色支持域、原始肤色环与候选—肤色环局部RGB距离复合门；平滑肤色环只参与分数，不再单点否决。随后以源图最长边32%为相邻距离建立空间连通簇，只保留候选数最多、同数时总分最高的一簇并截取最多5个；五候选按手部支持域中心的极角弧排序，4—5个标为`confidence: "medium"`，2—3个标为`confidence: "low"`，全部标为`source: "partial-closeup"`；不足2个时返回空数组并进入人工模式；
...
当前 `NailArtPicker` 将正式模型检测输入最长边限制在 800 像素，最多请求 10 个候选，Worker 超时为 15 秒。fallback原候选不会进入可确认区域；MediaPipe静态图检测超时为20秒，使用`modelComplexity: 1`、最多1只手、检测/跟踪阈值0.5。局部近景分析会再次把最长边限制为640像素；初始前景要求像素不属于皮肤且灰度低于225，连通域填充率下限0.32、原始肤色环下限0.52、最大手部支持环下限0.38、局部RGB均值距离下限18。平滑肤色环保留为评分证据，没有独立通过阈值。只有同一空间簇至少2个候选时才整体返回，最多5个。`detectPartialCloseupNails()`同时返回分析尺寸、组件数、合格/选中数与枚举拒绝计数；UI把它写入本地诊断JSON，不上传原图像素。该路径是本地几何候选生成器，不提供像素mask或正式模型质量承诺。人工新增区域默认按源图短边约8%宽、短边约16%长建立，并限制在原图边界内。候选确认后以 `NailAssignment[]` 返回，每个 `ImageBitmap` 的所有权转移给 `/ar-tryon` 页面，页面在替换或卸载时负责释放。
...
截至v1.1.271，上传完整手背参考图可通过本站同源MediaPipe资源自动生成五指候选；局部近景即使没有完整手部关键点，只要同图形成2—5枚清晰已上色甲面的连贯空间簇，也可生成候选。局部路径已覆盖低饱和银色、镜面高光与干燥/纹理化甲缘：现场五甲裁片在真实浏览器为5/5区域、5/5分配并完成五纹理提取，背景0候选，导出诊断可解释组件拒绝原因。两条路径都只是几何级辅助定位，没有像素级甲面mask；透明/裸色/极小/单甲、与背景低对比或结构证据不足时仍会进入人工处理。正式精细自动识别继续受“无批准生产ONNX”阻塞，不能把辅助定位通过表述为candidate5或后续分割模型PASS。实时AR已启用高精度手部关键点、TIP/DIP/PIP深度尺度、手背平面横轴、侧转仿射剪切、低拖尾平滑和五指独立校准，自动几何/页面回归通过；但自动化环境没有取得真实摄像头帧，跨设备原分辨率视觉贴合仍未PASS。
