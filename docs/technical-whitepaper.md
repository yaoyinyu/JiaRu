...
| 日期 | 版本 | 变更摘要 | 影响范围 |
| --- | --- | --- | --- |
| 2026-07-31 | v1.1.271 | 按“先文档、复审、再实施”完成美甲纹理局部识别恢复：方案v0.1因根因仍属推断被拒绝，逐组件复算确认五枚甲面组件均存在，食指/无名指仅被平滑肤色环0.24硬门误杀；v1.0改为最大手部支持域、原始肤色环、局部RGB色差与既有形状门复合判定，并加入640分析、开/闭运算、五指极角弧排序和拒绝原因诊断。真实浏览器现场裁片5/5区域、5/5唯一分配、五框视觉正确，一次提取后逐指五纹理均存在，控制台0错误；诊断为33组件/5合格/5选中且背景0候选。专项8/8、ESLint、显式Python 3.13单进程全量725/725及Next.js 16.2.9生产构建通过。MediaPipe第二阶段因第一阶段已达标未触发；正式ONNX、candidate5拒绝和产品HOLD不变。 | 实施前技术方案、局部近景召回、背景隔离、五指排序、诊断JSON、测试、发布HOLD |
