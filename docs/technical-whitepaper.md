...
| 数据集治理 | `model/datasets/nail-texture-v1` | 进行中 | 正式有效集为401图、2093个mask，split=293/45/63；第二批累计导入79图（78正样本、1 hard negative）和508个mask；5张源图因甲面截断/失焦排除，剩余29张继续返修。新增1001张Claude生成图已完成只读审计，仍为未授权、未标注的合成候选池 |
