...
| 数据集治理 | `model/datasets/nail-texture-v1` | 进行中 | 正式训练集仍为409图、2142个mask，split=300/46/63。candidate5训练160张保持既有商业训练授权；candidate4编号261—360与candidate5编号361—460两批独立留出均已被相应候选消费，永久`trainingUse=prohibited`。最新保护registry v4 SHA-256为`dd7648c3…951d`，单调继承v3并含7份manifest、4份training、3份holdout、728条唯一保护记录，阻止已消费留出回流训练 |
...
| 候选6正样本补强 | `record-positive-reinforcement-authorization.py`、`build-positive-reinforcement-annotation-workspace.py`及Git外`positive-reinforcement-annotation-workspace-v1` | 进行中（初审160/160；返修真值35张/142 mask） | 用户已对`requestedItemsSha256=1cdf2825…5bfe`的160张/76组/971枚可见甲面作精确商业训练授权，授权记录`67a17f2d…1b93`深验通过。8分片/80页首轮原分辨率审核为13张通过候选、147张返修、0排除。首批删误检返修终审4张/15 mask通过；第二批逐甲紧框仅1张/5 mask通过，两张透明低对比甲经人工多边形新增10 mask；后续15张人工多边形新增66枚完整甲面，均通过polygon合法、同图零交叠、几何66/66及整图/逐甲2倍终审。另有`nail_00521…_1.jpg`因旧第5区域为白手套背景而哈希绑定为rework、未晋级。唯一训练真值索引现为35张/142 mask、0拒绝/冗余/冲突，权威与重放SHA-256均为`eb3c4b3c…eea0bb`；索引仍明确`trainingUse=prohibited-until-materialization-audit`。其余125张尚未形成最终真值，完整来源隔离和训练物化尚未完成，禁止训练、val调参、冻结test推理、导出或部署 |
