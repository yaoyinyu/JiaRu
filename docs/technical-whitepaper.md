...
| 数据集治理 | `model/datasets/nail-texture-v1` | 进行中 | 正式有效集为402图、2101个mask，split=294/45/63；第二批累计导入80图（79正样本、1 hard negative）和516个mask；5张源图因甲面截断/失焦排除，剩余28张继续返修。新增1001张Claude生成图已完成只读审计，仍为未授权、未标注的合成候选池 |
| 训练/评估/导出 | `model/training/*` | 进行中 | v4、v5、v7 与 v8 已按门禁拒绝；v6 独立真实 test box/mask mAP50=0.853/0.848，11.03MB FP32 ONNX 和浏览器 WebGPU 热推理门通过，尚缺代表性样本规模与真机/Beta 验收 |
