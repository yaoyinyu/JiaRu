...
| AR 纯色试戴 | `/ar-tryon` | 待验证 | 单手摄像头、关键点、指甲绘制、手心/手背识别已实现；MediaPipe运行文件已改为本站同源静态资源，不再依赖jsDelivr，仍需多设备真机验收 |
| AR 纹理试戴 | `/ar-tryon` | 待验证 | 上传含完整手背的参考图时，即使正式甲面模型不可用，也会通过MediaPipe Hands自动生成并逐指绑定5个甲面区域；未检测到完整手部时才进入人工补选。MediaPipe脚本、模型、图配置和WASM已由本站同源提供。实时AR默认甲面几何已增大并校准，新增75%—140%大小和甲尖/指根±20%位置微调；摄像头最终贴合仍需多设备真机验收 |
...
| 美甲纹理自动识别 | `recognizeNailTextures()`、`detectNailsFromHandImage()` | 进行中（精细模型阻塞；手部几何可用） | 浏览器模型推理、Worker和后处理已实现，但candidate5因独立困难负样本门失败而拒绝，禁止切换生产权重或阈值。上传完整手部照片可通过本站同源MediaPipe Hands资源自动生成5个逐指候选；该路径没有像素级甲面mask，不能替代正式分割模型质量结论 |
...
1. 选择“纯色”或“纹理”；
2. 纯色模式可逐指选色或应用全部；
3. 纹理模式可上传单个纹理手动裁剪，也可上传参考图提取多个甲面；正式自动模型可用时展示模型候选，模型不可用时先用MediaPipe Hands从完整手背照片自动生成五指甲面区域；只有手部关键点也失败时才进入“在图片上添加甲面”模式；
...
运行约束：当前 `maxNumHands: 1`、`modelComplexity: 0`，检测和跟踪阈值均为 `0.5`。`hands.js`、WASM 和相关 MediaPipe 文件从固定本站路径`/vendor/mediapipe/hands/`加载，脚本等待超时为15秒；部署时必须完整保留该目录，缺失或内容发布不完整会导致AR无法启动。
...
5. 手部几何定位成功时生成`source: "mediapipe"`候选并按五指索引自动分配；明确手心、无完整手部、无可靠几何或资源加载失败时才进入人工添加模式；
6. 模型候选经过置信度评估、角度稳定、手指建议和mask纹理提取；MediaPipe与人工候选使用透明椭圆裁剪；
7. UI 必须展示去重后的低置信度、提取质量或安全降级告警，并允许用户点击新增、拖动/按钮调整、重新分配、删除或取消。

当前 `NailArtPicker` 将正式模型检测输入最长边限制在 800 像素，最多请求 10 个候选，Worker 超时为 15 秒。fallback原候选不会进入可确认区域；MediaPipe静态图检测超时为20秒，使用`modelComplexity: 1`、最多1只手、检测/跟踪阈值0.5。人工新增区域默认按源图短边约8%宽、短边约16%长建立，并限制在原图边界内。候选确认后以 `NailAssignment[]` 返回，每个 `ImageBitmap` 的所有权转移给 `/ar-tryon` 页面，页面在替换或卸载时负责释放。
...
截至v1.1.266，上传完整手背参考图可通过本站同源MediaPipe资源自动生成五指甲面候选，浏览器在主动阻断jsDelivr时仍完成5/5自动区域、5/5分配与提取闭环；原CDN加载单点故障已消除。但这仍只是几何级自动定位，没有像素级甲面mask；单独甲片、局部近景、未展示完整手部、手心、严重遮挡或关键点失败仍需人工处理。正式精细自动识别继续受“无批准生产ONNX”阻塞，不能把MediaPipe几何通过表述为candidate5或后续分割模型PASS。实时AR默认几何和微调控制已改善，但尚无本轮真实摄像头画面的跨设备视觉验收。
...
| 日期 | 版本 | 变更摘要 | 影响范围 |
| --- | --- | --- | --- |
| 2026-07-30 | v1.1.266 | 修复用户现场“手部关键点自动定位加载失败”：把锁定`@mediapipe/hands@0.4.1675469240`的脚本、TFLite、graph、packed assets和SIMD/普通WASM自托管到`public/vendor/mediapipe/hands/`，新增统一同源解析器，静态多纹理定位和实时AR均移除jsDelivr运行依赖；供应商文件排除ESLint，并新增关键资产非空及源码无MediaPipe CDN契约测试。生产浏览器主动阻断jsDelivr后，完整手部画面约1.66秒自动5/5区域、5/5分配并成功提取，所有关键资源同源HTTP 200、控制台0错误。完整手部几何边界、正式ONNX阻塞、candidate5拒绝及产品HOLD不变。 | MediaPipe自托管、自动纹理定位、实时AR启动、部署静态资源、测试、发布HOLD |
