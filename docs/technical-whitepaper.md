...
| 数据集治理 | `model/datasets/nail-texture-v1` | 进行中 | 正式训练集仍为409图、2142个mask，split=300/46/63。新增101张真实素材中9张跨旧批次精确重复排除；其余92张仅用于独立发布测试与长期回归，其中67张/384 mask冻结为受审核候选、25张排除、18个父来源组，训练用途始终禁止。2026_7_14目录1271张候选经近重复与26个来源组原子分片审核，565张进入待标注候选、601张排除；565张按来源组互斥规划为train 502、val 30、独立发布test 33，首批train选择160张/39来源组。首批SAM2运行160/160、0错误、0 fallback，几何审计796 pass、85 suspect；原分辨率初审完成160/160张，15张暂通过、138张返修、7张因裁断或遮挡残缺排除。当前101个批准报告归并为100张唯一图片/521个完整mask、1个冗余、0冲突，已达到最低100张train正样本门。批次067对`00538/00301`的10个SAM候选执行原分辨率视觉门，拦截整指、皮肤、衣物与背景污染后全部改用人工polygon；逐甲2×复核补齐透明甲尖、立体花饰和水钻边缘，一次38.5285像素真实交叠被构建器拒绝并纠正，终版2图10甲合法、零交叠、几何10/10。SAM与人工polygon都必须继续通过原分辨率视觉、几何、合法性和同图零交叠审核。val真值仍为0/30，约100张hard negative尚未建立；全部真值还须完成整批物化与来源隔离审计，训练用途继续禁止。1001张Claude生成图仍为未授权、未标注的合成候选池 |
| 训练真值最新增量 | 外部`training-truth-index-v1.json` | 进行中 | v1.1.167权威快照：80个批准报告归并为79张唯一图片/416个完整mask，1个冗余报告、0冲突，索引SHA-256为`6e4dadf9b7378c7f7a96c1b4c1eabacd6c7e587707a328e47e77d25d5bce378d`。批次058的`00531/00301/00489`新增3张/15 mask，批次059的`00194/00906`新增2张/9 mask；两批均以原分辨率整图和逐甲2×视觉门拦截并修复局部甲面、漏甲、皮肤、绸布、头发及猫毛背景污染，最终24/24 polygon合法、零交叠、几何全通过。最低100张train正样本完成79%、仍缺21张；val 0/30与约100张hard negative不变，整批物化和来源隔离前继续禁止训练。本行替代上方数据集治理长行中的旧数量快照 |
| 训练真值当前权威快照 | 外部工作区根目录`training-truth-index-v1.json` | 进行中 | v1.1.177：101个批准报告归并为100张唯一图片/521个完整mask，2个历史拒绝报告、1个冗余报告、0冲突，索引SHA-256为`13f606b547c32d2b8f34651f55e1bca1e826bf3ac13bdcdca345a1cef267f125`。批次067的`00538/00301`新增2张/10 mask；`01113`因仅4枚人类甲面完整可见而未进入本批。SAM2.1 large完成2图/10提示、1 fallback，初轮几何3 pass/7 suspect且视觉门发现整指、皮肤、衣物及背景污染，终版10枚甲面全部以人工polygon重建并通过整图与逐甲2×视觉审核、合法性、同图零交叠和几何10/10。最低100张train正样本门已达到；val 0/30、约100张hard negative、整批物化和来源隔离仍未完成，训练用途继续禁止。权威索引位于审核工作区根目录；`final/training-truth-index-v1.json`现为字节一致的当前镜像。本行取代v1.1.176及更早数量快照 |
| 验证真值当前快照 | 外部`val-annotation-workspace-v1/validation-truth-index-v1.json` | 进行中 | v1.1.179：30张/7个val-only来源组/159枚预期完整甲面的首轮审核已完成；角色感知终审当前批准6个报告，归并为6张唯一图片/24个完整mask、0冗余、0冲突，索引SHA-256为`adac6dc351eb06194f06e1a6991c49cf78db10658328d9daea81da7fc6b333c7`。`00456`直通；`00454/00457/00384`经误检删除或紧框SAM重建；`00829/00672`在SAM仍吸收皮肤或局部甲后改用人工polygon。所有6项绑定`assignedRole=val`且`trainingUse=prohibited`，通过原分辨率整图/逐甲2×、合法性、零交叠和几何审核。剩余24张仍须返修；按整套split全通过规则，30/30完成和物化审计前仍禁止阈值校准及候选训练 |
...
1. v6资产、协议和桌面性能证据有效，但冻结67张/384 mask在部署512下的扩展质量门失败，正式拒绝发布；部署阈值画像进一步确认压力组召回仅0.6983，透明相邻长甲、多甲同屏、低对比漏整甲/局部识别及手指/腕表误检是首要方向。代表性下限仍缺33张。2026_7_14当前1271张候选已获A授权并完成近重复视觉归并，105张重复或非照片候选排除；剩余1166张/193来源组已完成源图筛选，565张仅进入待标注候选、601张排除、0张待审，批次唯一覆盖终结门通过。565张已按来源组互斥规划为train 502、val 30、独立发布test 33；首批160张正样本的自动候选已生成881个polygon，机器几何为796 pass、85 suspect。mask审核工作区按39个来源组原子切成10个分片/83页，现已完成160/160原分辨率初审：15张暂通过、138张返修、7张因甲面裁断或遮挡残缺排除；分片009—010及跨分片批次011—018累计提升26张，批次019—023又拦截两张视觉边界不可信的零候选图。当前54个批准报告经唯一性索引归并为53张唯一图片/287 mask，1个重复报告绑定相同annotation、0冲突；新近人工返修`00704`、`00533`、`00531`、`00951`及同源`00274/00276/00277/00278`共新增40个合法且零交叠mask，均拦截或清除了几何通过但视觉污染的SAM/初始候选。初审覆盖已闭环但返修真值仍大量未完成，val真值仍为0/30。按最低候选训练包仍缺至少47张合格train正样本、30张来源隔离val和约100张独立合格hard negative，随后还需数据物化、来源隔离、polygon合法性与同图零交叠终审；所有计划项仍不能直接训练。同时仍缺用户典型失败案例、四类移动真机和至少100张Beta人工质量审核，正式模型审计继续blocked；
