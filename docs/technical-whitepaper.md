...
| 数据集治理 | `model/datasets/nail-texture-v1` | 进行中 | 正式训练集仍为409图、2142个mask，split=300/46/63。第二候选规范包为train258（100张正样本/521 mask + 158张零字节标签困难负样本）、val30/144 mask、test0、孤儿0；物化报告SHA-256为`033b8d02…2ec`，文件树SHA-256为`8709fa80…1170`，GPU前输入深审SHA-256为`b9bd6897…94dd`，与冻结test100张/554 mask零重叠。2026-07-24批160张已按原分辨率严格审核为158张通过、2张排除，schema v2批准清单SHA-256为`5e91f8ec…a265`。该批已被上一候选筛选并用于第二候选训练，只能作为训练/诊断证据；训练后新来源独立留出正在另行生成，必须在首次v2推理前冻结全批、严格排除低清/残缺/真人甲面，并保留不少于100张通过三变体零误检/零delta门 |
...
截至v1.1.217，158张批准困难负样本已与100张正样本物化为train258，保持val30/144 mask来源隔离、test0和孤儿0；输入深审重放通过后使用RTX 4060完成50轮续训，第二候选权重SHA-256为`e46968b7…3882`。规范val30部署512 box/mask mAP50为0.8788/0.8364，阈值0.45通过校准器深验；冻结test100的full/core/stress为0.9501/0.9454、0.9508/0.9567、0.9472/0.9221，质量报告SHA-256为`b2b7167d…c16e`并通过schema v2只读重放。上一候选在本轮训练负样本原图/裁切/模糊三变体分别误检52/65/52张，第二候选在0.45下三类均0误检，证明训练抑制目标达成；但该报告固定为training角色、`releaseGeneralizationEligible=false`，不能替代新来源独立留出。跨候选比较还显示core box mAP50下降0.02073，略超0.02诊断线，必须与后续独立留出、移动真机和Beta证据共同评估。训练后新批100张留出已按不同提示族开始并行生成，首次v2推理前必须先完成全批冻结和原分辨率质量审核；生产manifest、共享阈值和正式ONNX保持不变。
