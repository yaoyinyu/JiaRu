...
interface Props { nailColors: string[]; }
...
export function ArView({ nailColors }: Props) {
  ...
  useEffect(() => {
    let dead = false, rafId = 0;
    let handsInst: any = null, streamInst: MediaStream | null = null;
    ...
    async function start() {
      try {
        ...
        await new Promise<void>((resolve, reject) => {
          ...
          s.onload = () => { log("  hands.js 加载完成 ✅"); resolve(); };
        ...
        await new Promise<void>((resolve, reject) => {
          const s = document.createElement("script");
          s.src = "https://cdn.jsdelivr.net/npm/@mediapipe/camera_utils@0.3.1675466862/camera_utils.js";
          s.crossOrigin = "anonymous";
          s.onload = () => { log("  camera_utils.js 加载完成 ✅"); resolve(); };
        ...
        if (!H) { log("  ❌ Hands 未注册到 window"); return; }
        if (!C) { log("  ❌ Camera 未注册到 window"); return; }
        log("  Hands ✅ Camera ✅");
        ...
        if (!video) { log("  ❌ video ref 为空"); return; }

        // Camera 工具类会自动处理 getUserMedia
        log("  创建 Camera 实例...");
        const camera = new C(video, {
          onFrame: async () => {
            try { await handsInst.send({ image: video }); } catch (e: any) {
              log("send: " + (e?.message || "").slice(0, 60));
            }
          },
        ...
        handsInst.onResults((res: any) => {
          ...
          if (res.multiHandLandmarks?.length) {
            if (handCnt < 0) log("5/6 首次检测到手部! ✅");
            setHandCnt(res.multiHandLandmarks.length);
            for (let h = 0; h < res.multiHandLandmarks.length; h++) {
              paintNails(ctx, res.multiHandLandmarks[h], colRef.current, cvs.width, cvs.height);
            }
          } else {
            if (handCnt > 0) setHandCnt(0);
          }
        });
        ...
        if (!dead) {
          setStatus("error");
          setStatusMsg(m.includes("permission") ? "权限被拒绝" : "启动失败");
        }
      }
    }
  ...
  function paintNails(ctx: CanvasRenderingContext2D, lm: any[], colors: string[], w: number, h: number) {
    for (let f = 0; f < 5; f++) {
      const tip = lm[TIPS[f]], dip = lm[DIPS[f]], color = colors[f] || "#E8A0BF";
      if (!tip || !dip) continue;
      const tx = tip.x * w, ty = tip.y * h, dx = dip.x * w, dy = dip.y * h;
      const fx = tx - dx, fy = ty - dy, len = Math.sqrt(fx*fx + fy*fy);
      if (len < 5) continue;
      const a = Math.atan2(fy, fx), nl = len * 0.52, nw = len * 0.42;
      const cx = tx - (fx/len)*(len*0.18), cy = ty - (fy/len)*(len*0.18);
      ctx.save(); ctx.translate(cx, cy); ctx.rotate(a);
      ctx.beginPath(); ctx.ellipse(0, 0, nl*0.5, nw*0.5, 0, 0, Math.PI*2);
      ctx.fillStyle = color; ctx.globalAlpha = 0.8; ctx.fill();
      ctx.globalAlpha = 0.4; ctx.fillStyle = "white";
      ctx.beginPath(); ctx.ellipse(-nl*0.05, -nw*0.18, nl*0.3, nw*0.12, 0, 0, Math.PI*2); ctx.fill();
      ctx.restore();
    }
  }
  ...
  return (
    <div className="relative w-full max-w-[480px] mx-auto rounded-2xl overflow-hidden bg-black shadow-lg">
      <video ref={vref} playsInline muted className="w-full aspect-[3/4] object-cover" style={{ transform: "scaleX(-1)" }} />
      <canvas ref={cref} className="absolute inset-0 w-full h-full pointer-events-none" style={{ transform: "scaleX(-1)" }} />
      ...
      <div className="absolute top-1 left-1 right-1 z-20 bg-black/80 rounded-lg p-2 max-h-40 overflow-y-auto border border-pink-500/30">
        {diag.map((d, i) => (
          <p key={i} className={`text-[9px] font-mono leading-tight mb-0.5 ${
            d.includes("❌") ? "text-red-300" :
            d.includes("✅") ? "text-green-300" :
            d.includes("首次") ? "text-yellow-200" :
            "text-gray-300"
          }`}>{d}</p>
        ))}
      </div>
      ...
      {status === "ready" && (
        <div className="absolute bottom-0 left-0 right-0 pb-3 text-center pointer-events-none z-10">
          <span className="inline-block text-xs text-white/70 bg-black/30 px-3 py-1 rounded-full backdrop-blur-sm">
            {handCnt > 0 ? "检测到 " + handCnt + " 只手" : handCnt === 0 ? "请将手放在摄像头前" : "等待检测..."}
          </span>
        </div>
      )}
    </div>
