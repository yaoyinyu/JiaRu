#!/usr/bin/env python3
"""Bind isolated source copies and untinted old-mask crops for shard002 37/39."""
...
def build(shard_path: Path, progress_path: Path, output_dir: Path) -> dict:
    shard, progress = map(load, (shard_path, progress_path))
    if (shard["shard"]["shard"] != 2
            or shard["shard"]["sourceImages"] != 20
        ...
        raise ValueError("frozen shard/progress contract mismatch")
    ...
    for ordinal in ORDINALS:
        ...
        if (state["decision"] != "watermark_review_required"
                or state["trainingUse"] != "prohibited"
                or state["everyNailVisualApproval"]
                or source["sourceFileName"] != state["sourceFileName"]
                or source["sourceGroup"] != state["sourceGroup"]
                or source["sourceImage"] != state["sourceImage"]
                or source["sourceLabel"] != state["sourceLabel"]
        ...
        for key in ("sourceImage", "sourceLabel", "overview"):
            checked(source[key])
        isolated = output_dir / "images" / source["sourceFileName"]
        isolated.parent.mkdir(parents=True, exist_ok=True)
        if not isolated.exists():
            shutil.copyfile(source["sourceImage"]["path"], isolated)
        if sha(isolated) != source["sourceImage"]["sha256"]:
            raise ValueError(f"source {ordinal} isolated byte copy drift")
        with Image.open(isolated) as opened:
    ...
    return {"schemaVersion": 1, "ok": True,
            "decision": "source037039_old_polygon_original_resolution_review_pending",
            "inputs": {"sourceScript": bind(Path(__file__)),
                       "shardReport": bind(shard_path),
                       "progressV13": bind(progress_path)},
