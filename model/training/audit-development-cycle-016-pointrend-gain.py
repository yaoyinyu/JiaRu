#!/usr/bin/env python3
"""重建第七/第八轮逐实例预测并审计PointRend式精细化增益。"""
...
def validate_plan(plan: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], Path]:
    ...
    return seventh, eighth, prior, Path(dataset_report["outputDir"])
...
def compute(plan: dict[str, Any]) -> dict[str, Any]:
    seventh_report, eighth_report, prior_report, dataset_root = validate_plan(plan)
    ...
    seventh_architecture = load_module("audit-development-cycle-016-dinov2-highres-decoder-feasibility.py", "cycle016_pointrend_gain_seventh_architecture")
    seventh_trainer = load_module("train-development-cycle-016-soft-boundary-f1-pilot.py", "cycle016_pointrend_gain_seventh_trainer")
    eighth_architecture = load_module("audit-development-cycle-016-pointrend-feasibility.py", "cycle016_pointrend_gain_eighth_architecture")
    eighth_trainer = load_module("train-development-cycle-016-pointrend-pilot.py", "cycle016_pointrend_gain_eighth_trainer")
    ...
    eighth_model = load_frozen_model(eighth_report, Path(plan["inputs"]["eighthWeights"]["path"]), eighth_architecture, eighth_trainer, device)
    ...
    try:
        for images, truth, identities in loader:
            ...
            with torch.amp.autocast(device_type="cuda"):
                ...
                eighth_logits = eighth_model(pixel_values=images_device)
            seventh_predictions = (torch.sigmoid(seventh_logits) >= 0.5).cpu().numpy()[:, 0]
            eighth_predictions = (torch.sigmoid(eighth_logits) >= 0.5).cpu().numpy()[:, 0]
            truths = truth.numpy()[:, 0] >= 0.5
            for identity, target, seventh_prediction, eighth_prediction in zip(identities, truths, seventh_predictions, eighth_predictions, strict=True):
                key = str(identity)
                seventh_value = metrics(target, seventh_prediction, base)
                eighth_value = metrics(target, eighth_prediction, base)
                rows.append({"id": key, "localBoundaryContrast": contrast_by_id[key], "lowContrastQuartile": key in low_ids, "seventh": seventh_value, "eighth": eighth_value, "delta": {name: eighth_value[name] - seventh_value[name] for name in seventh_value}})
    finally:
        del seventh_model, eighth_model
    ...
    eighth_aggregate = aggregate(rows, "eighth", thresholds)
    ...
    eighth_fidelity = fidelity(eighth_aggregate, eighth_report["best"]["validation"], tolerance)
    if [row["id"] for row in rows] != expected_ids or not seventh_fidelity["ok"] or not eighth_fidelity["ok"]:
        return {"decision": "reconstruction_mismatch", "idOrderMatches": [row["id"] for row in rows] == expected_ids, "seventhAggregate": seventh_aggregate, "eighthAggregate": eighth_aggregate, "seventhFidelity": seventh_fidelity, "eighthFidelity": eighth_fidelity}
    ...
    mean_gain = float(eighth_aggregate["meanBoundaryF1"] - seventh_aggregate["meanBoundaryF1"])
    iou_delta = float(eighth_aggregate["meanIou"] - seventh_aggregate["meanIou"])
    ...
    boundary_iou_pass_fraction = eighth_aggregate["instanceGateCounts"]["boundaryIou"] / required
    architecture_limited = eighth_aggregate["meanBoundaryF1"] < float(contract["architectureLimitedMeanBoundaryF1Ceiling"]) or boundary_iou_pass_fraction < float(contract["architectureLimitedMaximumBoundaryIouPassFraction"])
    if meaningful and architecture_limited:
        decision = "pointrend_effective_but_quality_gate_limited"
    elif meaningful:
        decision = "pointrend_effective_keep_branch"
    else:
        decision = "pointrend_not_broadly_effective_close_branch"
    integrity = materializer.verify(Path(plan["inputs"]["datasetMaterializationReport"]["path"]))
    return {
        "decision": decision,
        "idOrderMatches": True,
        "seventhAggregate": seventh_aggregate,
        "eighthAggregate": eighth_aggregate,
        "seventhFidelity": seventh_fidelity,
        "eighthFidelity": eighth_fidelity,
        ...
        "remainingGap": {"meanBoundaryF1ToGate": float(0.9 - eighth_aggregate["meanBoundaryF1"]), "jointInstancePassRateToGate": float(0.8 - eighth_aggregate["jointInstancePassRate"])},
...
def verify(report_path: Path) -> dict[str, Any]:
    ...
    if replay != report["analysis"] or canonical_sha256(replay) != report["analysisSha256"]:
        raise ValueError("PointRend增益审计重放不一致")
    if not report.get("ok") or report.get("decision") == "reconstruction_mismatch":
        raise ValueError("PointRend增益审计保真门未通过")
