#!/usr/bin/env python3
"""在循环015 batch5开发数据集上只追加batch5的679张图3975 mask，并生成可重放物化报告。

单变量短训实验专用（用户2026-09-19『继续训练，不要停』指令；沿用三条裁决：甲面级过滤、裁断输出通道、免审直接训练）。
基础 = 2026_9_19_cycle015_real_material_batch4_development_dataset_v1（datasetFilesSha256 e20497c4…cabe）。
...
偏差声明：batch5 真值为模型预标注多边形（用户明确裁决免审直接训练并继续训练循环），
...
EXPECTED_REAL_IMAGES = 679
EXPECTED_REAL_MASKS = 3975
FOLD_NAME = "cycle015-real-material-batch5-positive"
DATASET_VERSION_OLD = "development-cycle-015-real-material-batch4/v1"
DATASET_VERSION_NEW = "development-cycle-015-real-material-batch5/v1"
FROZEN_IMAGE_DIR = Path(r"E:/AI Project/Codex/JiaRu_image/真实素材/2026_9_19_cycle015_real_material_v5")
WORKSPACE = Path(r"E:/AI Project/Codex/JiaRu_image/审核工作区/2026_9_19_cycle015_real_material_annotation_workspace_v7")
...
BATCH2_MODULE_PATH = Path(r"E:/AI Project/Codex/JiaRu/model/training/materialize-development-cycle-015-real-material-batch4-dataset.py")
...
def load_batch5_truths() -> list[dict[str, Any]]:
    ...
    if len(truths) != EXPECTED_REAL_IMAGES:
        raise ValueError(f"batch5条目数不符：期望{EXPECTED_REAL_IMAGES}，实际{len(truths)}")
    total = sum(item["maskCount"] for item in truths)
    if total != EXPECTED_REAL_MASKS:
        raise ValueError(f"batch5 mask总数不符：期望{EXPECTED_REAL_MASKS}，实际{total}")
    if len({item["imageSha256"] for item in truths}) != EXPECTED_REAL_IMAGES or len({item["sourceGroup"] for item in truths}) != EXPECTED_REAL_IMAGES:
        raise ValueError("batch5身份不唯一")
    return truths


def load_batch5_materializer():
    spec = importlib.util.spec_from_file_location("materialize_development_cycle_015_real_material_batch5_dataset_for_training", BATCH2_MODULE_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot load cycle015 batch5 real-material materializer")
...
def build_report(base_path: Path, output_dir: Path, *, materialize: bool) -> dict[str, Any]:
    ...
    if (
        ...
        or base.get("decision") != "development_cycle_015_real_material_batch4_dataset_materialized_for_single_short_experiment"
    ...
    return {
        ...
        "ok": True,
        "status": "PASS",
        "decision": "development_cycle_015_real_material_batch5_dataset_materialized_for_single_short_experiment",
        ...
        "inputs": {
            ...
            "freezeManifestV5": binding(FROZEN_IMAGE_DIR / "freeze-manifest-v5.json"),
            ...
            "truthProvenanceDeviation": {
                ...
                "new": "batch5：YOLO(cycle012 best)预标注多边形直接作为训练真值，conf≥0.6 + bbox边距≥8px统一过滤（无白名单），30图全取自新文件夹2",
            ...
            "gateDeviationPreRegistered": {
                ...
                "new": "+679 batch5真实素材图/+3975 mask在batch4数据集上第五次单变量长训（累计真实素材账837图）",
...
def verify_report(report_path: Path) -> dict[str, Any]:
    """重放batch5物化报告（含batch5基底报告强校验），供训练入口绑定同一文件树身份。"""

    report_path = report_path.resolve()
    existing = read_object(report_path, "待重放报告")
    inputs = existing.get("inputs") or {}
    load_batch5_materializer().verify_report(Path(inputs["baseMaterialization"]["path"]).resolve())
    replay = build_report(
