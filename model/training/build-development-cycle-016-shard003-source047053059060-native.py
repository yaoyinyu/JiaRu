...
TARGETS = (47, 53, 59, 60)
EXPECTED_NAILS = {47: 5, 53: 5, 59: 5, 60: 5}
...
def verify_images(report: dict) -> None:
    if report["counts"] != {"sourceImages": 4, "oldLabelNails": 20, "visualApprovals": 0}:
...
def build(shard_path: Path, triage_path: Path, output: Path) -> dict:
    ...
    for ordinal in TARGETS:
        ...
        if (len(historical) != (1 if ordinal == 47 else 0) or
        ...
        records.append({"ordinal": ordinal, "sourceFileName": source["sourceFileName"],
                        ...
                        "nails": nails, "sourceQualityDecision": "pending_full_nail_repair_review"})
    report = {"schemaVersion": 1, "ok": True,
              ...
              "counts": {"sourceImages": 4, "oldLabelNails": 20, "visualApprovals": 0},
              "sources": records,
...
def verify(report_path: Path) -> dict:
    ...
    for row in report["sources"]:
        ...
        if (row["sourceFileName"] != source["sourceFileName"] or
                ...
                row["sourceQualityDecision"] != "pending_full_nail_repair_review" or
