#!/usr/bin/env python3
"""Reconcile frozen shard-003 denominator after source-45 quality stoploss."""
...
EXPECTED = {
    "source_quality_excluded": {"sourceImages": 5, "oldLabelNails": 32},
    "mask_visual_pass_role_pending": {"sourceImages": 3, "oldLabelNails": 15},
    "mask_rework": {"sourceImages": 3, "oldLabelNails": 14},
...
def build(prior_path: Path, stoploss_path: Path) -> dict:
    ...
    if (prior["decision"] != "shard003_progress_v2_reconciled_training_still_prohibited" or
            prior["counts"]["sourceImages"] != 20 or
            prior["counts"]["oldLabelNails"] != 105 or
            prior["counts"]["sourceGroups"] != 10 or
            stoploss["decision"] != "exclude_source045_next_train_partial_nail_occlusion" or
            stoploss["counts"] != {"sourceImagesExcluded": 1,
    ...
    for item in stoploss["inputs"].values():
        checked(item)
    previous = next(row for row in prior["sources"] if row["ordinal"] == 45)
    focus = stoploss["source"]
    if (previous["currentDecision"] != "mask_rework" or
            previous["oldLabelNails"] != 10 or
            focus["ordinal"] != 45 or
        ...
        raise ValueError("source 45 identity or role drift")
    rows = []
    ...
    return {
        ...
        "decision": "shard003_progress_v3_reconciled_training_still_prohibited",
        "inputs": {"sourceScript": bind(Path(__file__)),
                   "priorProgress": bind(prior_path), "source045Stoploss": bind(stoploss_path)},
...
def main() -> None:
    ...
    parser.add_argument("--source045-stoploss", type=Path)
    ...
    if args.verify_report:
        ...
        current = build(Path(saved["inputs"]["priorProgress"]["path"]),
                        Path(saved["inputs"]["source045Stoploss"]["path"]))
        if current != saved:
            raise SystemExit("reconstruction_mismatch")
        result = {"ok": True, "decision": "verified_shard003_progress_v3",
                  "counts": saved["counts"]}
    else:
        if not args.prior_progress or not args.source045_stoploss or not args.report:
            parser.error("--prior-progress, --source045-stoploss, and --report required")
        if args.report.exists():
            raise FileExistsError(args.report)
        result = build(args.prior_progress.resolve(), args.source045_stoploss.resolve())
