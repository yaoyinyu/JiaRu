#!/usr/bin/env python3
"""训练cycle016第四轮DINOv2-small逐甲ROI开发pilot。"""
...
def build_model(plan: dict[str, Any], architecture: Any) -> torch.nn.Module:
    ...
    return architecture.Dinov2RoiSegmenter(backbone)
...
def validate_inputs(plan: dict[str, Any]) -> tuple[dict[str, Any], Path, Path]:
    ...
    for label, path_key, hash_key in (
        ("商用可行性报告", "commercialFeasibilityReport", "commercialFeasibilityReportSha256"),
    ...
    if browser.get("decision") != "commercial_pretrained_backbone_browser_feasible_for_fourth_pilot":
...
def train(plan_path: Path, output: Path) -> dict[str, Any]:
    ...
    architecture = load_module("audit-development-cycle-016-commercial-pretrained-backbone.py", "cycle016_dino_architecture")
    ...
    optimizer = torch.optim.AdamW(
        [
            ...
            {"params": list(model.decoder.parameters()) + list(model.refine.parameters()), "lr": float(config["decoderLearningRate"])},
        ],
    ...
    result = {
        "schemaVersion": 1,
        "ok": passed,
        "decision": "roi_dinov2_small_internal_pilot_pass" if passed else "roi_dinov2_small_internal_pilot_fail",
        "scope": plan["scope"],
        "inputs": {
            ...
            "commercialFeasibilityReport": {"path": plan["prerequisites"]["commercialFeasibilityReport"], "sha256": plan["prerequisites"]["commercialFeasibilityReportSha256"]},
        ...
        "architecture": {"name": "DINOv2-small ViT-S/14 plus fixed full-resolution ROI decoder", "parameterCount": parameter_count, "outputStride": 1, "singleChangedVariable": plan["singleChangedVariable"]},
        "training": config,
...
def verify(report_path: Path) -> dict[str, Any]:
    ...
    return {"ok": True, "decision": "verified_roi_dinov2_small_pilot", "pilotDecision": report["decision"], "best": report["best"]}
