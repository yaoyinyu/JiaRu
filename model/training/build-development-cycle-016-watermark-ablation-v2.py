...
def build(preflight_path: Path, shard_path: Path, output_dir: Path, *, verify: bool) -> dict:
    ...
    safe &= ~guarded
    # V1 colour threshold missed the shadowed middle letters on visual review.
    # Treat the whole source watermark window as evidence of potentially visible
    # text, while excluding the exact nail masks and their protective margin.
    mask = safe.astype(np.uint8) * 255
    if not 10000 <= int(safe.sum()) <= 19000:
        raise ValueError("watermark window out of expected range")
    removed = cv2.inpaint(image, mask, 9, cv2.INPAINT_TELEA)
    occluded = image.copy()
    ...
    blurred = image.copy()
    gaussian = cv2.GaussianBlur(image, (0, 0), 13)
    blurred[safe] = gaussian[safe]
    moved = removed.copy()
    dy, dx = 91, -75
    ...
    return {"schemaVersion": 1, "ok": True,
            ...
            "watermarkMaskPixels": int(safe.sum()),
