#!/usr/bin/env python3
"""Bind original-pixel source-quality stoploss for shard-003 sources 54/55."""

from __future__ import annotations

import argparse
import hashlib
import json
from pathlib import Path


FOCUS = {
    54: {
        "hardFailureNail": 1,
        "reason": "side_view_nail_root_merges_with_fingertip_without_confirmable_full_boundary",
        "otherDefects": [
            "nail_4_old_polygon_extends_past_visible_tip_onto_book",
            "nail_5_old_polygon_omits_transparent_area_against_page",
        ],
        "oldLabelNails": 5,
    },
    55: {
        "hardFailureNail": 4,
        "reason": "side_view_transparent_nail_root_and_lateral_boundary_not_confirmable",
        "otherDefects": [
            "nail_1_old_polygon_includes_book_lettering_under_clear_tip",
            "nail_2_old_polygon_omits_transparent_distal_tip",
            "nail_3_old_polygon_omits_nail_root_and_includes_book_page",
        ],
        "oldLabelNails": 4,
    },
}


def sha(path: Path) -> str:
    return hashlib.sha256(path.read_bytes()).hexdigest()


def bind(path: Path) -> dict:
    path = path.resolve()
    return {"path": str(path), "sha256": sha(path)}


def checked(item: dict) -> Path:
    path = Path(item["path"])
    if not path.is_file() or sha(path) != item["sha256"]:
        raise ValueError(f"bound evidence drift: {path}")
    return path


def build(native_path: Path, preflight_path: Path, progress_path: Path) -> dict:
    native = json.loads(native_path.read_text(encoding="utf-8"))
    preflight = json.loads(preflight_path.read_text(encoding="utf-8"))
    progress = json.loads(progress_path.read_text(encoding="utf-8"))
    for document in (native, preflight, progress):
        role = document.get("trainingUse", document.get("scope", {}).get("trainingUse"))
        if not document.get("ok") or role != "prohibited":
            raise ValueError("upstream decision or training role drift")
        for item in document["inputs"].values():
            checked(item)
    if (native["decision"] != "isolated_original_resolution_repair_crops_pending_visual" or
            native["counts"] != {"sourceImages": 2, "oldLabelNails": 9, "visualApprovals": 0} or
            [row["ordinal"] for row in native["sources"]] != [54, 55] or
            preflight["decision"] != "four_remaining_reworks_identity_verified_source045_first" or
            progress["decision"] != "shard003_progress_v3_reconciled_training_still_prohibited" or
            progress["counts"]["sourceImages"] != 20 or
            progress["counts"]["oldLabelNails"] != 105):
        raise ValueError("upstream report mismatch")
    rows = []
    for source in native["sources"]:
        ordinal = source["ordinal"]
        expected = FOCUS[ordinal]
        prior = next(row for row in progress["sources"] if row["ordinal"] == ordinal)
        preflight_row = next(row for row in preflight["sources"] if row["ordinal"] == ordinal)
        if (source["sourceImage"] != prior["sourceImage"] or
                source["sourceImage"] != preflight_row["sourceImage"] or
                source["sourceGroup"] != prior["sourceGroup"] or
                source["sourceGroup"] != preflight_row["sourceGroup"] or
                source["sourceLabel"] != preflight_row["sourceLabel"] or
                source["sourceImage"]["sha256"] != source["isolatedImage"]["sha256"] or
                prior["currentDecision"] != "mask_rework" or
                prior["trainingUse"] != "prohibited" or
                len(source["nails"]) != expected["oldLabelNails"] or
                len(source["historicalCanonicalTruths"]) != 1):
            raise ValueError(f"source {ordinal} identity, history, or role mismatch")
        checked(source["sourceImage"])
        checked(source["sourceLabel"])
        checked(source["isolatedImage"])
        nail = source["nails"][expected["hardFailureNail"] - 1]
        if nail["truthIndex"] != expected["hardFailureNail"]:
            raise ValueError("hard failure nail index drift")
        for row in source["nails"]:
            checked(row["raw3x"])
            checked(row["oldOutline3x"])
        rows.append({
            "ordinal": ordinal,
            "sourceFileName": source["sourceFileName"],
            "sourceGroup": source["sourceGroup"],
            "sourceImage": source["sourceImage"],
            "sourceLabel": source["sourceLabel"],
            "isolatedImage": source["isolatedImage"],
            "hardFailure": {
                "nailIndex": expected["hardFailureNail"],
                "reason": expected["reason"],
                "raw3x": nail["raw3x"],
                "oldOutline3x": nail["oldOutline3x"],
            },
            "otherObservedOldMaskDefects": expected["otherDefects"],
            "historicalCanonicalTruthCount": 1,
            "oldLabelNailsPreserved": expected["oldLabelNails"],
            "decision": "source_quality_excluded_next_train_only",
            "trainingUse": "prohibited",
        })
    if rows[0]["sourceGroup"] != rows[1]["sourceGroup"]:
        raise ValueError("same-source-group claim drift")
    return {
        "schemaVersion": 1,
        "ok": True,
        "decision": "exclude_sources054055_next_train_unconfirmable_side_nail_boundaries",
        "inputs": {"sourceScript": bind(Path(__file__)),
                   "nativeReview": bind(native_path),
                   "preflight": bind(preflight_path),
                   "priorProgress": bind(progress_path)},
        "counts": {"sourceImagesExcludedNextTrain": 2,
                   "oldNailsPreservedInHistoricalSnapshot": 9,
                   "sourceGroups": 1, "newTrainingApproved": 0},
        "sources": rows,
        "historicalSnapshotChanged": False,
        "historicalCanonicalChanged": False,
        "protectedRolesChanged": False,
        "sourceImageOrLabelChanged": False,
        "limitations": "Original-pixel visual exclusion is a human judgment; hash replay binds its evidence and does not turn old polygon errors into historical PASS.",
        "trainingUse": "prohibited",
    }


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--native-review", type=Path)
    parser.add_argument("--preflight", type=Path)
    parser.add_argument("--prior-progress", type=Path)
    parser.add_argument("--report", type=Path)
    parser.add_argument("--verify-report", type=Path)
    args = parser.parse_args()
    if args.verify_report:
        saved = json.loads(args.verify_report.read_text(encoding="utf-8"))
        for item in saved["inputs"].values():
            checked(item)
        current = build(*(Path(saved["inputs"][key]["path"]) for key in
                          ("nativeReview", "preflight", "priorProgress")))
        if current != saved:
            raise SystemExit("reconstruction_mismatch")
        result = {"ok": True, "decision": "verified_source054055_quality_stoploss",
                  "counts": saved["counts"]}
    else:
        if not all((args.native_review, args.preflight, args.prior_progress, args.report)):
            parser.error("--native-review, --preflight, --prior-progress, and --report required")
        if args.report.exists():
            raise FileExistsError(args.report)
        result = build(args.native_review.resolve(), args.preflight.resolve(),
                       args.prior_progress.resolve())
        args.report.parent.mkdir(parents=True, exist_ok=True)
        args.report.write_text(json.dumps(result, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({key: result[key] for key in ("ok", "decision", "counts")}, ensure_ascii=False))


if __name__ == "__main__":
    main()
