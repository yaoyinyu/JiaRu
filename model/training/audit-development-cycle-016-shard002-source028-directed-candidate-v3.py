#!/usr/bin/env python3
"""Audit source28's one directed SAM retry and freeze native review crops."""
...
def build(prompts_path: Path, sam_path: Path, crop_dir: Path,
    ...
    if (prompts["decision"] != "source028_nail3_one_directed_sam_retry_candidates_only" or
    ...
    for binding in prompts["inputs"].values():
        checked(binding)
    prior_prompts = prompts["revisionOf"]
    checked(prior_prompts)
    original_annotation_path = prompts_path.parent / "annotations" / "nail_00293_69cf27bb000000001a028441_4.json"
    old_annotation = json.loads(original_annotation_path.read_text(encoding="utf-8"))
    ...
    if len(old_annotation["annotations"]) != 5 or any(
        annotation["annotations"][index]["polygon"] != old_annotation["annotations"][index]["polygon"]
        for index in (0, 1, 3, 4)
    ) or annotation["annotations"][2]["polygon"] == old_annotation["annotations"][2]["polygon"]:
        raise ValueError("directed retry changed nails other than nail3 or failed to change nail3")
    ...
    return {"schemaVersion": 1, "ok": True,
            "decision": "source028_directed_sam_candidate_geometry_review_visual_pending",
            "inputs": {"sourceScript": bind(Path(__file__)), "prompts": bind(prompts_path),
                       ...
                       "fullOverlay": bind(overlay_path), "originalAnnotation": bind(original_annotation_path)},
