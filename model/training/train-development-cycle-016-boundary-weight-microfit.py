#!/usr/bin/env python3
"""双向高分辨率网络只把soft Boundary IoU边界权重0.2改为1.0的固定32例微型门。"""
...
def train(plan_path: Path, output: Path) -> dict[str, Any]:
    ...
    gradient = json.loads(Path(plan["inputs"]["boundaryWeightGradientReport"]["path"]).read_text(encoding="utf-8"))
    if gradient.get("decision") != "boundary_weight_feasible_for_one_train_only_microfit" or not gradient.get("ok") or gradient.get("optimizerSteps") != 0:
        raise ValueError("边界权重零训练梯度门未过")
    previous_report = json.loads(Path(plan["inputs"]["previousMicrofitReport"]["path"]).read_text(encoding="utf-8"))
    if previous_report.get("decision") != "soft_boundary_iou_train_only_microfit_fail_no_full_pilot" or previous_report.get("completedOptimizerSteps") != 256:
        raise ValueError("前序微拟合失败身份不符")
    previous_plan = json.loads(Path(plan["inputs"]["previousMicrofitPlan"]["path"]).read_text(encoding="utf-8"))
    for fixed in ("prerequisites", "dataset", "architecture", "training", "gates", "reachability"):
        if plan[fixed] != previous_plan[fixed]:
            raise ValueError(f"唯一变量以外的冻结合同漂移：{fixed}")
    old_loss = {key: value for key, value in previous_plan["loss"].items() if key != "boundaryWeight"}
    new_loss = {key: value for key, value in plan["loss"].items() if key != "boundaryWeight"}
    if old_loss != new_loss or float(previous_plan["loss"]["boundaryWeight"]) != 0.2 or float(plan["loss"]["boundaryWeight"]) != 1.0:
        raise ValueError("边界损失唯一变量不是0.2→1.0")
    if plan["singleChangedVariable"] != {"name": "boundaryWeight", "baseline": 0.2, "candidate": 1.0}:
        raise ValueError("预注册唯一变量声明不符")
    ...
    report = {"schemaVersion": 1, "ok": passed,
              "decision": "boundary_weight_train_only_microfit_pass" if passed else "boundary_weight_train_only_microfit_fail_no_full_pilot",
...
def verify(path: Path) -> dict[str, Any]:
    ...
    expected_decision = "boundary_weight_train_only_microfit_pass" if passed else "boundary_weight_train_only_microfit_fail_no_full_pilot"
    if (passed != report["ok"] or report["decision"] != expected_decision or
            report["completedOptimizerSteps"] != report["training"]["microfitOptimizerSteps"] or
            report["completedEpochs"] != report["training"]["microfitEpochs"]):
        raise ValueError("微型过拟合裁决不一致")
    return {"ok": True, "decision": "verified_boundary_weight_train_only_microfit", "microfitDecision": report["decision"], "metrics": metrics}
