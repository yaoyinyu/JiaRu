...
const reportPath = path.join(metadataDir, "phase1-readiness.json");
...
async function main() {
  ...
  const testNegativeCount = testSources.filter((record) => record.negative || record.originType === "negative").length;
  ...
  if (!gates.testSplitHasComplexBackground.ok) warnings.push("test split does not yet contain a complex-background sample");
