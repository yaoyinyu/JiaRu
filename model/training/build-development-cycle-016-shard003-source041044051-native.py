#!/usr/bin/env python3
"""Create isolated, untinted original-pixel review crops for shard-003 sources."""
...
TARGETS = (41, 44, 51)
EXPECTED_NAILS = {41: 5, 44: 10, 51: 2}
...
def inputs(shard_path: Path, triage_path: Path) -> tuple[dict, dict]:
    ...
    if [row["ordinal"] for row in triage["sourceTriage"] if row["decision"] == "needs_second_source_review"] != list(TARGETS):
        raise ValueError("二审源图清单漂移")
    return shard, triage
...
def verify_images(report: dict) -> None:
    if report["counts"] != {"sourceImages": 3, "oldLabelNails": 17, "visualApprovals": 0}:
    ...
    for ordinal in TARGETS:
        ...
        for index, (nail, polygon) in enumerate(zip(source["nails"], polygons, strict=True), start=1):
            ...
            nails.append({"truthIndex": index, "cropBox": box, "raw3x": bound(raw_path),
                          "oldOutline3x": bound(outline_path), "visualDecision": "pending_second_source_review"})
        records.append({"ordinal": ordinal, "sourceFileName": source["sourceFileName"],
                        ...
                        "nails": nails, "sourceQualityDecision": "pending_second_source_review"})
    report = {"schemaVersion": 1, "ok": True,
              "decision": "isolated_original_resolution_crops_pending_visual",
              "inputs": {"sourceScript": bound(Path(__file__)),
                         "shardReport": bound(shard_path), "sourceTriage": bound(triage_path)},
              "counts": {"sourceImages": 3, "oldLabelNails": 17, "visualApprovals": 0},
              "sources": records,
...
def verify(report_path: Path) -> dict:
    ...
    if report["decision"] != "isolated_original_resolution_crops_pending_visual":
    ...
    for row in report["sources"]:
        ...
        if (row["sourceFileName"] != source["sourceFileName"] or
                ...
                row["sourceQualityDecision"] != "pending_second_source_review" or
                any(nail["visualDecision"] != "pending_second_source_review" for nail in row["nails"])):
            raise ValueError("裁块报告源图身份或决策漂移")
    verify_images(report)
    return {"ok": True, "decision": "verified_isolated_original_resolution_crops_pending_visual",
