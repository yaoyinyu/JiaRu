...
def validate_plan(plan: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], Path]:
    ...
    if ninth.get("decision") not in {"roi_dinov2_local_boundary_refiner_internal_pilot_pass", "roi_dinov2_local_boundary_refiner_internal_pilot_fail"} or ninth.get("completedEpochs") != 30:
        raise ValueError("第九轮完整冻结报告不一致")
    ...
    return seventh, ninth, prior, Path(dataset_report["outputDir"])
...
def compute(plan: dict[str, Any]) -> dict[str, Any]:
    seventh_report, ninth_report, prior_report, dataset_root = validate_plan(plan)
    ...
    seventh_architecture = load_module("audit-development-cycle-016-dinov2-highres-decoder-feasibility.py", "cycle016_local_refiner_gain_seventh_architecture")
    seventh_trainer = load_module("train-development-cycle-016-soft-boundary-f1-pilot.py", "cycle016_local_refiner_gain_seventh_trainer")
    ninth_architecture = load_module("audit-development-cycle-016-local-boundary-refiner-feasibility.py", "cycle016_local_refiner_gain_ninth_architecture")
    ninth_trainer = load_module("train-development-cycle-016-local-boundary-refiner-pilot.py", "cycle016_local_refiner_gain_ninth_trainer")
    ...
    seventh_model = load_frozen_model(seventh_report, Path(plan["inputs"]["seventhWeights"]["path"]), seventh_architecture, seventh_trainer, device)
    ninth_model = load_frozen_model(ninth_report, Path(plan["inputs"]["ninthWeights"]["path"]), ninth_architecture, ninth_trainer, device)
    ...
    try:
        for images, truth, identities in loader:
            ...
            with torch.amp.autocast(device_type="cuda"):
                ...
                ninth_logits = ninth_model(pixel_values=images_device)
            seventh_predictions = (torch.sigmoid(seventh_logits) >= 0.5).cpu().numpy()[:, 0]
            ninth_predictions = (torch.sigmoid(ninth_logits) >= 0.5).cpu().numpy()[:, 0]
            truths = truth.numpy()[:, 0] >= 0.5
            for identity, target, seventh_prediction, ninth_prediction in zip(identities, truths, seventh_predictions, ninth_predictions, strict=True):
                key = str(identity)
                seventh_value = metrics(target, seventh_prediction, base)
                ninth_value = metrics(target, ninth_prediction, base)
                rows.append({"id": key, "localBoundaryContrast": contrast_by_id[key], "lowContrastQuartile": key in low_ids, "seventh": seventh_value, "ninth": ninth_value, "delta": {name: ninth_value[name] - seventh_value[name] for name in seventh_value}})
    finally:
        del seventh_model, ninth_model
    ...
    ninth_aggregate = aggregate(rows, "ninth", thresholds)
    ...
    ninth_fidelity = fidelity(ninth_aggregate, ninth_report["best"]["validation"], tolerance)
    if [row["id"] for row in rows] != expected_ids or not seventh_fidelity["ok"] or not ninth_fidelity["ok"]:
        return {"decision": "reconstruction_mismatch", "idOrderMatches": [row["id"] for row in rows] == expected_ids, "seventhAggregate": seventh_aggregate, "ninthAggregate": ninth_aggregate, "seventhFidelity": seventh_fidelity, "ninthFidelity": ninth_fidelity}
    ...
    mean_gain = float(ninth_aggregate["meanBoundaryF1"] - seventh_aggregate["meanBoundaryF1"])
    iou_delta = float(ninth_aggregate["meanIou"] - seventh_aggregate["meanIou"])
    ...
    boundary_iou_pass_fraction = ninth_aggregate["instanceGateCounts"]["boundaryIou"] / required
    architecture_limited = ninth_aggregate["meanBoundaryF1"] < float(contract["architectureLimitedMeanBoundaryF1Ceiling"]) or boundary_iou_pass_fraction < float(contract["architectureLimitedMaximumBoundaryIouPassFraction"])
    if meaningful and architecture_limited:
        decision = "local_boundary_refiner_effective_but_quality_gate_limited"
    elif meaningful:
        decision = "local_boundary_refiner_effective_keep_branch"
    else:
        decision = "local_boundary_refiner_not_broadly_effective_close_branch"
    integrity = materializer.verify(Path(plan["inputs"]["datasetMaterializationReport"]["path"]))
    return {
        "decision": decision,
        "idOrderMatches": True,
        "seventhAggregate": seventh_aggregate,
        "ninthAggregate": ninth_aggregate,
        "seventhFidelity": seventh_fidelity,
        "ninthFidelity": ninth_fidelity,
        ...
        "remainingGap": {"meanBoundaryF1ToGate": float(0.9 - ninth_aggregate["meanBoundaryF1"]), "jointInstancePassRateToGate": float(0.8 - ninth_aggregate["jointInstancePassRate"])},
...
def verify(report_path: Path) -> dict[str, Any]:
    ...
    if replay != report["analysis"] or canonical_sha256(replay) != report["analysisSha256"]:
        raise ValueError("局部边界残差修正器增益审计重放不一致")
    if not report.get("ok") or report.get("decision") == "reconstruction_mismatch":
        raise ValueError("局部边界残差修正器增益审计保真门未通过")
