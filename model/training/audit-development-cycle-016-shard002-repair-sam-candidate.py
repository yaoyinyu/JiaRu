#!/usr/bin/env python3
"""Audit an isolated shard-002 SAM repair candidate and freeze native crops."""
...
def build(prompts_path: Path, sam_path: Path, crop_dir: Path,
    ...
    if (prompts["decision"] != "shard002_sam_candidates_only" or
    ...
    for binding in prompts["inputs"].values():
        if isinstance(binding, list):
            for item_binding in binding:
                checked(item_binding)
        else:
            checked(binding)
    ...
    if (sha(source_path) != item["sourceImageSha256"] or
            prompts["inputs"]["sourceImage"]["sha256"] != item["sourceImageSha256"]):
    ...
    spec = json.loads(Path(prompts["inputs"]["manualPromptSpec"]["path"]).read_text(encoding="utf-8"))
    if (image.size != overlay.size or [width, height] != spec["expectedDimensions"] or
            spec["sourceOrdinal"] != prompts["sourceOrdinal"]):
    ...
    for index, nail in enumerate(annotation["annotations"], start=1):
        ...
        polygon_valid = shape.is_valid and shape.area > 16
        shapes.append(shape)
        ...
        for suffix, source_image in (("source-native", image), ("candidate-native", overlay)):
            path = crop_dir / f"source{prompts['sourceOrdinal']:03d}-nail-{index:02d}-{suffix}.png"
        ...
        rows.append({"truthIndex": index, "polygonAreaPixels": round(shape.area, 3),
                     "polygonValid": polygon_valid,
                     "invalidReason": None if polygon_valid else explain_validity(shape),
                     "cropBox": list(box), "positivePointsInside": all(
                         shape.covers(Point(x * width, y * height)) for x, y in positive)
                         if polygon_valid else False,
    ...
    overlap_pairs = sum(shapes[i].intersection(shapes[j]).area > 1e-10
                        ...
                        if rows[i]["polygonValid"] and rows[j]["polygonValid"])
    legal_count = sum(row["polygonValid"] for row in rows)
    point_inside = sum(row["positivePointsInside"] for row in rows)
    point_outside = sum(row["negativePointsOutside"] for row in rows)
    geometry_pass = legal_count == 5 and overlap_pairs == 0 and point_inside == 5 and point_outside == 5
    return {"schemaVersion": 1, "ok": True,
            "decision": ("shard002_sam_candidate_geometry_review_visual_pending"
                         if geometry_pass else "shard002_sam_candidate_geometry_rejected"),
            ...
            "counts": {"sources": 1, "nails": 5, "legalPolygons": legal_count,
                       "overlapPairs": overlap_pairs,
                       "positivePointRowsInside": point_inside,
                       "negativePointRowsOutside": point_outside,
                       "visualApproved": 0, "trainingApproved": 0},
            "sourceOrdinal": prompts["sourceOrdinal"],
