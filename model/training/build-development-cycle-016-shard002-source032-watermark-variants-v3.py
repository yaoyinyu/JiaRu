#!/usr/bin/env python3
"""Build source32 corner-logo ablation variants with residual-only relocation.
...
def build(preflight_path: Path, output_dir: Path, *, verify: bool) -> dict:
    ...
    residual = (source[ys, xs].astype(np.float32) -
                removed[ys, xs].astype(np.float32))
    moved[target_y, target_x] = np.rint(
        np.clip(removed[target_y, target_x].astype(np.float32) +
                residual * alpha, 0, 255)
    ).astype(np.uint8)
