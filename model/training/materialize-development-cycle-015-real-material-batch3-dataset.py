#!/usr/bin/env python3
"""在循环015 batch2开发数据集上只追加batch3的30张图209 mask，并生成可重放物化报告。

单变量短训实验专用（用户2026-09-19『继续训练，不要停』指令；沿用三条裁决：甲面级过滤、裁断输出通道、免审直接训练）。
基础 = 2026_9_19_cycle015_real_material_batch2_development_dataset_v1（datasetFilesSha256 2cd219c7…801b）。
...
偏差声明：batch3 真值为模型预标注多边形（用户明确裁决免审直接训练并继续训练循环），
...
EXPECTED_REAL_IMAGES = 30
EXPECTED_REAL_MASKS = 209
FOLD_NAME = "cycle015-real-material-batch3-positive"
DATASET_VERSION_OLD = "development-cycle-015-real-material-batch2/v1"
DATASET_VERSION_NEW = "development-cycle-015-real-material-batch3/v1"
FROZEN_IMAGE_DIR = Path(r"E:/AI Project/Codex/JiaRu_image/真实素材/2026_9_19_cycle015_real_material_v3")
WORKSPACE = Path(r"E:/AI Project/Codex/JiaRu_image/审核工作区/2026_9_19_cycle015_real_material_annotation_workspace_v5")
...
BATCH2_MODULE_PATH = Path(r"E:/AI Project/Codex/JiaRu/model/training/materialize-development-cycle-015-real-material-batch2-dataset.py")
...
def load_batch3_truths() -> list[dict[str, Any]]:
    ...
    for entry in report.get("items") or []:
        ...
        if expected_masks <= 0:
            raise ValueError(f"过滤账目为零掩码：{name}")
    ...
    if len(truths) != EXPECTED_REAL_IMAGES:
        raise ValueError(f"batch3条目数不符：期望{EXPECTED_REAL_IMAGES}，实际{len(truths)}")
    total = sum(item["maskCount"] for item in truths)
    if total != EXPECTED_REAL_MASKS:
        raise ValueError(f"batch3 mask总数不符：期望{EXPECTED_REAL_MASKS}，实际{total}")
    if len({item["imageSha256"] for item in truths}) != EXPECTED_REAL_IMAGES or len({item["sourceGroup"] for item in truths}) != EXPECTED_REAL_IMAGES:
        raise ValueError("batch3身份不唯一")
    return truths


def load_batch2_materializer():
    spec = importlib.util.spec_from_file_location("materialize_development_cycle_015_real_material_batch2_dataset_for_training", BATCH2_MODULE_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot load cycle015 batch2 real-material materializer")
...
def build_report(base_path: Path, output_dir: Path, *, materialize: bool) -> dict[str, Any]:
    base = read_object(base_path, "基础物化报告(cycle015 batch2)")
    if (
        base.get("ok") is not True
        or base.get("decision") != "development_cycle_015_real_material_batch2_dataset_materialized_for_single_short_experiment"
    ...
    return {
        ...
        "ok": True,
        "status": "PASS",
        "decision": "development_cycle_015_real_material_batch3_dataset_materialized_for_single_short_experiment",
        ...
        "inputs": {
            ...
            "freezeManifestV3": binding(FROZEN_IMAGE_DIR / "freeze-manifest-v3.json"),
            ...
            "truthProvenanceDeviation": {
                ...
                "new": "batch3：YOLO(cycle012 best)预标注多边形直接作为训练真值，conf≥0.6 + bbox边距≥8px统一过滤（无白名单），30图全取自新文件夹2",
            ...
            "gateDeviationPreRegistered": {
                ...
                "new": "+30 batch3真实素材图/+209 mask在batch2数据集上第三次单变量短训（累计真值94/90已越过90图账）",
...
def verify_report(report_path: Path) -> dict[str, Any]:
    """重放batch3物化报告（含batch2基底报告强校验），供训练入口绑定同一文件树身份。"""

    report_path = report_path.resolve()
    existing = read_object(report_path, "待重放报告")
    inputs = existing.get("inputs") or {}
    load_batch2_materializer().verify_report(Path(inputs["baseMaterialization"]["path"]).resolve())
    replay = build_report(
