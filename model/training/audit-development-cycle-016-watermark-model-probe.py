#!/usr/bin/env python3
"""Fixed 512-input, one-pass source-7 watermark sensitivity probe.
...
def build(variant_report_path: Path, weight_path: Path) -> dict:
    variants = json.loads(variant_report_path.read_text(encoding="utf-8"))
    if not variants["ok"] or variants["nailPixelsChanged"] != 0 or variants["sourceOrdinal"] != 7:
    ...
    return {"schemaVersion": 1, "ok": True,
            "decision": "single_train_source_watermark_model_probe_diagnostic_only",
            "inputs": {"sourceScript": bind(Path(__file__)),
                       "variantBuild": bind(variant_report_path), "frozenWeight": bind(weight_path)},
...
def main() -> None:
    ...
    if args.verify_report:
        ...
        return
    if not all((args.variant_report, args.weight, args.report)):
        parser.error("--variant-report, --weight, --report required")
    if args.report.exists():
        raise FileExistsError(args.report)
    report = build(args.variant_report.resolve(), args.weight.resolve())
