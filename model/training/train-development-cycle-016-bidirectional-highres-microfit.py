#!/usr/bin/env python3
"""双向高分辨率表示的固定32例train-only微型过拟合门；不读取val。"""
...
def train(plan_path: Path, output: Path) -> dict[str, Any]:
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_index, (images, truth, _) in enumerate(train_loader, start=1):
            ...
            with torch.amp.autocast(device_type="cuda"):
                logits = model(images)
            loss, _ = loss_module.soft_boundary_f1_loss(logits, truth, plan["loss"])
    ...
    report = {"schemaVersion": 1, "ok": passed,
              "decision": "bidirectional_highres_train_only_microfit_pass" if passed else "bidirectional_highres_train_only_microfit_fail_no_full_pilot",
...
def verify(path: Path) -> dict[str, Any]:
    ...
    return {"ok": True, "decision": "verified_bidirectional_highres_microfit", "microfitDecision": report["decision"], "metrics": metrics}
