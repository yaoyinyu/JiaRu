#!/usr/bin/env python3
"""训练cycle016第七轮DINOv2可微软Boundary F1监督pilot。"""
...
def validate_inputs(plan: dict[str, Any]) -> tuple[dict[str, Any], Path, Path]:
    ...
    for label, path_key, hash_key in (
        ...
        ("软边界F1可达性计划", "softBoundaryFeasibilityPlan", "softBoundaryFeasibilityPlanSha256"),
        ("软边界F1可达性报告", "softBoundaryFeasibilityReport", "softBoundaryFeasibilityReportSha256"),
    ...
    feasibility = json.loads(Path(prerequisite["softBoundaryFeasibilityReport"]).read_text(encoding="utf-8"))
    if feasibility.get("decision") != "soft_boundary_f1_supervision_reachable_for_seventh_pilot" or not feasibility.get("ok"):
        raise ValueError("软边界F1可达性门未通过")
...
def train(plan_path: Path, output: Path) -> dict[str, Any]:
    ...
    result = {
        "schemaVersion": 1,
        "ok": passed,
        "decision": "roi_dinov2_soft_boundary_f1_internal_pilot_pass" if passed else "roi_dinov2_soft_boundary_f1_internal_pilot_fail",
        "scope": plan["scope"],
        "inputs": {
            ...
            "softBoundaryFeasibilityPlan": {"path": plan["prerequisites"]["softBoundaryFeasibilityPlan"], "sha256": plan["prerequisites"]["softBoundaryFeasibilityPlanSha256"]},
            "softBoundaryFeasibilityReport": {"path": plan["prerequisites"]["softBoundaryFeasibilityReport"], "sha256": plan["prerequisites"]["softBoundaryFeasibilityReportSha256"]},
        ...
        "architecture": {"name": "DINOv2-small plus unchanged 384/192/96 spatial-skip decoder with training-only soft Boundary F1 supervision", "parameterCount": parameter_count, "outputStride": 1, "singleChangedVariable": plan["singleChangedVariable"]},
        "training": config,
...
def verify(report_path: Path) -> dict[str, Any]:
    ...
    materializer = load_module("materialize-development-cycle-016-roi-dataset-v2.py", "cycle016_soft_boundary_verify_materializer")
    ...
    return {"ok": True, "decision": "verified_roi_dinov2_soft_boundary_f1_pilot", "pilotDecision": report["decision"], "best": report["best"]}
