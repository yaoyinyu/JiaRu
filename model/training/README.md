...
3. 新批文件必须使用`hard_negative_training_YYYYMMDD_NNN_family_VV`，序号在声明区间内连续，最短边不少于768像素。旧`hard_negative_ai_*`、`hard_negative_independent_*`或`nail_*`命名只允许在registry绑定的历史受保护清单中兼容，不能用于新训练批次。
   ...
   - `build-positive-recognition-quality-report.py`把冻结正样本的总体mAP拆为逐图/逐甲强门：绑定snapshot、evaluation materialization、预测制品、权重和规范阈值，重算匹配、漏甲、完整mask、重复、额外候选及预测拓扑；`--verify-report`重放全部输入。默认`--gate-mode weighted`（schema v2）在保留召回≥0.90、完整mask≥0.85、缺甲图率≤0.10等结构门的同时，以严重度加权伪实例率`（1.0×重复+1.5×无效+2.0×额外）/真值数 ≤ --max-weighted-spurious-rate`（默认0.02）取代v1三个互相矛盾的零缺陷门，零缺陷状态保留在`zeroDefectDiagnostics`诊断字段；`--gate-mode zero-defect`完整复刻历史v1八门语义，仅用于重放既有v1报告，`--verify-report`按报告`schemaVersion`自动分派。该工具只评估冻结证据，禁止据此调整阈值或把test图片回流训练。
