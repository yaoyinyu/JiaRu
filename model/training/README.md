...
3. 新批文件必须使用`hard_negative_training_YYYYMMDD_NNN_family_VV`，序号在声明区间内连续，最短边不少于768像素。旧`hard_negative_ai_*`、`hard_negative_independent_*`或`nail_*`命名只允许在registry绑定的历史受保护清单中兼容，不能用于新训练批次。
   ...
   - `build-positive-recognition-quality-report.py`把冻结正样本的总体mAP拆为逐图/逐甲强门：绑定snapshot、evaluation materialization、预测制品、权重和规范阈值，重算匹配、漏甲、完整mask、重复、额外候选及预测拓扑；`--verify-report`重放全部输入。默认schema v2保留至少100图、召回≥0.90、完整mask≥0.85、缺甲图率≤0.10等结构门，并以严重度加权伪实例率`（1.0×重复+1.5×无效+2.0×额外）/真值数 ≤ 0.02`约束伪实例。所有CLI门参数只能收紧、不能放宽；普通构建禁止`zero-defect`，历史schema v1只允许在`--verify-report`内部重放。零缺陷状态继续保留为诊断，不替代正式加权门。该工具只评估冻结证据，禁止据此调整阈值或把test图片回流训练。
...
Replay the first scoped speedup evidence with `audit-development-cycle-015-annotation-acceleration.py --verify-report <report>`. The first measured batch accepted 44/45 PCA-SAM boundaries after original-resolution review and required one residual manual edit; this result applies only to the exact-count subset and does not waive visual, topology, overlap, role-isolation, or materialization gates.

For rolling batches, `build-development-cycle-015-generated-annotation-workspace.py --existing-development-evaluation-truth <index>` removes already finalized source identities before copying the next increment and derives the cumulative intake decision for any count from 1 through 133. `generate-yolo-prelabels.py` installs the read-only Ultralytics image verifier before model loading; an inference run made without that guard cannot be canonical even if a later manifest replay shows no byte changes. Build three internal PCA points and bounded overprediction trimming with `build-pca-multipoint-sam-prompts.py`; trimming is candidate triage only, and every removed candidate must be confirmed at original resolution as a duplicate for an already represented nail with no additional nail instance removed from the denominator.

Replay the guarded incremental evidence with `audit-development-cycle-015-incremental-annotation-acceleration.py --verify-report <report>`. The second batch produced 60/60 reviewed SAM boundaries with zero manual boundary edits and brought the cumulative development-evaluation index to 23 images / 145 masks. This validates reduction of boundary drawing and rework for the observed batch; it does not establish an end-to-end release-date reduction until collection, machine processing, original-resolution review, and residual repair are separately timed on a subsequent batch.

Replay the timed difficult-batch evidence with `audit-development-cycle-015-batch3-annotation-acceleration.py --verify-report <report>`. Batch 3 accepted 51/60 SAM boundaries, used nine residual manual polygons, and brought the cumulative index to 33 images / 205 masks. Across the last two batches, 111/120 boundaries passed without manual drawing. After one targeted SAM repair still fails original-resolution review, switch the residual boundary to manual polygon instead of repeating inference. The measured machine stage was 39.290 seconds and the final 60-mask review was 99.883 seconds; active manual drawing time is intentionally unknown because the task was suspended.

Replay batch 4 with `audit-development-cycle-015-batch4-annotation-acceleration.py --verify-report <report>`. Only images with exact coarse-candidate counts, or at most two overpredictions confirmed at original resolution as non-nail regions, enter the bulk PCA multipoint SAM path. An undercount keeps the full truth denominator and permits one targeted recovery attempt; failure routes the whole image to original-resolution manual polygons. Batch 4 accepted 55/55 bulk SAM boundaries and used five manual polygons for one undercount image, bringing the cumulative development-evaluation index to 43 images / 265 masks. Across the last three batches, 166/180 final boundaries avoided manual polygon authoring. The canonical machine stages took 44.896 seconds; the observed 248.534-second final-review window includes evidence-build and correction iterations and is not an active-labor benchmark.
