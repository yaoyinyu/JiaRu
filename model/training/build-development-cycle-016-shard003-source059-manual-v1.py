#!/usr/bin/env python3
"""Build a five-nail source-59 correction candidate from bound original pixels."""
...
def build(native_path: Path, progress_path: Path, first_visual_path: Path,
    ...
    polygons = [[(float(x), float(y)) for x, y in MANUAL_N1], *old[1:]]
    ...
    annotation = {
        "version": "shard003-source059-historical-correction-candidate/v1",
        ...
        "annotations": [
            {"id": f"n{index}", "label": "nail_texture",
             ...
             "method": "manual_original_resolution" if index == 1 else "retained_old"}
