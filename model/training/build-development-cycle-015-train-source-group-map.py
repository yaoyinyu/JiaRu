#!/usr/bin/env python3
"""从循环015真实素材数据集物化报告构建训练折叠源组映射（balanced sampler 输入）。

镜像 cycle012 group map 结构：images 按文件名排序，counts 记录 trainImages/sourceGroups。
"""
...
def main() -> int:
    ...
    if (
        ...
        or report.get("decision") != "development_cycle_015_real_material_dataset_materialized_for_single_short_experiment"
    ):
        raise ValueError("物化报告未通过循环015门禁")
