...
DECISIONS = {
    ...
    34: ("pending_every_nail_review", "原图有四枚完整可见甲面，第五指甲未露；四mask仍需逐甲审核。", []),
...
def build(shard_path: Path) -> dict:
    ...
    if status_counts != {"pending_every_nail_review": 10, "confirmed_label_rework": 6,
                         "confirmed_source_exclude": 2, "watermark_review_required": 2}:
        raise ValueError("source triage status drift")
    return {"schemaVersion": 1, "evidenceOk": True,
            "decision": "shard002_source_triage_only_no_mask_approval",
