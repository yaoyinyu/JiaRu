#!/usr/bin/env python3
"""训练 cycle016 带全分辨率边界细化头的逐甲 ROI pilot。"""
...
class RefinedSegformer(nn.Module):
    ...
    def forward(self, pixel_values: torch.Tensor) -> torch.Tensor:
        coarse = self.backbone(pixel_values=pixel_values).logits
        coarse = F.interpolate(coarse, size=pixel_values.shape[-2:], mode="bilinear", align_corners=False)
        return coarse + self.refine(torch.cat((pixel_values, coarse), dim=1))


def build_model(seed: int) -> RefinedSegformer:
    ...
    return RefinedSegformer(config)
...
def validate(model: RefinedSegformer, loader: DataLoader, device: torch.device) -> dict[str, float]:
    model.eval()
    losses: list[float] = []
    ious: list[float] = []
...
def main() -> int:
    ...
    plan = json.loads(plan_path.read_text(encoding="utf-8"))
    dataset_report = json.loads(Path(plan["dataset"]["materializationReport"]).read_text(encoding="utf-8"))
    dataset_root = Path(dataset_report["outputDir"])
    install_guard, remove_caches = load_guards()
    install_guard()
    remove_caches(dataset_root)
    try:
        report = train(plan_path, Path(args.output_dir).resolve())
    finally:
        remove_caches(dataset_root)
