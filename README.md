...
- **循环012 为当前唯一活动里程碑**：固定13张/13个新来源组/65-mask 真实困难正图，计划 SHA-256 `94869841…3e3e`，真值物化前训练启动门保持关闭，最多运行1次短程实验。
...
- candidate57 为当前 TEST HOLD 候选（冻结test100实例召回率`0.95848375`通过，漏检图片率`0.13`与加权杂散率`0.04693141`失败），生产 ONNX 仍未导出；当前主线为来源隔离的预注册开发循环，循环012正在进行中，详见路线图 Phase 4
...
| 文档 | 说明 |
| --- | --- |
| [技术白皮书](docs/technical-whitepaper.md) v1.1.628 | 模块状态、接口契约、使用方式、已知限制——项目唯一总入口 |
...
- [ ] 完成循环012固定13张/13个新来源组/65-mask真实困难正图的源图筛选、原分辨率终审和隔离物化；通过启动门后只运行1次短程实验
