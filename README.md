...
| 隐私政策 | `/privacy` | ✅ | 完整静态说明页：核心原则（本地优先/不上传照片/不追踪）+ 各功能数据流向（试色/AI/AR/识别）+ 第三方服务与用户权利；法律合规审核仍待外部 |
...
- candidate7 已形成80张/487个严格完整甲面真值（旧真实33张/237 mask、新正样本47张/250 mask，两索引按文件名零重叠），与当前train120、val30、冻结test100保持图片身份隔离；未通过原分辨率完整甲面门或存在应见甲数冲突的返修项均不计数。新增不少于80张目标按严格真值已达成；完成精确授权与物化审计前，全批继续禁止训练。
...
- candidate7 当前严格真值为80张/487 mask（旧真实33张/237 mask、新正样本47张/250 mask），80张目标已达成；下一步完成精确训练清单、授权和物化输入审计
...
完整说明见 [`/privacy`](http://localhost:3000/privacy)（最后更新 2026-08-15）。AI 生图会把你的文字描述转发给第三方图像生成服务（Agnes AI），生成结果由浏览器直接访问第三方图片地址；描述中请勿输入身份证号、电话等个人信息。项目不使用 Cookie 追踪、无账户体系、无云同步，联系邮箱 `3181484805@qq.com`。
...
- [ ] 通过冻结test100逐实例完整识别门（candidate6已否决，需下一候选）
