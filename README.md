...
| 文档 | 说明 |
| --- | --- |
| [技术白皮书](docs/technical-whitepaper.md) v1.1.681 | 模块状态、接口契约、使用方式、已知限制——项目唯一总入口 |
...
- [ ] 循环016 下一活动：冻结端到端高分辨率表示计划——DINOv2-small 语义 token + 384/192/96 三条 `64/96/128` 通道空间分支、两次双向跨尺度融合与全分辨率残差边界塔；须先过参数/显存、全梯度、精确静态 ONNX、ORT CPU 与真实 Chromium WASM/WebGPU 门，再用预注册 train-only 32 实例微型过拟合，Boundary F1/联合率未达 `0.90/0.80` 即关闭且不跑全量；禁止复用第七/九轮 best 初始化，v2 全量通过前禁止阈值校准，旧 val/test/holdout 禁止训练或调参，产品继续 HOLD
