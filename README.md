...
- 请求安全（2026-09-24 安全审计加固）：按 IP 限流中间件（验证码 30/5min、请求验证码 5/10min、验证 20/10min、换绑 10/10min、OAuth 起点 20/10min，阈值可经 `JIARU_RL_*` 覆盖）；对 `/api/*` 状态变更请求强制 Origin 校验（CSRF，白名单含 `JIARU_ALLOWED_ORIGINS`）；生图与认证错误响应统一脱敏不再回显内部异常；换绑接口先消费验证码再判号码归属，消除注册状态枚举
...
| 文档 | 说明 |
| --- | --- |
| [技术白皮书](docs/technical-whitepaper.md) v1.1.714 | 模块状态、接口契约、使用方式、已知限制——项目唯一总入口 |
