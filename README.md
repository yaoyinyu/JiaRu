...
- **循环014 已实测关闭，当前主线为循环015 全新正图供给**：循环014 多裁片评估在零训练实测中把杂散率从`0.0960452`恶化到`0.20762712`（假阳性10→28），固定网格按合同关闭——**扩大候选面会反向恶化杂散**，扩容必须与杂散控制一起设计。循环015 转向数据供给线：库存重隔离后旧333图仅7图可用、复核后3图/3来源组/26枚预期甲面入队，首个内置生成正图冻结批次3图/3来源组/20枚甲面（与全部现有角色交叠0），**133张源图合格目标进度3/133**，总最低新批准正图缺口424图。
...
- candidate57 为当前 TEST HOLD 候选（冻结test100实例召回率`0.95848375`通过，漏检图片率`0.13`与加权杂散率`0.04693141`失败），生产 ONNX 仍未导出；循环012—014均已按预注册合同失败或证伪关闭，当前主线为循环015首批50张来源隔离正图供给，详见路线图 Phase 4
...
| 文档 | 说明 |
| --- | --- |
| [技术白皮书](docs/technical-whitepaper.md) v1.1.645 | 模块状态、接口契约、使用方式、已知限制——项目唯一总入口 |
...
- [x] 循环015 数据供给线：库存重隔离（旧333图仅7图无当前角色交叠，复核后3图/26枚入队）、唯一恢复图返修止损（`01063`排除、可返修容量归零、总缺口424图）、提速漏斗验证（源图通过率`0.48456261`、真值批准率`0.75`，活动里程碑=133张源图合格候选）、首个内置生成正图冻结批次（3图/3来源组/20枚甲面，与全部现有角色交叠0）——**进度3/133**，下一步再冻结10张至13/133
- [ ] 循环015 继续：再生成、原分辨率审核并冻结10张（进度至13/133），每个冻结来源组立即进入完整mask流水线；训练启动门维持关闭
