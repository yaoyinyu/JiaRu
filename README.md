...
- **循环014 已实测关闭，当前主线为循环015 来源组数据流水线**：循环014 多裁片评估在零训练实测中把杂散率从`0.0960452`恶化到`0.20762712`（假阳性10→28），固定网格按合同关闭。循环015首批13图/13来源组/85甲已完成完整mask终审，开发评估真值索引为13图/85 mask、0冲突且训练禁用；计数正确子集以PCA长轴多正点SAM取得44/45可接受边界，仅1甲人工修边。**当前进度为13/133源图、13/90开发评估真值**，下一批端到端推进至23/133与23/90，训练门继续关闭。
...
- candidate57 为当前 TEST HOLD 候选（冻结test100实例召回率`0.95848375`通过，漏检图片率`0.13`与加权杂散率`0.04693141`失败），生产 ONNX 仍未导出；循环012—014均已按预注册合同失败或证伪关闭，当前主线为循环015至少133张来源隔离合格源图和90张开发评估真值，详见路线图 Phase 4
...
| 文档 | 说明 |
| --- | --- |
| [技术白皮书](docs/technical-whitepaper.md) v1.1.651 | 模块状态、接口契约、使用方式、已知限制——项目唯一总入口 |
...
- [x] 循环015 首批真值终审：13图/85甲全部通过逐甲原分辨率、合法性和零交叠门；计数正确子集PCA长轴多正点SAM接受44/45，仅1甲人工修边；开发评估真值13/90、训练禁用
- [ ] 循环015 继续：新增10个独立来源组，并以PCA多点SAM加残差人工修边端到端推进至23/133源图与23/90开发评估真值；复杂图继续混合返修，训练启动门维持关闭
