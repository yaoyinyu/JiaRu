...
- candidate7 已形成57张/367个严格完整甲面真值（旧真实33张/237 mask、新正样本24张/130 mask），与当前train120、val30、冻结test100保持图片身份隔离；另有20张旧真实返修项仍未通过最终真值门，不计入正式数量。距新增不少于80张目标按严格真值仍缺23张；完成剩余返修、精确授权与物化审计前，全批禁止训练。
...
- candidate7 当前严格真值为57张/367 mask，另有20张旧真实返修项待逐甲终审；下一步至少再形成23张严格真值并完成精确训练清单、授权和物化输入审计
