...
- 下一主线是candidate8定向补强整图多甲召回和完整mask；只有先通过冻结test100逐实例门，才另建不少于100张训练后全新未见困难负样本并执行部署512三变体零误检/零delta审计。
...
- candidate7 已完成训练与冻结test100，因逐实例完整识别门失败而否决；candidate8将从全新来源定向补强漏甲和不完整mask
...
- [ ] 通过冻结test100逐实例完整识别门（candidate6、candidate7均已否决，需candidate8）
