...
- **下一活跃里程碑为循环014**：依判决重新设计候选供给侧单变量并预注册；训练启动门维持关闭。Wilson方法论结论：58图规模下“少漏1—2张图”的相对信号均在噪声内，分辨0.2241与0.1724需每组约932张正图，任何相对门结论必须附Wilson区间与最小可判别差。
...
- candidate57 为当前 TEST HOLD 候选（冻结test100实例召回率`0.95848375`通过，漏检图片率`0.13`与加权杂散率`0.04693141`失败），生产 ONNX 仍未导出；循环012已按预注册合同失败关闭，当前主线为循环013两阶段mask替换原型，详见路线图 Phase 4
...
| 文档 | 说明 |
| --- | --- |
| [技术白皮书](docs/technical-whitepaper.md) v1.1.635 | 模块状态、接口契约、使用方式、已知限制——项目唯一总入口 |
...
- [ ] 依循环013判决重新设计候选供给侧单变量并预注册（循环014），训练启动门维持关闭
