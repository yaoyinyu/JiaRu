...
- candidate24/25均已在val30否决：candidate24为128匹配/16漏检/27误检，candidate25最佳点仅与candidate21的128/16/19打平；candidate27三种候选复核器也未能在保持召回时降低误检，三者均未运行冻结test100、未导出或部署。当前改为candidate28“完整美甲识别优先”：新困难负样本计划暂停在70/160，训练时只使用既有批准集合的有限来源平衡子集；首张新增真值`01138…_7`使补强索引达到61张/424 mask。发布侧独立困难负样本三变体零误检门不变。
...
- [ ] 通过冻结test100逐实例完整识别门（candidate6、candidate7、candidate8均已否决）
